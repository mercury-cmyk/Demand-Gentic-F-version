...
WHERE
  ...
  OR (LENGTH(direct_phone_e164) < 11 AND direct_phone_e164 IS NOT NULL)
  OR (LENGTH(mobile_phone_e164) < 11 AND mobile_phone_e164 IS NOT NULL);
```

### Option 2: Fix Phone Numbers Manually

If these are real contacts with wrong data:

1. **Export the list** from the script output
2. **Look up correct phone numbers** from your source system
3. **Update in database**:
   ```sql
   UPDATE contacts
   SET direct_phone_e164 = '+441234567890'
   WHERE id = 'contact-id-here';
   ```
4. **Re-add to campaign queue** if needed

### Option 3: Null Out Invalid Numbers

Keep the contacts but remove invalid phone fields:

```sql
UPDATE contacts
SET direct_phone_e164 = NULL
WHERE LENGTH(direct_phone_e164) > 16
   OR direct_phone_e164 LIKE '%00000000%'
   OR LENGTH(direct_phone_e164) < 11;

UPDATE contacts
SET mobile_phone_e164 = NULL
WHERE LENGTH(mobile_phone_e164) > 16
   OR mobile_phone_e164 LIKE '%00000000%'
   OR LENGTH(mobile_phone_e164) < 11;
```

## Verifying Cleanup Worked

After cleanup, your campaign should work without phone format errors:

```bash
# Check for any remaining invalid phones in queue
npx tsx cleanup-invalid-phones.ts

# Should show: "✅ No invalid phone numbers found!"
```
...
When importing contacts, validate phone numbers:
...
function validateE164(phone: string): boolean {
  ...
  // Must be 11-16 characters total
  if (phone.length < 11 || phone.length > 16) return false;
