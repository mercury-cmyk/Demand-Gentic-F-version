...
Degraded Calls:              Target <5% of total
```
...
```bash
# View keepalive messages (every ~30s)
tail -f logs/production.log | grep "turn_complete\|keepalive"

# View audio quality reports
tail -f logs/production.log | grep "CALL QUALITY REPORT" -A 15

# Check for backpressure events
grep "Buffer backpressure" logs/production.log | wc -l

# Monitor reconnection attempts
tail -f logs/production.log | grep "Reconnect attempt"

# View all audio quality alerts
grep "⚠️\|❌" logs/production.log | grep -i audio
```
...
| Issue | First Check | Resolution |
|-------|-------------|-----------|
| Calls still dropping | Look for keepalive messages | Restart server if absent |
| Audio still distorted | Check backpressure events | Reduce MAX_BUFFER_SIZE if >3/call |
