...
async function analyzeAllJan15Calls() {
  ...
  const durationBuckets = {
    '20-30s': allCalls.filter(c => c.duration >= 20 && c.duration < 30).length,
    '30-60s': allCalls.filter(c => c.duration >= 30 && c.duration < 60).length,
    '60-90s': allCalls.filter(c => c.duration >= 60 && c.duration < 90).length,
    '90-120s': allCalls.filter(c => c.duration >= 90 && c.duration < 120).length,
    '120-180s': allCalls.filter(c => c.duration >= 120 && c.duration < 180).length,
    '180s+': allCalls.filter(c => c.duration >= 180).length,
  };
  ...
  Object.entries(durationBuckets).forEach(([bucket]) => {
    ...
    const bucketCalls = allCalls.filter(c => c.duration >= min && c.duration < (max || 9999));
    const transcribed = bucketCalls.filter(c => c.hasTranscript).length;
  ...
  if (transcriptionRate < 90) {
    console.log(`   ⚠️  Only ${transcriptionRate.toFixed(1)}% of calls transcribed`);
    console.log(`   → ${transcriptionGap} calls need transcription`);
    console.log(`   → Consider auto-transcription setup for future calls`);
  } else {
    console.log(`   ✅ ${transcriptionRate.toFixed(1)}% transcription rate (excellent!)`);
  }

  console.log('\n2. Voicemail Detection:');
  const vmAccuracy = voicemailCalls.filter(c => c.disposition === 'voicemail').length / Math.max(voicemailCalls.length, 1) * 100;
  if (vmAccuracy < 80) {
    console.log(`   ⚠️  Voicemail detection accuracy: ${vmAccuracy.toFixed(1)}%`);
    console.log(`   → Review voicemail detection settings`);
    console.log(`   → ${vmWithOtherDisp.length} calls detected as VM but different disposition`);
  } else {
    console.log(`   ✅ Voicemail detection accuracy: ${vmAccuracy.toFixed(1)}%`);
  }

  console.log('\n3. Call Duration:');
  const shortCalls = allCalls.filter(c => c.duration < 30).length;
  if (shortCalls > allCalls.length * 0.5) {
    console.log(`   ⚠️  ${((shortCalls / allCalls.length) * 100).toFixed(1)}% of calls are very short (<30s)`);
    console.log(`   → May indicate connection issues or quick hang-ups`);
    console.log(`   → Review call scripts and opening statements`);
  } else {
    console.log(`   ✅ Good engagement - ${((shortCalls / allCalls.length) * 100).toFixed(1)}% very short calls`);
  }

  console.log('\n4. Connection Rate:');
  const connectionRate = (connected.length / allCalls.length) * 100;
  if (connectionRate < 20) {
    console.log(`   ⚠️  Low connection rate: ${connectionRate.toFixed(1)}%`);
    console.log(`   → Consider calling at different times`);
    console.log(`   → Review phone number quality`);
  } else {
  ...
  console.log(`  Total Calls (>=20s): ${allCalls.length}`);
