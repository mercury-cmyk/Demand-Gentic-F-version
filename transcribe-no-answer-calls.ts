...
async function transcribeNoAnswerCalls() {
  ...
  for (const row of result.rows) {
    ...
    if (transcript.length < 10) {
      needsTranscription.push(row);
    } else {
      alreadyHasTranscript.push(row);
    }
  }

  console.log(`  Already has transcript: ${alreadyHasTranscript.length}`);
  console.log(`  Needs transcription:    ${needsTranscription.length}`);
  console.log(`  Has recording URL:      ${needsTranscription.filter(r => r.recording_url || r.recording_s3_key).length}`);
  ...
  if (needsTranscription.length > 0) {
    ...
    for (let i = 0; i < needsTranscription.length; i++) {
      const row = needsTranscription[i];
      const hasRecording = row.recording_url || row.recording_s3_key;

      console.log(`\n[${i + 1}/${needsTranscription.length}] ${row.first_name || 'Unknown'} ${row.last_name || ''} | ${row.company_name || 'N/A'} | ${row.duration_sec}s | ${row.campaign_name}`);

      if (!hasRecording) {
        console.log('  -> SKIP: No recording available');
      ...
      if (i < needsTranscription.length - 1) {
