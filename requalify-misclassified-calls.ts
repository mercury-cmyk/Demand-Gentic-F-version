...
async function findMisclassifiedCallsWithTranscripts(): Promise<MisclassifiedCall[]> {
  ...
  for (let i = 0; i < suspiciousCalls.length; i++) {
    const call = suspiciousCalls[i];
    process.stdout.write(`\rProcessing ${i + 1}/${suspiciousCalls.length}...`);
    ...
    // Check for transcript in leads table via telnyxCallId
    ...
    if (call.telnyxCallId) {
      ...
      if (lead) {
        leadId = lead.id;
        hasTranscript = !!lead.transcript && lead.transcript.trim().length > 0;
