...
async function getMercuryStatusCounts() {
  const result = await db.execute(sql`
    ...
    ORDER BY status;
  `);
  return ((result as any)?.rows ?? []) as Array<{ status: string; count: number }>;
}
...
async function main() {
  ...
  const rounds: Array<{ round: number; processed: number; succeeded: number; failed: number }> = [];

  for (let i = 1; i <= MAX_ROUNDS; i++) {
    const result = await mercuryEmailService.processOutbox(BATCH_SIZE);
    rounds.push({ round: i, ...result });
  ...
  const mercuryAfter = await getMercuryStatusCounts();
  const txAfter = await getTransactionalPendingCounts();

  const totalProcessed = rounds.reduce((sum, r) => sum + r.processed, 0);
