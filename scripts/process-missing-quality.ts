  ...
  for (const session of candidates) {
    ...
    try {
      ...
      const analysis = await analyzeConversationQuality({
        ...
        callDurationSeconds: session.duration || 0,
        disposition: session.aiDisposition || undefined,
        campaignId: session.campaignId || undefined,
        campaignName: campaign?.name || undefined,
        campaignObjective: campaign?.description || undefined,
        contactName: contact ? (contact.fullName || contact.firstName) : undefined,
