  ...
  try {
    ...
    for (let i = 0; i < leads.length; i++) {
      ...
      if (!transcript || transcript.trim().length < 20) {
        console.log(`[${i + 1}/${leads.length}] ${lead.contact_name || '?'} — skipped (no usable transcript)`);
        skipped++;
        continue;
      }

      console.log(`[${i + 1}/${leads.length}] ${lead.contact_name || '?'} | ${(lead.campaign_name||'?').substring(0,30)} | ${transcript.length} chars`);
      ...
      try {
        ...
        if (analysis.status !== 'ok') {
          console.log(`  -> Analysis failed: ${analysis.issues?.map((i: any) => i.description || i).join(', ')}`);
    ...
    if (scoredResults.length > 0) {
      ...
      const nonQualifying = scores.filter(s => s < 50).length;
      const avg = scores.reduce((a, b) => a + b, 0) / scores.length;
      console.log(`\n  Score >= 50 (QUALIFY):    ${qualifying}`);
      console.log(`  Score < 50 (NOT QUALIFY): ${nonQualifying}`);
      console.log(`  Average Score:            ${avg.toFixed(1)}`);
      
      console.log('\n  Qualifying leads:');
      for (const r of scoredResults.filter(r => r.score! >= 50).sort((a, b) => b.score! - a.score!)) {
