...
async function scoreOneLead(lead: LeadRow, index: number, batchOffset: number, grandTotal: number): Promise<Result> {
...
async function fetchBatch(offset: number): Promise<LeadRow[]> {
  const client = await pool.connect();
  try {
