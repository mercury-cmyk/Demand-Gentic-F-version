...
async function transferDispositions() {
  ...
  const byDisposition: Record<string, typeof callAttempts> = {};
  ...
  if (newContactIds.length > 0) {
    ...
    for (let i = 0; i < optOutRecords.length; i += batchSize) {
      const batch = optOutRecords.slice(i, i + batchSize);
      try {
        await db.insert(campaignOptOuts).values(batch).onConflictDoNothing();
    ...
    console.log(`\n✅ Created ${inserted} campaign opt-out records`);
  }

  // Step 5b: Transfer qualified leads from leads table
  console.log("\n[5b] Finding qualified leads from leads table...");
  ...
  const qualifiedLeads = await db
    ...
    .from(leads)
    .where(eq(leads.campaignId, sourceMatch.id));

  console.log(`  Found ${qualifiedLeads.length} total leads in source campaign`);

  // Filter for approved or AI-qualified leads
  const leadsToTransfer = qualifiedLeads.filter(l =>
    l.qaStatus === 'approved' ||
  ...
  if (newLeadContactIds.length > 0) {
    ...
    for (let i = 0; i < leadOptOutRecords.length; i += batchSize) {
      const batch = leadOptOutRecords.slice(i, i + batchSize);
      try {
        await db.insert(campaignOptOuts).values(batch).onConflictDoNothing();
        insertedLeads += batch.length;
        console.log(`  - Inserted batch ${Math.floor(i / batchSize) + 1}: ${batch.length} qualified lead opt-outs`);
      } catch (error: any) {
        console.error(`  - Error inserting batch: ${error.message}`);
      }
    }

    console.log(`\n✅ Created ${insertedLeads} qualified lead opt-out records`);
  }

  // Step 6: Ensure DNC contacts are in global_dnc
  console.log("\n[6] Checking global DNC entries...");

  const dncContacts = callAttempts.filter(ca => ca.disposition === 'do_not_call' && ca.contactId);
