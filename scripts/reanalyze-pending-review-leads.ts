...
async function getQueueSnapshot(client: QueryClient): Promise<QueueSnapshot> {
  const { rows: [row] } = await client.query(`
    SELECT
      ...
      COUNT(*) FILTER (WHERE transcript IS NULL OR length(transcript) <= 20)::int AS without_transcript,
      COUNT(*) FILTER (WHERE ai_qualification_status = 'qualified')::int AS ai_qualified,
      COUNT(*) FILTER (WHERE ai_qualification_status = 'not_qualified')::int AS ai_not_qualified,
      COUNT(*) FILTER (WHERE ai_qualification_status = 'needs_review')::int AS ai_needs_review,
      COUNT(*) FILTER (WHERE ai_qualification_status IS NULL)::int AS ai_missing
    FROM leads
  ...
  return {
    ...
    new_count: toInt(row?.new_count),
    under_review_count: toInt(row?.under_review_count),
    legacy_pending_count: toInt(row?.legacy_pending_count),
    with_transcript: toInt(row?.with_transcript),
    without_transcript: toInt(row?.without_transcript),
    ai_qualified: toInt(row?.ai_qualified),
    ai_not_qualified: toInt(row?.ai_not_qualified),
    ai_needs_review: toInt(row?.ai_needs_review),
    ai_missing: toInt(row?.ai_missing),
  };
}

async function getPendingLeads(client: QueryClient): Promise<PendingLead[]> {
...
async function main() {
  ...
  try {
    ...
    for (let index = 0; index < analyzableLeads.length; index++) {
      const lead = analyzableLeads[index];
      const label = `${lead.contact_name || lead.id} | ${lead.campaign_name || 'Unknown campaign'}`;
      console.log(`\n[${index + 1}/${analyzableLeads.length}] Re-analyzing ${label}`);

      try {
        const result = await analyzeLeadQualification(lead.id);
        if (result) {
          analyzed++;
          console.log(`  -> AI result: ${result.qualification_status} | score ${result.score}/100`);
        } else {
    ...
    for (const lead of unresolvedPending) {
      ...
      if (qualificationStatus === 'qualified') {
        ...
        movedToApproved++;
      } else if (qualificationStatus === 'not_qualified' && score < 30) {
        nextStatus = 'rejected';
        qaDecision = `Backfill: Auto-rejected — AI score ${score}/100`;
        movedToRejected++;
      } else {
        nextStatus = 'under_review';
        qaDecision = `Backfill: Moved to review — AI score ${score}/100, status: ${qualificationStatus}`;
        keptUnderReview++;
      }
      ...
      await client.query(`
        ...
        SET qa_status = $2,
            ...
            updated_at = NOW()
        WHERE id = $1
      `, [lead.id, nextStatus, qaDecision]);
    }

    console.log(`\nBackfill complete:`);
    console.log(`  Approved leads aligned to AI qualified: ${approvedBackfill.rowCount}`);
    console.log(`  Pending -> approved: ${movedToApproved}`);
    ...
    const remainingByReason = new Map<string, number>();
    const remainingByCampaign = new Map<string, number>();
