...
async function analyze() {
  ...
  for (let i = 0; i < recentCalls.length; i++) {
    ...
    console.log(`   Started: ${call.started_at?.toISOString().substring(0, 19)}`);
    console.log(`   Duration: ${call.duration_sec}s`);
    console.log(`   Disposition: ${call.ai_disposition}`);
    
    if (call.ai_transcript) {
      const lines = call.ai_transcript.split('\n').slice(0, 8);
      console.log(`   Transcript (first 8 lines):`);
      for (const line of lines) {
        console.log(`     ${line}`);
      }
    }
    
    // Check if prospect showed interest
    const transcript = (call.ai_transcript || '').toLowerCase();
    const interestSignals = {
      hasQuestions: /\?|tell me more|how|what|when|why|interested|interested|learn more|demo|meeting/i.test(transcript),
      hasPositiveLanguage: /yes|sure|okay|interested|sounds good|great|perfect|exactly|definitely/i.test(transcript),
      hasEngagement: /challenge|problem|issue|need|solution|budget|timeline|process/i.test(transcript),
    };
    
    const signals = Object.entries(interestSignals)
      .filter(([, value]) => value)
      .map(([key]) => key);
  ...
  } else if (qualifiedPercent < 5) {
    console.log('⚠️  WARNING: Less than 5% of calls are being qualified');
    console.log('   Recommendations: Same as above');
  } else {
    console.log('✅ Qualification rate is reasonable');
  }
