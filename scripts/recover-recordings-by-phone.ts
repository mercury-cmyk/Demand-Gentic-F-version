    ...
    try {
      ...
      if (recordings.length > 0) {
        ...
        const recording = recordings.find(r => {
          ...
          return diff < 30; // Within 30 seconds of expected duration
        }) || recordings[0];
        ...
        if (downloadUrl) {
          ...
          console.log(`     Duration: ${Math.round(recording.duration_millis / 1000)}s`);

          // Update lead with fresh URL
          ...
          // Download to GCS if requested
          if (DOWNLOAD_TO_GCS) {
            console.log(`  📥 Downloading to GCS...`);
            const s3Key = await downloadAndStoreRecording(downloadUrl, row.lead_id);
            if (s3Key) {
              await db.update(leads)
                .set({
                ...
                .where(eq(leads.id, row.lead_id));
              console.log(`  ✅ Saved to GCS: ${s3Key}`);
              downloaded++;
            }
          }
        } else {
          console.log(`  ⚠️ Recording found but no download URL`);
          notFound++;
        }
      } else {
        console.log(`  ❌ No recordings found in Telnyx`);
        notFound++;
      }
    } catch (error) {
