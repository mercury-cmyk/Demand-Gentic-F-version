...
async function analyze() {
  ...
  const byDuration = await db.execute(sql`
    SELECT 
      CASE 
        ...
        WHEN duration_sec < 5 THEN 'under_5s'
  ...
  console.log('\n=== TRANSCRIPTS BY DURATION ===');
  for (const r of byDuration.rows as any[]) {
    const bucket = (r.duration_bucket || 'null').padEnd(12);
    const tot = String(r.total).padStart(4);
    const with_t = String(r.with_transcript).padStart(4);
    const pct = r.total > 0 ? Math.round(r.with_transcript / r.total * 100) : 0;
