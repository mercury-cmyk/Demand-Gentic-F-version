...
async function checkConfig() {
  ...
  if (parseInt(ls.with_gcs_key) < parseInt(ls.with_url)) {
    console.log('⚠️  Many leads have recording URLs but no GCS storage key.');
    console.log('   This means recordings are not being permanently stored.\n');
    console.log('   To fix this:');
    console.log('   1. Ensure disposition engine downloads recordings immediately');
    console.log('   2. Or set up a webhook to trigger on recording.completed');
    console.log('   3. Or run a background worker to sync recordings periodically\n');
  }

  if (parseInt(stats.telnyx_url) > 0) {
