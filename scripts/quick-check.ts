...
async function quick() {
  ...
  for (let i = 0; i < recent.length; i++) {
    const r = recent[i];
    const lines = (r.ai_transcript || '').split('\n').slice(0, 4);
    console.log(`\n${i+1}. Disposition: ${r.ai_disposition}`);
    for (const line of lines) if (line) console.log(`   ${line.substring(0, 70)}`);
  }
  
  process.exit(0);
}

quick().catch(e => console.error(e));
