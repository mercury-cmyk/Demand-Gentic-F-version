...
async function ensureCallSession(lead: LeadRow): Promise<string> {
...
async function scoreOneLead(lead: LeadRow, index: number, total: number, oldScore: number | null): Promise<Result> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (attempt < MAX_RETRIES) {
        ...
        return { lead_id: lead.lead_id, name: lead.contact_name, campaign: lead.campaign_name, old_convq: oldScore, new_convq: 0, new_overall: 0, qualified: false, error: err.message };
      }
    }
  }
  return { lead_id: lead.lead_id, name: lead.contact_name, campaign: lead.campaign_name, old_convq: oldScore, new_convq: 0, new_overall: 0, qualified: false, error: 'unknown' };
}
...
async function main() {
  ...
  console.log(`RESCORE ${LEAD_IDS.length} SPECIFIC LEADS - Campaign Context & Objective Reanalysis`);
  console.log('='.repeat(80));

  // Step 1: Get old scores for comparison
  const client = await pool.connect();
  const { rows: oldScores } = await client.query(`
    SELECT id, ai_score FROM leads WHERE id = ANY($1)
  `, [LEAD_IDS]);
  const oldScoreMap = new Map(oldScores.map(r => [r.id, r.ai_score]));
  ...
  const { rows: leads } = await client.query<LeadRow & { old_ai_score: number | null }>(`
    SELECT
      l.id as lead_id,
  ...
  if (leads.length < LEAD_IDS.length) {
    console.log(`WARNING: ${LEAD_IDS.length - leads.length} leads skipped (no transcript or deleted)`);
  }
  console.log('');

  // Step 4: Score leads sequentially (with rate limiting)
  ...
  async function worker() {
    while (nextIndex < leads.length) {
      ...
      const oldScore = oldScoreMap.get(lead.lead_id) || null;
      const result = await scoreOneLead(lead, idx, leads.length, oldScore);
      results.push(result);
  ...
  const declined = results.filter(r => !r.error && r.old_convq != null && r.new_convq < r.old_convq);
  const unchanged = results.filter(r => !r.error && r.old_convq != null && r.new_convq === r.old_convq);
  ...
  if (nonQualifying.length > 0) {
    console.log('\n--- NOT QUALIFYING (ConvQ < 70) ---');
    nonQualifying.sort((a, b) => b.new_convq - a.new_convq);
  ...
  // Campaign breakdown
  const byCampaign: Record<string, { total: number; qualify: number; avgOld: number; avgNew: number; scores: { old: number | null; new: number }[] }> = {};
