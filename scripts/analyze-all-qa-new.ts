...
async function analyzeAllQANew() {
  ...
  try {
    ...
    const nonQualifying = leads.filter(l => l.effective_score !== null && parseFloat(l.effective_score) < 50);
    const noScore = leads.filter(l => l.effective_score === null);
    ...
    console.log(`  Score < 50 (NOT QUALIFY):   ${nonQualifying.length}`);
    console.log(`  No score available:         ${noScore.length}`);
    
    // With/without transcript
    const withTranscript = leads.filter(l => l.has_transcript || l.session_has_transcript);
    ...
    // Campaign breakdown
    const byCampaign: Record<string, { total: number; qualify: number; notQualify: number; noScore: number }> = {};
    ...
    if (scored.length > 0) {
      ...
      for (const d of dist) {
        ...
        if (count > 0) console.log(`  ${d.label.padEnd(25)} ${String(count).padStart(3)} ${'█'.repeat(Math.min(count, 50))}`);
      }
    }
    ...
    for (let i = 0; i < qualifying.length; i++) {
      const l = qualifying[i];
      const score = parseFloat(l.effective_score).toFixed(0);
      const disp = l.assigned_disposition || l.session_disposition || 'N/A';
      console.log(`\n  #${i+1} | Score: ${score} | ${l.qa_status}`);
      console.log(`    Contact:    ${l.contact_name || '?'} | ${l.job_title || 'N/A'}`);
      console.log(`    Account:    ${l.account_name || '?'} | ${l.account_industry || 'N/A'}`);
      console.log(`    Campaign:   ${l.campaign_name || '?'}`);
      console.log(`    Duration:   ${l.call_duration || l.session_duration || '?'}s | Transcript: ${l.has_transcript || l.session_has_transcript ? 'Yes' : 'No'}`);
      console.log(`    Disposition: ${disp} | Sentiment: ${l.sentiment || 'N/A'}`);
      console.log(`    Created:    ${l.created_at ? new Date(l.created_at).toISOString().slice(0,10) : '?'}`);
      if (l.disposition_accurate === false) {
        console.log(`    ⚠️ Disposition mismatch: assigned=${l.assigned_disposition} expected=${l.expected_disposition}`);
      }
    }

    // NON-QUALIFYING LEADS summary
    console.log(`\n${'='.repeat(80)}`);
    console.log(`NON-QUALIFYING LEADS (Score < 50) — ${nonQualifying.length} leads`);
    console.log('='.repeat(80));
    for (const l of nonQualifying) {
      const score = parseFloat(l.effective_score).toFixed(0);
      const disp = l.assigned_disposition || l.session_disposition || 'N/A';
      console.log(`  Score: ${score.padStart(3)} | ${(l.contact_name||'?').padEnd(25)} | ${(l.account_name||'?').padEnd(25)} | ${(l.campaign_name||'?').substring(0,25).padEnd(25)} | ${disp}`);
    }

    // NO SCORE leads
    console.log(`\n${'='.repeat(80)}`);
    console.log(`LEADS WITHOUT SCORES — ${noScore.length} leads`);
    console.log('='.repeat(80));
    const noScoreWithTranscript = noScore.filter(l => l.has_transcript || l.session_has_transcript);
