...
async function comprehensiveDeepDive(): Promise<void> {
  ...
  const allCalls: DetailedCall[] = (allCallsResult as any).rows || [];
  console.log(`Total calls loaded: ${allCalls.length}\n`);

  // ============================================================================
  // SECTION 2: Analyze all "not_interested" calls in detail
  // ============================================================================
  console.log('='.repeat(120));
  console.log('SECTION 2: DETAILED ANALYSIS OF ALL "NOT INTERESTED" CALLS');
  console.log('='.repeat(120));
  console.log();

  const notInterestedCalls = allCalls.filter(c =>
  ...
  const hourlyDistribution: Record<number, { total: number; connected: number; voicemail: number; noAnswer: number }> = {};
  ...
  // Analyze by campaign
  const campaignStats: Record<string, { name: string; total: number; connected: number }> = {};
...
function analyzeCallInDetail(call: DetailedCall): CallAnalysisResult {
  ...
  const callCutShort = pitchStarted && !pitchComplete && call.durationSec < 60;

  if (callCutShort) {
    reasons.push('call_cut_short_during_pitch');
    positiveSignals.push('pitch_started_before_cutoff');
  }

  // Check duration
  if (call.durationSec >= 60 && hasRealConversation) {
  ...
  // Medium priority: engagement but no explicit request
    ...
    recommendedAction = 'Send follow-up email with value proposition';
  }
  // Medium priority: call cut short during engagement
  else if (callCutShort && positiveScore >= 1) {
    shouldFollowUp = true;
    priority = 'MEDIUM';
  ...
  else if (positiveScore >= 1 && negativeScore <= positiveScore) {
    shouldFollowUp = true;
    priority = 'LOW';
    reasons.push('mixed_signals');
    recommendedAction = 'Consider email follow-up in 2 weeks';
  }

  // Engagement level assessment
  let engagementLevel = 'none';
  if (hasRealConversation && call.durationSec >= 60) engagementLevel = 'high';
