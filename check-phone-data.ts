...
async function main() {
  ...
  (contactsWithPhones.rows as any[]).forEach((c, i) => {
    ...
    console.log(`${i + 1}. ${c.first_name} | Raw: "${c.direct_phone}" | E164: "${c.direct_phone_e164}" | Country: "${c.country}"${hasIssue ? ' <-- BAD FORMAT' : ''}`);
  });

  // Check specifically for bad +0 format
  console.log('\n\n=== Contacts with +0 Format (BROKEN) ===');
  const brokenPhones = await db.execute(sql`
    SELECT first_name, direct_phone, direct_phone_e164, country 
    FROM contacts 
    WHERE direct_phone_e164 LIKE '+0%'
    LIMIT 20
  `);
  
  console.log(`Found ${brokenPhones.rows.length} contacts with +0 format:`);
  (brokenPhones.rows as any[]).forEach((c, i) => {
