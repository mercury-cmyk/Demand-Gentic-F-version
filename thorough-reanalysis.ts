...
function analyzeEngagement(call: CallRecord): EngagementAnalysis {
  ...
  } else if (negativeSignals.includes('EXPLICIT_NOT_INTERESTED') && positiveSignals.length < 2) {
    recommendation = 'SKIP';
    reason = 'Explicit not interested with no counter-signals';
  } else if (score >= 8) {
...
async function thoroughReanalysis(): Promise<void> {
  ...
  const allCalls: CallRecord[] = (result as any).rows || [];
  console.log(`Total calls: ${allCalls.length}\n`);

  // Filter calls with transcripts
  const callsWithTranscripts = allCalls.filter(c => c.aiTranscript && c.aiTranscript.length > 30);
  console.log(`Calls with transcripts: ${callsWithTranscripts.length}\n`);

  // Analyze each call
  const createLeadCalls: Array<CallRecord & { analysis: EngagementAnalysis }> = [];
  const followUpCalls: Array<CallRecord & { analysis: EngagementAnalysis }> = [];
  const retryCalls: Array<CallRecord & { analysis: EngagementAnalysis }> = [];
  const skipCalls: Array<CallRecord & { analysis: EngagementAnalysis }> = [];
