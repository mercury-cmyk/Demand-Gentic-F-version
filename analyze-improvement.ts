  ...
  const durationResult = await db.execute(sql`
    SELECT
      CASE
        ...
        WHEN call_duration_seconds < 45 THEN '30-45s'
        WHEN call_duration_seconds < 60 THEN '45-60s'
        WHEN call_duration_seconds < 75 THEN '60-75s'
        WHEN call_duration_seconds < 90 THEN '75-90s'
        ELSE '90s+'
      END as duration_bucket,
      COUNT(*) as count,
      MIN(call_duration_seconds) as min_val
    FROM dialer_call_attempts
