    ...
    if (cqCols.has('campaign_id') && cqCols.has('status')) {
      ...
      const qSql = `
        SELECT
          ...
          ${hasNext ? ", COUNT(*) FILTER (WHERE status='queued' AND (next_attempt_at IS NULL OR next_attempt_at <= NOW()))::int AS callable_now, COUNT(*) FILTER (WHERE status='queued' AND next_attempt_at > NOW())::int AS deferred, MIN(next_attempt_at) FILTER (WHERE status='queued') AS next_queued_at" : ''}
        FROM campaign_queue
        WHERE campaign_id = $1
      `;
      ...
      if (stateCol) {
        const aSql = `
          SELECT
            ...
            ${schedCol ? `, COUNT(*) FILTER (WHERE ${stateCol}='queued' AND (${schedCol} IS NULL OR ${schedCol} <= NOW()))::int AS callable_now, MIN(${schedCol}) FILTER (WHERE ${stateCol}='queued') AS next_scheduled` : ''}
          FROM agent_queue
          WHERE campaign_id = $1
        `;
        const a = await client.query(aSql, [campaignId]);
        console.log('AGENT_QUEUE_STATS');
        console.table(a.rows);
      }
    }

    // dialer attempts stats and errors
    const dCols = colMap.dialer_call_attempts;
    if (dCols.has('campaign_id')) {
      const createdCol = pick(dCols, ['created_at','started_at']);
      const errCol = pick(dCols, ['error_message','error']);
      const attemptsSql = `
        SELECT
          COUNT(*)::int AS attempts_24h,
          ${errCol ? `COUNT(*) FILTER (WHERE ${errCol} IS NOT NULL)::int AS errors_24h,` : '0::int AS errors_24h,'}
          ${createdCol ? `MAX(${createdCol}) AS last_attempt_at` : 'NULL::timestamptz AS last_attempt_at'}
        FROM dialer_call_attempts
        WHERE campaign_id = $1
        ${createdCol ? `AND ${createdCol} > NOW() - INTERVAL '24 hours'` : ''}
      `;
