...
async function analyzeTranscriptsManually() {
  ...
  if (potentialLeads.length > 0) {
    ...
    for (let i = 0; i < potentialLeads.length; i++) {
      ...
      console.log(`Duration: ${lead.duration}s (${(lead.duration / 60).toFixed(1)} min)`);
      console.log(`Positive Signals: ${analysis.positiveMatches.join(', ') || 'None detected'}`);
      console.log(`\nTRANSCRIPT PREVIEW:`);
      console.log(lead.transcript.substring(0, 1000) + (lead.transcript.length > 1000 ? '...' : ''));
      console.log('\n' + '='.repeat(60));
    }
  }
