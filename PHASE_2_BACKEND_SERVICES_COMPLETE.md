...
curl -X POST http://localhost:5000/api/campaigns/analyze-spam \
  ...
  -d '{
    "subject": "FREE MONEY!!!",
    "html": "<p>Click here for your free prize</p>"
  }'
```
...
const result = renderEmail('<h1>Hello {{contact.first_name}}</h1>', {
...
console.log(result.html);      // <h1>Hello John</h1> + footer
...
2. **HTML Structure:**
   - Automatically wraps partial HTML in proper `<html>` tags
   - Handles both `<body>` and direct content insertion
