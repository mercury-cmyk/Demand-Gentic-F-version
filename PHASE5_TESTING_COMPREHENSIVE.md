...
npm install --save-dev vitest @vitest/ui supertest @testing-library/react @testing-library/jest-dom
...
npm run test -- campaign-send-routes.test.ts

# Run tests matching pattern
npm run test -- --grep "A/B test"
...
- [ ] Authentication required tests passing
...
- [ ] Email rendering < 100ms/email
- [ ] Queue processing > 1,000 emails/min
...
Email Rendering Time: < 100ms
Campaign Send Rate: 2,000+ emails/min
...
export default function() {
  const res = http.get('http://localhost:3000/api/campaigns');
...
it('should match snapshot', () => {
  const { container } = render(<EmailPreview html={htmlContent} />);
