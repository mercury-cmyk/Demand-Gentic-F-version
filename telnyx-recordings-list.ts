...
async function main() {
  ...
  console.log(`\nTotal Telnyx recordings found: ${allRecordings.length}`);
  
  // Build a lookup map by call_control_id, call_leg_id, call_session_id
  const byCallControlId = new Map<string, TelnyxRecording>();
  const byCallLegId = new Map<string, TelnyxRecording>();
  const byCallSessionId = new Map<string, TelnyxRecording>();
