...
ssh_source_cidr  = "YOUR_IP/32"  # Restrict SSH to your IP
...
```bash
# Firewall rules are automatically created by Terraform, but verify:
gcloud compute firewall-rules list --filter="name~^sip-"
...
```bash
# SSH into instance
gcloud compute ssh demandgentic-sip-server --zone us-central1-a
...
| `DRACHTIO_HOST` | localhost | Drachtio server hostname |
| `DRACHTIO_PORT` | 9022 | Drachtio command port |
| `SIP_LISTEN_PORT` | 5060 | SIP listening port |
| `RTP_PORT_MIN` | 10000 | Minimum RTP port |
| `RTP_PORT_MAX` | 20000 | Maximum RTP port |
| `STUN_SERVERS` | Google STUN | Comma-separated STUN URLs |
| `TURN_SERVERS` | - | JSON array of TURN servers |
| `TURN_USERNAME` | - | TURN authentication username |
| `TURN_PASSWORD` | - | TURN authentication password |
| `USE_SIP_CALLING` | false | Enable SIP-based calling |
| `AUTO_INIT_SIP` | false | Auto-initialize SIP on startup |

### Drachtio Configuration

Located in `drachtio.conf`:
...
timeout 5 bash -c 'cat < /dev/null > /dev/tcp/<PUBLIC_IP>/5060' && echo "SIP port open" || echo "SIP port closed"

# Using netcat (if available)
nc -u -w1 <PUBLIC_IP> 5060 && echo "SIP port open"
```
...
```bash
# Using turnclient
turnclient -v -h <PUBLIC_IP> -p 3478 -u turnuser -w turnpass -a 1.2.3.4
...
```bash
# Using SIPp tool
sipp -u user1 -p 5061 -s "sip:user@<PUBLIC_IP>" <PUBLIC_IP>:5060
```
...
# Filter by component
gcloud logging read "logName=projects/<PROJECT>/logs/demandgentic-sip" --limit 50
```
