...
async function analyzeTranscript(transcript: string, contactName: string): Promise<AnalysisResult> {
...
async function main() {
  ...
  for (let i = 0; i < rows.length; i++) {
    const lead = rows[i];
    console.log(`\n[${ i + 1}/${rows.length}] Analyzing lead ${lead.lead_id}`);
    console.log(`  Contact: ${lead.contact_name}`);
    console.log(`  Current QA Status: ${lead.qa_status}`);
    console.log(`  Created: ${lead.created_at}`);

    try {
      const analysis = await analyzeTranscript(lead.transcript, lead.contact_name);
      stats.analyzed++;

      console.log(`  📊 Analysis Result:`);
      console.log(`     Disposition: ${analysis.disposition}`);
      console.log(`     Confidence: ${(analysis.confidence * 100).toFixed(0)}%`);
      console.log(`     AI Screening: ${analysis.isAiCallScreening ? 'YES ⚠️' : 'No'}`);
      console.log(`     Human Contact: ${analysis.humanContactMade ? 'Yes' : 'No'}`);
      console.log(`     Reasoning: ${analysis.reasoning}`);

      if (analysis.isAiCallScreening) {
        stats.aiScreeningDetected++;
      }
      if (analysis.isVoicemail || analysis.disposition === 'voicemail') {
        stats.voicemailDetected++;
      }
      if (analysis.disposition === 'qualified_lead') {
        stats.qualified++;
      }
      if (analysis.disposition === 'not_interested') {
        stats.notInterested++;
      }
      ...
      // Update the lead
      const updateNote = `[${new Date().toISOString()}] DeepSeek Re-analysis: ${analysis.disposition} (${(analysis.confidence * 100).toFixed(0)}% confidence). ${analysis.reasoning}`;
      
      await db.execute(sql`
        UPDATE leads 
        SET 
          qa_status = ${newQaStatus},
          ai_qualification_status = ${newQualificationStatus},
          ai_analysis = ${JSON.stringify(analysis)}::jsonb,
          notes = COALESCE(notes, '') || E'\n' || ${updateNote}
        WHERE id = ${lead.lead_id}
      `);

      if (lead.qa_status !== newQaStatus) {
        stats.changed++;
        console.log(`  ✅ Updated: ${lead.qa_status} → ${newQaStatus}`);
      } else {
