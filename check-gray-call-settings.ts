...
async function checkGrayCallSettings() {
  ...
  for (const row of sessions.rows) {
    ...
    for (const [key, value] of Object.entries(s)) {
      if (value !== null && value !== undefined && String(value).length < 200) {
  ...
  // Check the full notes in the call attempt
  console.log('\n========================================');
  console.log('FULL NOTES FROM CALL ATTEMPT');
  console.log('========================================\n');

  console.log(`Notes length: ${call.notes?.length || 0} characters`);
  if (call.notes) {
    console.log('\nFULL NOTES:');
