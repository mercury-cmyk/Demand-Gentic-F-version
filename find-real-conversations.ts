...
function isLikelyHumanResponse(text: string): boolean {
  ...
  if (text.length < 50 && !IVR_PATTERNS.some(p => lower.includes(p))) {
    return true;
  }
...
async function findRealConversations() {
  ...
  if (realConversations.length > 0) {
    ...
    for (let i = 0; i < Math.min(realConversations.length, 20); i++) {
      const conv = realConversations[i];
      console.log(`\n--- CONVERSATION ${i + 1} ---`);
      console.log(`Name: ${conv.contactName}`);
      console.log(`Company: ${conv.company}`);
      console.log(`Email: ${conv.email || 'N/A'}`);
      console.log(`Phone: ${conv.phone}`);
      console.log(`Duration: ${conv.duration}s (${(conv.duration / 60).toFixed(1)} min)`);
      console.log(`Human responses: ${conv.humanResponses} / ${conv.totalResponses}`);
      console.log(`Sample user responses:`);
      for (const response of conv.sampleResponses) {
