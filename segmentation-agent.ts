  ...
  const rules = definition.rules
    .map((rule: any) => {
      ...
      for (let i = 0; i < fieldParts.length - 1; i++) {
        currentFilter[fieldParts[i]] = {};
        currentFilter = currentFilter[fieldParts[i]];
      }
      const finalField = fieldParts[fieldParts.length - 1];

      switch (operator) {
        case 'equals':
          currentFilter[finalField] = { equals: value };
          break;
        case 'contains':
          currentFilter[finalField] = { contains: value, mode: 'insensitive' };
          break;
        case 'greater_than':
          currentFilter[finalField] = { gt: value };
          break;
        case 'less_than':
          currentFilter[finalField] = { lt: value };
          break;
        default:
          return null;
      }
