...
function analyzeTranscript(transcript: string, successCriteria: string | null, campaignType: string): TranscriptAnalysis {
  ...
  for (const line of transcriptSection.split('\n')) {
    ...
    if (agentMatch) {
      ...
      if (content.length > 3 && !isIVRLine(content) && !isAgentSpeechMislabeled(content)) {
        ...
        const isGarbled = /^[^a-zA-Z]*$/.test(content) || content.split(' ').length <= 1;
        if (!isGarbled) {
          realContactLines.push(content);
        }
      }
    }
  }

  // If no labeled turns, try JSON
  if (agentLines.length === 0 && rawContactLines.length === 0) {
    try {
      const parsed = JSON.parse(transcript);
      if (Array.isArray(parsed)) {
        for (const t of parsed as any[]) {
          if (t.role === 'assistant' || t.role === 'agent') agentLines.push(t.content || '');
          if (t.role === 'user' || t.role === 'contact') {
            rawContactLines.push(t.content || '');
            const c = (t.content || '').trim();
            if (c.length > 3 && !isIVRLine(c) && !isAgentSpeechMislabeled(c)) {
  ...
  if (reversedLabels) {
    ...
    contactTurns = agentLines.filter(l => 
      l.length > 3 && l.length < 80 && !/(?:am I speaking|may I speak|calling on behalf|hello\?|hi,)/i.test(l)
    ).length;
  }

  const contactText = reversedLabels 
    ? agentLines.join(' ').toLowerCase()  // Use agent lines as contact text when reversed
    : realContactLines.join(' ').toLowerCase();

  // Signal detection — use ONLY real contact text for contact-specific signals
  const positiveSignals = POSITIVE_SIGNALS.filter(s => contactText.includes(s));
  const negativeSignals = NEGATIVE_SIGNALS.filter(s => contactText.includes(s));
  const isVoicemail = VOICEMAIL_SIGNALS.some(s => text.includes(s)) && contactTurns < 2;
  const isGatekeeper = GATEKEEPER_SIGNALS.some(s => contactText.includes(s) || text.includes(s));
  ...
  // Voicemail
  else if (isVoicemail && contactTurns < 2) {
    suggestedDisposition = 'voicemail';
    confidence = 0.9;
    reasoning = 'Voicemail/answering machine detected';
  }
  // Real human engagement analysis
  else if (hasRealHumanEngagement) {
    // Meeting/appointment campaigns
    ...
    const isEventCampaign = ['webinar_invite', 'live_webinar', 'on_demand_webinar', 'executive_dinner', 'leadership_forum', 'conference', 'event_registration_digital_ungated', 'event_registration_digital_gated', 'in_person_event'].includes(campaignType);
    const isContentCampaign = ['content_syndication', 'high_quality_leads'].includes(campaignType);

    // Check for explicit decline first
    if (contactDeclined && negativeSignals.length > 0 && positiveSignals.length === 0) {
  ...
  // Gatekeeper only
  else if (isGatekeeper && contactTurns <= 2) {
    suggestedDisposition = 'no_answer';
    confidence = 0.7;
    reasoning = 'Gatekeeper interaction only — did not reach decision maker';
  }
  // Voicemail not caught above
  else if (contactTurns <= 1 && isVoicemail) {
    suggestedDisposition = 'voicemail';
    confidence = 0.85;
    reasoning = 'Voicemail system detected with no real contact engagement';
  }
  // Catch-all: real but insufficient engagement
  else {
    suggestedDisposition = 'needs_review';
    confidence = 0.55;
    reasoning = `Ambiguous: ${contactTurns} real contact turns, ${positiveSignals.length} pos / ${negativeSignals.length} neg signals. Contact said: "${realContactLines.slice(0, 2).join(' | ').substring(0, 100)}"`;
  }

  return {
    hasRealHumanEngagement,
    contactTurns,
    agentTurns,
    positiveSignals,
    negativeSignals,
    ...
    isGatekeeper,
    isIVR,
    contactSaidIdentityConfirm,
    contactAskedQuestions,
    contactExpressedInterest,
    contactRequestedCallback,
    contactRequestedInfo,
    contactAgreedToMeeting,
    contactDeclined,
    contactRequestedDNC,
...
async function runAnalysis() {
  ...
  console.log(`${LOG} ==========================================`);
  console.log(`${LOG} DISPOSITION ENGAGEMENT ANALYSIS`);
  console.log(`${LOG} Mode: ${DRY_RUN ? 'DRY RUN (use --apply to update)' : '⚡ LIVE UPDATE MODE'}`);
  console.log(`${LOG} ==========================================\n`);

  // 1. Current disposition breakdown
  console.log('=== CURRENT DISPOSITION BREAKDOWN (30 days) ===');
  const dispBreakdown = await db.execute(sql`
    SELECT 
      COALESCE(disposition::text, 'NULL') as disp,
      COUNT(*) as cnt,
      ROUND(AVG(call_duration_seconds)) as avg_dur,
      COUNT(CASE WHEN call_duration_seconds >= 30 THEN 1 END) as over_30,
  ...
  const stats = {
    ...
    wouldDowngrade: 0,
    wouldChange: 0,
    byNewDisp: {} as Record<string, number>,
    byOldDisp: {} as Record<string, number>,
  };
  ...
  if (changes.length > 0) {
    ...
    const upgrades = changes.filter(c => {
      const r: Record<string, number> = { qualified_lead: 5, callback_requested: 4, needs_review: 3, not_interested: 2, no_answer: 1, voicemail: 1, do_not_call: 0 };
    ...
    for (const ch of changes) {
      try {
        ...
        if (ch.confidence < 0.6) {
          console.log(`  SKIP (low confidence ${Math.round(ch.confidence * 100)}%): ${ch.id}`);
          continue;
        }

        // Never downgrade do_not_call — DNC is sticky, must be removed manually
        if (ch.currentDisp === 'do_not_call' && ch.newDisp !== 'do_not_call') {
          console.log(`  SKIP (DNC is sticky): ${ch.id} — ${ch.contact}`);
          continue;
        }
        ...
        await db.execute(sql`
          ...
          SET 
            disposition = ${ch.newDisp}::canonical_disposition,
            notes = COALESCE(notes, '') || ${`\n[Auto-reclassified ${new Date().toISOString()}] ${ch.currentDisp} → ${ch.newDisp}: ${ch.reasoning}`}
          WHERE id = ${ch.id}
        `);

        // Also update the corresponding call_session if exists
        await db.execute(sql`
          UPDATE call_sessions
          SET ai_disposition = ${ch.newDisp}
          WHERE id IN (
            SELECT call_session_id FROM dialer_call_attempts WHERE id = ${ch.id} AND call_session_id IS NOT NULL
          )
        `);

        applied++;
        console.log(`  ✅ ${ch.id}: ${ch.currentDisp} → ${ch.newDisp}`);
      } catch (err) {
        errors++;
        console.error(`  ❌ Failed ${ch.id}:`, err);
      }
    }

    console.log(`\n${LOG} Applied: ${applied}, Skipped (low confidence): ${changes.length - applied - errors}, Errors: ${errors}`);
