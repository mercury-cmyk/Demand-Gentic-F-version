...
- Co-locate feature code: `client/src/components/<feature>/` for UI, `server/services/<feature>.ts` for logic.
...
- There is no single `npm test`; run targeted Vitest files or `npx tsx <script>.ts` for the area you change.
