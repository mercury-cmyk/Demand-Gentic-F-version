...
async function main() {
  ...
  try {
    ...
    for (const row of res.rows) {
      ...
      const isShort = row.duration_sec < 45; 
      
      // Heuristic: Does it have explicit interest signals?
      const hasCallback = transcriptStr.includes('call back') || transcriptStr.includes('callback');
      const hasMeeting = transcriptStr.includes('meeting') || transcriptStr.includes('calendar');
      const hasEmail = transcriptStr.includes('email') || transcriptStr.includes('send it');
      const hasYes = transcriptStr.includes(' yes ') || transcriptStr.includes('sure') || transcriptStr.includes('okay');
      
      // DECISION:
      // It IS actionable if:
      // - NOT a known screener AND
      // - (Has specific interest keywords OR (Is a decent length > 15s AND has some human response))
