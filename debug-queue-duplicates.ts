...
async function debugQueueDuplicates() {
  ...
  const immediateRetryAnalysis = await db.execute(sql`
    ...
    FROM ranked_calls
    WHERE call_num > 1
      AND EXTRACT(EPOCH FROM (created_at - prev_call_time))/60 < 5
    ORDER BY minutes_since_prev
    LIMIT 30
  `);

  console.log('\n========================================');
  console.log('IMMEDIATE RETRIES - QUEUE ITEM ANALYSIS');
  console.log('========================================\n');

  let sameQueueItem = 0;
  let differentQueueItem = 0;
  let nullQueueItem = 0;

  immediateRetryAnalysis.rows.forEach((row: any) => {
