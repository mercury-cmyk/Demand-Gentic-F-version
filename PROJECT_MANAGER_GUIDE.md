...
for region in us-central1 us-east1 eu-west1; do
  npm run project -- add "app-$region" << EOF
$region
org-123
billing-account
App deployment in $region
EOF
done
...
```bash
# Export current projects for backup/sharing
cp .gcp-projects/projects.json projects_backup.json

# Or generate report
npm run project -- list > projects_report.txt
```
...
| `add <id>` | Create project (interactive) | `npm run project -- add my-proj` |
| `switch <id>` | Change to project | `npm run project -- switch my-proj` |
| `show <id>` | Display project details | `npm run project -- show my-proj` |
| `remove <id>` | Delete project config | `npm run project -- remove my-proj` |
