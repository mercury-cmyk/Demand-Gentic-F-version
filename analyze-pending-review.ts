...
function analyzeCall(transcript: string, successCriteria: string | null, campaignType: string): {
  ...
  // Signal detection
  const positiveSignals = POSITIVE_KEYWORDS.filter(s => contactText.includes(s));
  const negativeSignals = NEGATIVE_KEYWORDS.filter(s => contactText.includes(s));
  const isVoicemail = VOICEMAIL_KEYWORDS.some(s => text.includes(s)) && realContactTurns < 2;
  const isIVR = rawContactLines.some(l => isIVRLine(l));
  ...
  if (requestedDNC) {
    ...
    engagementLevel = 'low';
  } else if (isVoicemail && realContactTurns < 2) {
    ...
    if (isIVR) {
      ...
      confidence = 0.8;
      reasoning = 'IVR only — no human reached';
    } else {
      suggestedDisposition = 'no_answer';
      confidence = 0.8;
      reasoning = `No real contact turns (${rawContactLines.length} raw lines filtered as IVR/agent)`;
    }
    engagementLevel = 'none';
  } else if (declined && !expressedInterest && negativeSignals.length > 0) {
...
async function main() {
  ...
  if (wouldDowngrade.length > 0) {
    ...
    console.log('='.repeat(80));
    const byDisp: Record<string, typeof wouldDowngrade> = {};
  ...
  if (APPLY) {
    ...
    for (const a of analyses) {
      ...
      try {
        const note = `\n[Pending Review QA ${new Date().toISOString()}] needs_review → ${a.suggestedDisposition}: ${a.reasoning}`;
        await db.execute(sql`
          UPDATE dialer_call_attempts
          SET disposition = ${a.suggestedDisposition}::canonical_disposition,
              notes = COALESCE(notes, '') || ${note}
          WHERE id = ${a.id}
        `);
        await db.execute(sql`
          UPDATE call_sessions
          SET ai_disposition = ${a.suggestedDisposition}
          WHERE id IN (
            SELECT call_session_id FROM dialer_call_attempts WHERE id = ${a.id} AND call_session_id IS NOT NULL
          )
        `);
        applied++;
        console.log(`  ✅ ${a.contactName}: needs_review → ${a.suggestedDisposition} (${Math.round(a.confidence * 100)}%)`);
      } catch (err) {
        errors++;
        console.error(`  ❌ ${a.contactName}: ${err}`);
      }
    }

    console.log(`\nApplied: ${applied} | Skipped (low confidence): ${skipped} | Errors: ${errors}`);
  }
  ...
  console.log('  SUMMARY');
  console.log('='.repeat(80));
  console.log(`  Total pending review:       ${analyses.length}`);
  console.log(`  With real engagement:        ${analyses.filter(a => a.engagementLevel !== 'none').length}`);
