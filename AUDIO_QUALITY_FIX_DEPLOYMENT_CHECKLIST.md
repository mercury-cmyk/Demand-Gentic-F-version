...
✓ Successful calls complete without timeout
✓ Reconnection works (max 5 attempts)
```

---

### Step 3: Gradual Production Rollout (1 week)

#### Phase 3A: 25% Rollout (Day 1-2)
**Servers:** 1-2 production servers (25% of traffic)

```bash
# Deploy to 25% of servers
git pull origin main
npm install && npm run build
# Restart specified servers only
```

**Monitor:**
- [ ] Call completion rate (compare to baseline)
- [ ] Audio quality scores (target >80/100)
...
Audio complaints: 15% → <3%
No-answer errors: 5% → <2%
```

#### Phase 3B: 50% Rollout (Day 3-4)
**Servers:** 4-5 production servers (50% of traffic)

**Decision Gate:** If Phase 3A metrics are green, proceed. Otherwise, investigate and fix.

```bash
# Deploy to additional servers
git pull origin main
npm install && npm run build
# Restart additional servers
```

**Metrics same as Phase 3A, continued monitoring**

#### Phase 3C: 100% Rollout (Day 5-7)
**Servers:** All production servers (100% of traffic)

**Decision Gate:** If Phase 3B metrics stable and improving, proceed to full rollout.

```bash
# Deploy to all remaining servers
git pull origin main
npm install && npm run build
# Restart all servers (staggered to avoid downtime)
```

**Final Verification:**
- [ ] All servers reporting keepalive messages
- [ ] Quality metrics across entire fleet
- [ ] Call completion rate meets target
...
SELECT AVG(quality_score) FROM call_metrics WHERE date = TODAY();

# Check for errors
grep -i "ERROR\|CRITICAL" logs/production.log | wc -l

# Monitor reconnection attempts
grep "Reconnect attempt" logs/production.log | wc -l

# Audio timeout incidents
grep "Audio timeout" logs/production.log | wc -l
```

**Alert Thresholds:**
- Quality score < 70: Investigate
- Audio timeouts > 5/hour: Alert
...
- [ ] Quality scores consistently <50
...
Calls dropping after 60s: <1%
Audio complaints: <3% (80%+ reduction)
"Can't hear": <1%
"Too much distortion": <1%
"No answer" misclassifications: <2%
Quality metrics available for all calls
