...
function analyzeTranscriptForQualification(transcript: string | null, analysis: any): {
  ...
  if (!transcript || transcript.length < 50) {
    return { shouldBeQualified: false, confidence: 'low', reasons: ['no_transcript'] };
  }
  ...
  if (/yeah[,.]?\s*(yeah[,.]?\s*)+/i.test(transcript)) { score += 3; reasons.push('multiple_affirmatives'); }
  if (/go ahead/i.test(transcript)) { score += 3; reasons.push('permission_to_continue'); }
  if (/tell me more/i.test(transcript)) { score += 3; reasons.push('asking_for_info'); }
  if (/send (me|it|that)/i.test(transcript)) { score += 4; reasons.push('requesting_materials'); }
  if (/email (me|it)/i.test(transcript)) { score += 4; reasons.push('requesting_email'); }
  if (/call (me )?back/i.test(transcript)) { score += 4; reasons.push('callback_request'); }
  if (/interested/i.test(transcript) && !/not interested/i.test(transcript)) { score += 3; reasons.push('expressed_interest'); }
  if (/schedule|meeting|demo/i.test(transcript)) { score += 4; reasons.push('meeting_interest'); }
  if (/sounds? (good|interesting)/i.test(transcript)) { score += 2; reasons.push('positive_response'); }

  // Check AI analysis
  if (analysis) {
    if (analysis.sentiment === 'positive') { score += 2; reasons.push('positive_sentiment'); }
    if (analysis.engagement_level === 'high') { score += 2; reasons.push('high_engagement'); }
    if (analysis.follow_up_consent === 'yes') { score += 4; reasons.push('follow_up_consent'); }
  }

  // Negative signals (reduce score)
  if (/not interested/i.test(transcript)) { score -= 5; reasons.push('explicit_not_interested'); }
  if (/no thanks/i.test(transcript)) { score -= 3; reasons.push('declined'); }
  if (/don'?t call/i.test(transcript)) { score -= 5; reasons.push('do_not_call'); }
  if (/remove me/i.test(transcript)) { score -= 5; reasons.push('removal_request'); }
...
async function reanalyzeWithFixes(): Promise<void> {
  ...
  console.log(`Total calls to re-analyze: ${calls.length}\n`);

  // Statistics
  let originalNotInterested = 0;
  let fixedWouldRetry = 0;
  let fixedWouldQualify = 0;
  let transcriptBasedQualified = 0;

  const missedLeads: Array<CallRecord & { analysis: ReturnType<typeof analyzeTranscriptForQualification>; newDisposition: string }> = [];
  const shouldRetry: Array<CallRecord & { newDisposition: string; oldDisposition: string }> = [];
