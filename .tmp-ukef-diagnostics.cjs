  ...
  try {
    ...
    const queue = await client.query(`
      SELECT
        ...
        COUNT(*) FILTER (WHERE status = 'queued' AND (next_attempt_at IS NULL OR next_attempt_at <= NOW()))::int AS callable_now,
        COUNT(*) FILTER (WHERE status = 'queued' AND next_attempt_at > NOW())::int AS deferred,
    ...
    const agentQueue = await client.query(`
      SELECT
        ...
        COUNT(*) FILTER (WHERE queue_state='queued' AND (scheduled_for IS NULL OR scheduled_for <= NOW()))::int AS callable_now,
        MIN(scheduled_for) FILTER (WHERE queue_state='queued') AS next_scheduled
      FROM agent_queue
      WHERE campaign_id = $1
    `, [campaignId]);
    console.log('AGENT_QUEUE');
    console.table(agentQueue.rows);
