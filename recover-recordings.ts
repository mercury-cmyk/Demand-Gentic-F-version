...
async function getLegIdFromCallEvents(callControlId: string): Promise<string | null> {
...
async function downloadToTemp(url: string, leadId: string): Promise<string | null> {
  try {
    const resp = await fetch(url);
    if (!resp.ok) {
