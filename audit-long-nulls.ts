...
async function auditLongNulls() {
  ...
  const withDisp = await db.select({
    ...
    hasTranscript: sql<boolean>`${callSessions.aiTranscript} IS NOT NULL AND LENGTH(${callSessions.aiTranscript}) > 0`,
    transcriptLen: sql<number>`COALESCE(LENGTH(${callSessions.aiTranscript}), 0)`,
  })
  .from(callSessions)
  .where(and(
    gte(callSessions.startedAt, yesterday),
