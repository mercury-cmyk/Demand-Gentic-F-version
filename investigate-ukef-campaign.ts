...
async function investigateUKEFCampaign() {
  ...
  const sessionsByTime = await pool.query(`
    ...
    WHERE cs.created_at >= (SELECT MIN(created_at) FROM dialer_call_attempts WHERE campaign_id = $1)
      AND cs.created_at <= (SELECT MAX(created_at) FROM dialer_call_attempts WHERE campaign_id = $1)
    GROUP BY campaign_id, c.name
    ORDER BY count DESC
    LIMIT 10
  `, [campaignId]);
  
  console.log('\n  Call sessions in same timeframe (by campaign):');
  sessionsByTime.rows.forEach((r: any) => console.log(`    ${r.campaign_name || r.campaign_id}: ${r.count}`));
