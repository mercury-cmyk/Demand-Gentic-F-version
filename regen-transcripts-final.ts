...
async function makeFetchRequest(url: string, options: any): Promise<any> {
...
async function regenerateTranscripts() {
  ...
  try {
    ...
    const allIds = [...csRows.rows, ...daRows.rows].map((r: any) => r.id);
    ...
    for (let i = 0; i < allIds.length; i += batchSize) {
      ...
      const batch = allIds.slice(i, Math.min(i + batchSize, allIds.length));
      ...
      try {
        ...
        const response = await makeFetchRequest(`${baseUrl}/api/call-intelligence/transcription-gaps/regenerate`, {
          method: 'POST',
        ...
        if (json.success && json.data) {
          totalSucceeded += json.data.succeeded;
          totalFailed += json.data.failed;
          console.log(`[${batchNum}/${totalBatches}] ✅ Queued: ${json.data.queued}, Succeeded: ${json.data.succeeded}`);
        } else {
