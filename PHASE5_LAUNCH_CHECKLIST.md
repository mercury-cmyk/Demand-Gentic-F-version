...
- [ ] Email send rate > 1,000 emails/min
- [ ] Database queries < 50ms
- [ ] Frontend load time < 2s
- [ ] No memory leaks detected
- [ ] No database connection issues

### Monitoring (24 Hours Before)
...
```bash
# Final health checks
curl http://staging.app.com/health
docker ps --all
docker exec db pg_isready
redis-cli ping

# Verify backups
aws s3 ls s3://pmp-backups/ --recursive
# Recent backups should exist

# Check error rates
curl http://staging.app.com/api/metrics
# Should show < 0.5% error rate

# Verify deployment ready
docker images | grep pmp:latest
...
- [ ] Verify all logs flowing to ELK
...
Error Rate                      < 0.5%          < 1%             > 2%
Email Send Rate                 > 1000/min      > 500/min        < 100/min
Database Query Time (p95)       < 50ms          < 100ms          > 200ms
Frontend Load Time              < 2s            < 3s             > 5s
Uptime                          99.9%           99.5%            < 99%
Queue Processing Time           < 1s            < 2s             > 5s
```

### User Experience Metrics
```
Metric                          Target          Acceptable       Fail
Campaign Creation Time          < 5 min         < 10 min         > 30 min
Email Preview Speed             < 2s            < 5s             > 10s
Form Validation Speed           < 500ms         < 1000ms         > 2000ms
Template Selection              < 2s            < 5s             > 10s
...
```
Metric                          Target          Acceptable       Fail
Campaigns Launched (Daily)      50+             30+              < 10
Avg Campaign Size               5,000           3,000            < 1,000
Email Delivery Rate             > 98%           > 95%            < 90%
Customer Satisfaction           > 4.5/5         > 4/5            < 3/5
Support Tickets                 < 5/day         < 10/day         > 20/day
```
...
- **Escalation**: Team lead → Engineering lead → CTO

### Communication Channels
- **Slack**: #pmp-production for alerts
- **PagerDuty**: For critical incidents
- **Email**: ops@company.com for non-urgent
