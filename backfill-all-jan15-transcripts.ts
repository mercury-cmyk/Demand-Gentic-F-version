...
async function backfillAllJan15Transcripts() {
  ...
  const leadsResult = await pool.query<any>(`
    SELECT
      id,
  ...
  console.log(`  Found ${leadsResult.rows.length} leads needing transcription`);

  // 2. Get call_sessions with recordings but no transcript
  console.log("Fetching call_sessions...");
  const sessionsResult = await pool.query<any>(`
    SELECT
      id,
      recording_url AS "recordingUrl",
    ...
    WHERE created_at >= '2026-01-15'
      AND recording_url IS NOT NULL
      AND (ai_transcript IS NULL OR LENGTH(ai_transcript) < 50)
  ...
  console.log(`  Found ${sessionsResult.rows.length} call_sessions needing transcription`);

  // 3. Get test_calls - either with recording_url OR linked to call_session with transcript
  console.log("Fetching test_calls...");
  const testCallsResult = await pool.query<any>(`
    SELECT
      ctc.id,
    ...
    WHERE ctc.created_at >= '2026-01-15'
      ...
      AND (ctc.recording_url IS NOT NULL OR (cs.ai_transcript IS NOT NULL AND LENGTH(cs.ai_transcript) >= 50))
