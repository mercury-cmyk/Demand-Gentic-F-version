...
async function evaluateNotInterested() {
  ...
  if (withNotes < notInterestedCalls.rows.length * 0.5) {
    console.log('\n⚠️  WARNING: Most calls don\'t have notes recorded.');
    console.log('   The AI may not be capturing call details properly.');
    console.log('   Review recordings to evaluate disposition accuracy.');
  }

  // Duration distribution
  console.log('\n--- Duration Distribution ---');
  const durations = (notInterestedCalls.rows as any[]).map(c => c.call_duration_seconds);
