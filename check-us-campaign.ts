...
async function check() {
  ...
  console.log('\n=== CONTACTS READY FOR CALLING (next_attempt_at <= NOW) ===');
  console.log(`Count: ${nextAttempt.rows.length}`);
  for (const row of nextAttempt.rows as any[]) {
    console.log(`  ${row.first_name}: next_attempt=${row.next_attempt_at}, reason=${row.enqueued_reason || 'none'}`);
  }
  
  // Check campaign configuration
  const campaign = await db.execute(sql`
    SELECT id, name, status, dial_mode, ai_agent_settings
    FROM campaigns
    WHERE id = ${campaignId}
  `);
  
  console.log('\n=== CAMPAIGN CONFIG ===');
  const camp = campaign.rows[0] as any;
  console.log(`Name: ${camp.name}`);
  console.log(`Status: ${camp.status}`);
  console.log(`Dial Mode: ${camp.dial_mode}`);
  const settings = camp.ai_agent_settings || {};
  console.log(`Max Concurrent Calls: ${settings.maxConcurrentCalls || 'default (50)'}`);
  console.log(`Business Hours Config:`, settings.businessHours || 'not set');
