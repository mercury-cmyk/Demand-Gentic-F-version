...
async function analyzeNoAnswerDispositions() {
  ...
  for (const row of result.rows) {
    ...
    } else if (duration < 15 || !hasTranscript) {
      categories.trueNoAnswer.push(row);
    } else {
      categories.shortCall.push(row);
    }
  }
  ...
  console.log('=' .repeat(60));
  console.log(`  True No Answer (correct):        ${categories.trueNoAnswer.length}`);
  console.log(`  Had Conversation (MISCLASSIFIED): ${categories.hadConversation.length}`);
  console.log(`  Actually Voicemail:               ${categories.voicemail.length}`);
  console.log(`  AI Screener (Google Voice etc):    ${categories.screener.length}`);
  console.log(`  Language Barrier:                  ${categories.languageBarrier.length}`);
  console.log(`  Short/Ambiguous:                   ${categories.shortCall.length}`);
  console.log('=' .repeat(60));
  console.log('');

  // Detail the misclassified ones (had actual conversation)
