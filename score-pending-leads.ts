...
function scoreLead(
  ...
  // Summary section override
  const hasSummary = fullText.includes('[call summary]');
  const summaryMatch = fullText.match(/(\d+)\s*total turns\s*\((\d+)\s*agent,\s*(\d+)\s*contact\)/);
  let summaryContactTurns = -1;
  if (summaryMatch) summaryContactTurns = parseInt(summaryMatch[3]);
  
  let realContactTurns = realContactLines.length;
  if (hasSummary && summaryContactTurns === 0) realContactTurns = 0;

  const contactText = realContactLines.join(' ').toLowerCase();
  const contactSnippets = realContactLines.slice(0, 5);

  // Signal detection
  const posSignals = POSITIVE_SIGNALS.filter(s => contactText.includes(s));
  const negSignals = NEGATIVE_SIGNALS.filter(s => contactText.includes(s));
  const isVoicemail = VOICEMAIL_SIGNALS.some(s => fullText.includes(s)) && realContactTurns < 2;
  const isScreener = rawContactLines.some(l => isIVRLine(l)) && realContactTurns < 2;

  // Deep engagement checks
  const identityConfirmed = /yes.*(?:speaking|this is|that's me|it's me|i am)|speaking.*how can|this is (?:he|she)|yes that's me/i.test(contactText);
  ...
  if (duration < 15 && realContactTurns === 0) durationScore = 0;

  // --- Campaign Fit Score (0-10) ---
  ...
  if ((isVoicemail || isScreener) && realContactTurns < 2) qualificationStatus = 'not_qualified';

  // Build reasoning
  let reasoning = '';
  if (dncRequested) {
    reasoning = 'Contact requested DNC — not qualified';
  } else if (isVoicemail) {
    reasoning = 'Voicemail/answering machine — no human reached';
  } else if (isScreener && realContactTurns < 2) {
    reasoning = 'Call screener only — prospect never connected';
  } else if (realContactTurns === 0) {
    reasoning = `No real contact turns detected (${rawContactLines.length} raw lines filtered)`;
  } else if (meetingAgreed) {
    reasoning = `Meeting/appointment agreed — high qualification. Contact turns: ${realContactTurns}`;
  } else if (interestExpressed && requestedInfo) {
    reasoning = `Interest expressed + info requested. Positive signals: ${posSignals.slice(0, 3).join(', ')}`;
  } else if (interestExpressed) {
    reasoning = `Interest expressed. Signals: ${posSignals.slice(0, 3).join(', ')}. Turns: ${realContactTurns}`;
  } else if (callbackRequested) {
    reasoning = `Callback requested — contact engaged but timing wasn't right`;
  } else if (declined) {
    reasoning = `Contact declined. Negative signals: ${negSignals.slice(0, 3).join(', ')}`;
  } else if (realContactTurns >= 2) {
...
async function main() {
  ...
  for (const row of results.rows as any[]) {
    ...
    if (!transcript || transcript.length < 10) {
      // No transcript — score as 0
      scored.push({
        id: row.id,
        contactName: row.contact_name || 'Unknown',
        company: row.account_name || 'Unknown',
        campaign: row.campaign_name || 'Unknown',
        campaignType: row.campaign_type || 'call',
        qaStatus: row.qa_status,
        existingScore: row.ai_score ? parseFloat(row.ai_score) : null,
        existingQual: row.ai_qualification_status,
        result: {
          score: 0,
          qualificationStatus: 'not_qualified',
          analysis: {
            engagementLevel: 'none',
            realContactTurns: 0, rawContactTurns: 0, agentTurns: 0,
            positiveSignals: [], negativeSignals: [],
            identityConfirmed: false, interestExpressed: false, meetingAgreed: false,
            callbackRequested: false, declined: false, dncRequested: false,
            isVoicemail: false, isScreener: false,
            contactSnippets: [],
            reasoning: 'No transcript available',
            scoreBreakdown: { engagementScore: 0, interestScore: 0, qualificationScore: 0, durationScore: 0, campaignFitScore: 0 },
          },
        },
        transcript,
        duration,
        dcaDisposition: row.dca_disposition,
      });
      continue;
    }
  ...
  for (const r of ranges) {
    const count = scored.filter(s => s.result.score >= r.min && s.result.score <= r.max).length;
    const bar = '█'.repeat(Math.min(count, 40));
    console.log(`  ${r.label.padEnd(22)} ${String(count).padStart(3)} ${bar}`);
  }
  ...
  if (notQualified.length > 0) {
    console.log('\n' + '='.repeat(80));
    console.log(`  NOT QUALIFIED (Score < 40) [${notQualified.length}]`);
    console.log('='.repeat(80));
    for (const s of notQualified.slice(0, 15)) {
      console.log(`  ❌ [${s.result.score}] ${s.contactName} @ ${s.company} — ${s.result.analysis.reasoning.substring(0, 100)}`);
    }
    if (notQualified.length > 15) console.log(`  ... +${notQualified.length - 15} more`);
  }
  ...
  console.log(`  Not Qualified (<40):   ${notQualified.length}`);

  const engByLevel = {
    high: scored.filter(s => s.result.analysis.engagementLevel === 'high').length,
