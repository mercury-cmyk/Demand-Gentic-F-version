...
async function diagnoseAILeadsIssue() {
        ...
        if (realUserText.length > 20) {
          ...
          if (examples.length < 10) {
            examples.push({
              ...
              userText: realUserText.slice(0, 200),
  ...
  console.log(`\n  Checked: ${mismarked.rows.length} sessions with no_answer/voicemail disposition`);
  console.log(`  With user responses: ${withUserTurns}`);
  console.log(`  REAL conversations (not VM/IVR): ${realConversations}`);

  console.log('\n  Examples of REAL conversations wrongly marked:');
  for (const ex of examples) {
    console.log(`\n    Session: ${ex.id}`);
    console.log(`    Wrong Disposition: ${ex.disposition}`);
    console.log(`    User turns: ${ex.userTurns}`);
    console.log(`    User said: "${ex.userText}"`);
    
    try {
      const analysis = typeof ex.analysis === 'string' ? JSON.parse(ex.analysis) : ex.analysis;
      console.log(`    AI Analysis Outcome: ${analysis?.outcome || 'N/A'}`);
    } catch (e) {}
  }

  // 4. Check what determines disposition
  console.log('\n\n' + '='.repeat(80));
  console.log('🔬 CHECKING AI_ANALYSIS OUTCOME VALUES');
  console.log('='.repeat(80));
  
  const outcomes = await pool.query(`
    SELECT 
      ai_analysis->>'outcome' as outcome,
      ai_disposition,
      COUNT(*) as count
    FROM call_sessions
    WHERE campaign_id = $1
