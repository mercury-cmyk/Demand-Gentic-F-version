...
async function getTelnyxRecordingUrl(telnyxCallId: string): Promise<string | null> {
  try {
...
async function main() {
  try {
    ...
    for (let i = 0; i < result.rows.length; i++) {
      ...
      console.log(`[${idx}/${result.rows.length}] ${call.contact_email}`);
      ...
      try {
        // Try stored URL first (might still be valid)
        let recordingUrl = call.recording_url;
        let urlSource = "stored";

        if (recordingUrl) {
          const checkResponse = await fetch(recordingUrl, { method: "HEAD", timeout: 5000 }).catch(
            () => null
          );
