...
const orphans = await client.query(`
  ...
  WHERE created_at > '2026-03-04T05:00:00Z'
    AND created_at < '2026-03-04T06:00:00Z'
  ORDER BY created_at DESC
  LIMIT 10
`);

console.log('\n=== March 4 05:xx UTC call attempts ===');
for (const row of orphans.rows) {
  console.log(`  ID: ${row.id} | Duration: ${row.call_duration_seconds}s | Disp: ${row.disposition} | Phone: ${row.phone_dialed} | Created: ${new Date(row.created_at).toISOString()} | Started: ${row.call_started_at ? new Date(row.call_started_at).toISOString() : 'null'}`);
}

// Now check: are there call attempts with disposition_processed_at on March 5?
const march5 = await client.query(`
  SELECT
    id, call_duration_seconds, disposition,
    call_session_id IS NOT NULL as has_session,
    CASE WHEN full_transcript IS NOT NULL AND LENGTH(full_transcript) > 10 THEN LENGTH(full_transcript) ELSE 0 END as transcript_len,
