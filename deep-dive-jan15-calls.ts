...
async function deepDiveAnalysis() {
  ...
  calls.forEach(call => {
    ...
    if (uniqueRatio < 0.3) hasOnlyAgentSpeaking++;
    if (uniqueRatio > 0.5) hasBackAndForth++;
    if (hasTwoWay && !hasVM) {
      hasTwoWayConversation++;
      if (conversationSamples.length < 10) {
  ...
  console.log(`  Voicemail greetings detected: ${hasVoicemailGreeting} (${(hasVoicemailGreeting/calls.length*100).toFixed(1)}%)`);
  console.log(`  Has dialogue patterns: ${hasDialoguePattern} (${(hasDialoguePattern/calls.length*100).toFixed(1)}%)`);
  console.log(`  Repetitive/agent only: ${hasOnlyAgentSpeaking} (${(hasOnlyAgentSpeaking/calls.length*100).toFixed(1)}%)`);
  console.log(`  Varied content: ${hasBackAndForth} (${(hasBackAndForth/calls.length*100).toFixed(1)}%)`);
  console.log(`  Possible two-way conversations: ${hasTwoWayConversation} (${(hasTwoWayConversation/calls.length*100).toFixed(1)}%)\n`);
