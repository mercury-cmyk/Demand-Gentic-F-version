...
| Call Setup Time | <500ms | SIP INVITE to 200 OK |
| RTP Ports | 10,000 | Range 10000-20000 |
| Concurrent Calls (e2-standard-4) | 100-500 | Depends on media processing |
...
### Development
- Compute: e2-medium (~$35/month)
- Database: db-f1-micro (~$10/month)
- Redis: 1GB (~$30/month)
- **Total: ~$80/month**

### Production (HA)
- Compute: 3x e2-standard-4 + LB (~$400/month)
- Database: High-availability (~$150/month)
- Redis: HA (~$100/month)
- Network: ~$100/month
- **Total: ~$750/month**
...
timeout 5 bash -c 'cat < /dev/null > /dev/tcp/<PUBLIC_IP>/5060' && echo "OK" || echo "FAILED"
```

### SIP Registration
```bash
# Using SIPp
sipp -u user1 -s "sip:user@<PUBLIC_IP>" <PUBLIC_IP>:5060
```

### TURN Connectivity
```bash
# Using turnclient
turnclient -h <PUBLIC_IP> -p 3478 -u turnuser -w turnpass
```
