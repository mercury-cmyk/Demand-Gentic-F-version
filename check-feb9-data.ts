...
async function checkData() {
  ...
  const callsWithAnalysis = await db.select({ count: sql<number>`count(*)` })
    .from(callSessions)
    .where(and(
        gte(callSessions.startedAt, startOfDay),
        lt(callSessions.startedAt, endOfDay),
  ...
  const callsWithTranscript = await db.select({ count: sql<number>`count(*)` })
    .from(callSessions)
    .where(and(
        gte(callSessions.startedAt, startOfDay),
        lt(callSessions.startedAt, endOfDay),
  ...
  const qualityRecordsCount = await db.select({ count: sql<number>`count(*)` })
