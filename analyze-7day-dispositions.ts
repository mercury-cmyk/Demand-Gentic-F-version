...
function isVoicemailTranscript(transcript: string): boolean {
  ...
  // Check for very short transcripts with no conversation
  const words = transcript.split(/\s+/).filter(w => w.length > 2);
  if (words.length < 15) {
    // Very short - likely voicemail or no conversation
    return !hasRealConversation(transcript);
  }
  
  return false;
}

function hasRealConversation(transcript: string): boolean {
  if (!transcript || transcript.length < 30) return false;
  
  const lowerTranscript = transcript.toLowerCase();
  
  // Look for back-and-forth dialogue
  const hasHello = /hello|hi|good (morning|afternoon|evening)/i.test(transcript);
  const hasQuestion = /\?/.test(transcript) || /what|how|when|where|why|who/i.test(transcript);
  const hasResponse = /(yes|yeah|sure|okay|alright|no|not really|maybe)/i.test(transcript);
  
  // Check for speaker turns (Agent/Prospect pattern)
  const hasSpeakerLabels = /\b(agent|prospect|caller|rep|customer)\s*:/i.test(transcript);
...
function analyzeCall(call: CallRecord): AnalysisResult {
  ...
  const hasShortDuration = duration < 15;
  
  // Analyze disposition
  if (isVoicemail && !hasConversation) {
    suggestedDisposition = 'voicemail';
    confidence = 0.9;
    reason = 'Transcript contains voicemail patterns with no real conversation';
    
    if (call.leadId) {
      incorrectlyHasLead = true;
      reason += ' - LEAD SHOULD NOT EXIST';
    }
  } else if (hasConversation && qualSignals.isQualified && !isNotInterested) {
    suggestedDisposition = 'qualified_lead';
    confidence = 0.85;
    reason = `Qualification signals detected: ${qualSignals.signals.slice(0, 3).join(', ')}`;
    shouldHaveLead = true;
    
    if (!call.leadId) {
      reason += ' - MISSING LEAD!';
    }
  } else if (hasConversation && isNotInterested) {
    suggestedDisposition = 'not_interested';
    confidence = 0.85;
    reason = 'Contact explicitly expressed non-interest';
  } else if (hasConversation && hasLongDuration) {
    // Long conversation but no clear qualification - might be worth reviewing
    if (qualSignals.signals.length > 0) {
      suggestedDisposition = 'qualified_lead';
      confidence = 0.7;
  ...
  } else if (!transcript || transcript.length < 30) {
    if (hasShortDuration) {
      suggestedDisposition = 'no_answer';
      confidence = 0.7;
      reason = 'Short duration with no transcript';
    }
  }
  
  // Check for misclassified leads
  if (call.leadId) {
    const leadDisp = currentDisposition.toLowerCase();
    if (leadDisp.includes('voicemail') || leadDisp.includes('no_answer')) {
      incorrectlyHasLead = true;
      reason = 'Lead created from voicemail/no_answer disposition - SHOULD NOT EXIST';
    } else if (isVoicemail) {
      incorrectlyHasLead = true;
      reason = 'Lead created from voicemail transcript - SHOULD NOT EXIST';
    }
  }
  ...
  return {
    ...
    company: call.companyName || 'Unknown',
    campaign: call.campaignName || 'Unknown',
    currentDisposition,
    ...
    agentType: call.agentType || 'unknown',
...
async function main() {
  ...
  console.log('7-DAY DISPOSITION ANALYSIS');
  console.log('Analyzing calls from the last 7 days to identify disposition issues');
  console.log('='.repeat(120));
  console.log();
  ...
  // Query all calls from call_sessions with lead info
  ...
  // Also query dialer_call_attempts
  const dialerCallsResult = await db.execute(sql`
    SELECT 
      dca.id,
      ...
      l.id as "leadId",
      l.qa_status as "leadQaStatus",
      CASE WHEN dca.human_agent_id IS NOT NULL THEN 'human' ELSE 'ai' END as "agentType"
  ...
  const stats = {
    ...
    bySuggestedDisposition: {} as Record<string, number>,
  ...
  const byCampaign: Record<string, { total: number; leads: number; missing: number; incorrect: number }> = {};
  for (const a of analyses) {
