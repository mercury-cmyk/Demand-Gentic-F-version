...
async function updateRecordingUrls() {
  ...
  for (const row of callAttempts.rows) {
    ...
    const match = recordings.find(r => {
      ...
      return Math.abs(recDuration - callDuration) <= 1; // within 1 second
    });

    if (match && match.status === 'completed') {
      const downloadUrl = match.download_urls?.mp3 || match.download_urls?.wav;

      if (downloadUrl) {
        console.log(`  Found recording: ${match.id}`);
        console.log(`  Duration: ${Math.floor(match.duration_millis / 1000)}s`);
        console.log(`  URL type: ${match.download_urls?.mp3 ? 'MP3' : 'WAV'}`);

        // Update the lead with this URL
