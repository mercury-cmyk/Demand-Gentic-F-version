...
export type AiGovernanceProvider = z.infer<typeof aiGovernanceProviderSchema>;
...
export type AiGovernanceScope = z.infer<typeof aiGovernanceScopeSchema>;
...
export const AI_GOVERNANCE_TASKS = [
  ...
  label: string;
  description: string;
  allowedProviders: readonly AiGovernanceProvider[];
}>;

export const AI_GOVERNANCE_TASK_MAP: Record<
  AiGovernanceScope,
  {
    label: string;
    description: string;
...
export const AI_PROVIDER_LABELS: Record<AiGovernanceProvider, string> = {
...
export const AI_MODEL_SUGGESTIONS: Record<
  AiGovernanceScope,
  Partial<Record<AiGovernanceProvider, string[]>>
> = {
  voice_realtime: {
    google: [
...
export type AiModelPolicyMap = Record<AiGovernanceScope, AiGovernancePolicy>;
...
export const aiGovernancePoliciesSchema: z.ZodType<AiModelPolicyMap> = z.object({
  ...
  for (const [scopeKey, policy] of Object.entries(policies) as Array<[AiGovernanceScope, AiGovernancePolicy]>) {
...
export type AiGovernanceUpdateInput = z.infer<typeof aiGovernanceUpdateSchema>;
...
export function normalizeAiModelPolicies(input: unknown): AiModelPolicyMap {
  ...
  for (const scope of Object.keys(base) as AiGovernanceScope[]) {
    if (scope in (input as Record<string, unknown>)) {
      merged[scope] = {
        ...base[scope],
        ...((input as Record<string, unknown>)[scope] as Record<string, unknown>),
      };
    }
  }
