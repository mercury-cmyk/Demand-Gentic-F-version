...
async function generateIntelligenceDaily() {
  ...
  const failedAccounts: Array<{ account_id: string; name: string; retries: number }> = [];
    ...
    const batchPromises = batch.map(async (account) => {
      ...
      while (retries <= RETRY_LIMIT) {
        try {
          ...
          if (isRetryable && retries < RETRY_LIMIT) {
            console.log(`   ⚠️  Retryable error: ${accountName} - ${errorMsg.substring(0, 60)}...`);
            retries++;
            continue; // Retry
          } else {
            console.log(`   ❌ Failed: ${accountName} - ${errorMsg.substring(0, 80)}`);
            return {
              status: 'error',
              account: accountName,
              error: errorMsg,
