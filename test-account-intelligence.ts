...
async function testAccountIntelligence() {
  ...
  if (stats.accounts_fully_prepared < stats.total_accounts) {
    console.log(`\n❌ PROBLEM IDENTIFIED:`);
    console.log(`   ${unpreparedCount} accounts (${Math.round(unpreparedCount / stats.total_accounts * 100)}%) lack pre-generated intelligence data`);
    console.log(`\n   When a call is initiated to these accounts:`);
    console.log(`   1. buildSystemPrompt() calls getOrBuildAccountIntelligence(accountId)`);
    console.log(`   2. No cached intelligence found → triggers AI generation`);
    console.log(`   3. AI generation can take 5-30 seconds`);
    console.log(`   4. During generation, the call is already connected`);
    console.log(`   5. If generation times out → incomplete system prompt → SILENT CALL`);
    console.log(`\n💡 SOLUTION:`);
    console.log(`   Pre-generate intelligence for all accounts before starting campaigns`);
    console.log(`   Run: npm run generate-account-intelligence`);
  } else {
    console.log(`\n✅ All accounts have pre-generated intelligence!`);
    console.log(`   System prompts should build quickly and completely`);
    console.log(`\n   If calls are still silent, check:`);
    console.log(`   1. Virtual agent system prompt is not empty`);
    console.log(`   2. Campaign objective/context is configured`);
    console.log(`   3. Server logs for other errors during buildSystemPrompt()`);
  }

  console.log(`\n${"=".repeat(80)}`);
}

testAccountIntelligence()
  .then(() => {
    console.log("\n✅ Test complete");
    process.exit(0);
  })
