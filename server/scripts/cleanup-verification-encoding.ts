 ...
 * Usage: npx tsx server/scripts/cleanup-verification-encoding.ts <campaignId>
 */
...
if (!campaignId) {
  ...
  console.error('Usage: npx tsx server/scripts/cleanup-verification-encoding.ts <campaignId>');
  process.exit(1);
}
...
async function cleanupEncoding() {
  ...
  try {
    ...
    const textFields: Array<keyof typeof verificationContacts.$inferSelect> = [
      'fullName',
      'firstName',
      'lastName',
      'title',
    ...
    for (const contact of contacts) {
        ...
        if (value && typeof value === 'string') {
          if (hasCorruptCharacters(value)) {
            ...
            if (sanitized && sanitized !== value) {
              ...
              if (corruptionSamples.length < 20) {
                corruptionSamples.push(`${field}: "${value}" → "${sanitized}"`);
              }
            }
          }
        }
      ...
      // Update contact if corruption was found
      if (hasCorruption) {
        await db
        ...
        if (updatedContacts % 100 === 0) {
          console.log(`  Progress: ${updatedContacts} contacts fixed...`);
        }
      }
    }
    ...
    console.log(`   Corrupt fields fixed: ${totalCorruptFields}`);
