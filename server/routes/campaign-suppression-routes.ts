...
router.post('/:campaignId/suppressions/upload', async (req: Request, res: Response) => {
  try {
    ...
    const parseResult = parseCSVWithAutoDelimiter<Record<string, any>>(csvContent, {
    ...
    const { results: processedRows, errors: processingErrors, skipped } = await processCSVRows<Record<string, any>, ProcessedRow>(
      parseResult.data,
      (row, index) => {
