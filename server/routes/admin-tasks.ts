...
type TaskPriority = z.infer<typeof aiPrioritySchema>;
...
function formatAiTaskDetails(suggestion: z.infer<typeof aiTaskSuggestionSchema>): string {
  const blocks = [
...
function getLeastLoadedAssignee(workloads: Map<string, number>): string | null {
  if (workloads.size === 0) {
    return null;
  }
...
function bumpAssigneeLoad(workloads: Map<string, number>, assigneeName: string | null) {
  if (!assigneeName) {
    return;
  }
...
function sanitizeAiSuggestions(
  suggestions: Array<z.infer<typeof aiTaskSuggestionSchema>>,
...
function buildHeuristicTaskPlan(params: {
  ...
  objective: string;
  maxSuggestions: number;
  assigneeWorkload: Map<string, number>;
}) {
  ...
  const issues: Array<z.infer<typeof aiIssueSchema>> = [];
  ...
  const suggestions: Array<z.infer<typeof aiTaskSuggestionSchema>> = issues.map((issue, index) => {
  ...
  try {
    ...
    const assignmentSummary = Array.from(
      sanitizedSuggestions.reduce((acc, suggestion) => {
        ...
        const current = acc.get(key) || { assigneeName: key, plannedTasks: 0, priorities: new Set<string>() };
        current.plannedTasks += 1;
        current.priorities.add(suggestion.priority);
        acc.set(key, current);
        return acc;
      }, new Map<string, { assigneeName: string; plannedTasks: number; priorities: Set<string> }>()),
    ).map(([, value]) => ({
