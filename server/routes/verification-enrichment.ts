...
export async function autoTriggerEnrichment(
  campaignId: string,
  threshold: number = 20
): Promise<{ triggered: boolean; reason: string; stats?: any }> {
  try {
    // Check job title coverage threshold (Smart Workflow Trigger System)
    const [coverage] = await db
      .select({
        total: sql<number>`COUNT(*)`,
        withJobTitle: sql<number>`SUM(CASE WHEN ${verificationContacts.title} IS NOT NULL AND ${verificationContacts.title} != '' THEN 1 ELSE 0 END)`,
      })
    ...
    if (coveragePercent < threshold) {
      return {
        triggered: false,
        reason: `Job title coverage ${coveragePercent.toFixed(1)}% below ${threshold}% threshold`,
        stats: { totalEligible, withTitle, coveragePercent },
      };
    }
    ...
    const [needsEnrichment] = await db
      .select({
        needsPhone: sql<number>`SUM(CASE WHEN ${verificationContacts.aiEnrichedPhone} IS NULL THEN 1 ELSE 0 END)`,
        needsAddress: sql<number>`SUM(CASE WHEN ${verificationContacts.aiEnrichedAddress1} IS NULL OR ${verificationContacts.aiEnrichedCity} IS NULL THEN 1 ELSE 0 END)`,
      })
      .from(verificationContacts)
      .where(
        and(
...
export async function enrichCampaignInBackground(
  ...
  try {
    ...
    for (let i = 0; i < contactsNeedingEnrichment.length; i += batchSize) {
