...
router.get("/producer-metrics/summary", requireAuth, requireRole('admin', 'manager'), async (req, res) => {
  try {
    ...
    const summary = await db
      .select({
        producerType: producerMetrics.producerType,
        totalCalls: sql<number>`SUM(${producerMetrics.totalCalls})::int`,
        connectedCalls: sql<number>`SUM(${producerMetrics.connectedCalls})::int`,
        qualifiedLeads: sql<number>`SUM(${producerMetrics.qualifiedLeads})::int`,
        qcPassedLeads: sql<number>`SUM(${producerMetrics.qcPassedLeads})::int`,
        qcFailedLeads: sql<number>`SUM(${producerMetrics.qcFailedLeads})::int`,
        dncRequests: sql<number>`SUM(${producerMetrics.dncRequests})::int`,
        optOutRequests: sql<number>`SUM(${producerMetrics.optOutRequests})::int`,
        handoffsToHuman: sql<number>`SUM(${producerMetrics.handoffsToHuman})::int`,
        avgCallDuration: sql<number>`AVG(${producerMetrics.avgCallDuration})::numeric(10,2)`,
        avgQualityScore: sql<number>`AVG(${producerMetrics.avgQualityScore})::numeric(5,2)`,
        avgConversionRate: sql<number>`AVG(${producerMetrics.conversionRate})::numeric(5,4)`,
        avgContactabilityRate: sql<number>`AVG(${producerMetrics.contactabilityRate})::numeric(5,4)`,
      })
      .from(producerMetrics)
      .where(whereClause)
