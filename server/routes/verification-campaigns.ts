...
router.put("/api/verification-campaigns/:id", async (req, res) => {
  try {
    ...
    const validatedData = updateSchema.parse(req.body) as Partial<typeof verificationCampaigns.$inferInsert>;
...
router.get("/api/verification-campaigns/:id/export-smart", async (req, res) => {
  try {
    ...
    console.log(`[SMART EXPORT] ✓ Exporting ${contactsToExport.length} eligible contacts from ${contactsRaw.length} total`);
    console.log(`[SMART EXPORT]   Quality Breakdown:`);
    ...
    console.log(`[SMART EXPORT]   • Incomplete (4+ blanks): ${incompleteContacts.length} contacts`);
    ...
    try {
      ...
      const [stats] = await db
        .select({
          ...
          eligible: sql<number>`SUM(CASE WHEN ${verificationContacts.eligibilityStatus} = 'Eligible' THEN 1 ELSE 0 END)`,
          outOfScope: sql<number>`SUM(CASE WHEN ${verificationContacts.eligibilityStatus} = 'Out_of_Scope' THEN 1 ELSE 0 END)`,
          needsEmailValidation: sql<number>`SUM(CASE WHEN ${verificationContacts.emailStatus} IS NULL OR ${verificationContacts.emailStatus} = 'unknown' THEN 1 ELSE 0 END)`,
          needsAddressEnrichment: sql<number>`SUM(CASE WHEN ${verificationContacts.contactAddress1} IS NULL OR ${verificationContacts.contactCity} IS NULL THEN 1 ELSE 0 END)`,
          needsPhoneEnrichment: sql<number>`SUM(CASE WHEN ${verificationContacts.phone} IS NULL AND ${verificationContacts.mobile} IS NULL THEN 1 ELSE 0 END)`,
        })
        .from(verificationContacts)
        .where(
          and(
