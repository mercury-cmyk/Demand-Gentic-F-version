...
router.get("/credentials", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // PRIORITY 1: Try to get config from database
    ...
    if (config) {
      ...
      callerIdNumber = config.callerIdNumber || undefined;
    } else {
      // FALLBACK: Use environment variables
      console.log(`${LOG_PREFIX} No database config, using environment variables`);
