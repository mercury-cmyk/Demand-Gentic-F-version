...
/**
 * POST /api/preview-studio/generate-email
 * Generate personalized email preview with account and contact context
 */
router.post("/generate-email", requireAuth, async (req, res) => {
  try {
    const { campaignId, accountId, contactId, emailType } = req.body;

    if (!campaignId || !accountId) {
      return res.status(400).json({ message: "Campaign and Account are required" });
    }

    console.log(`[Preview Studio] Generating email preview for campaign: ${campaignId}, account: ${accountId}`);
    ...
    // Get account intelligence for context
    let accountIntelligence = null;
    try {
      const intelligenceRecord = await getOrBuildAccountIntelligence(accountId);
      accountIntelligence = intelligenceRecord?.payloadJson;
    } catch (e) {
      console.warn(`[Preview Studio] Could not get account intelligence:`, e);
    }
    ...
    const contextParts = [
      ...
      campaign.valueProposition ? `Value Proposition: ${campaign.valueProposition}` : '',
      ...
      account.employeeCount ? `Size: ${account.employeeCount} employees` : '',
    ];

    if (contact) {
      contextParts.push(
        `\nCONTACT: ${contact.fullName || `${contact.firstName || ''} ${contact.lastName || ''}`.trim()}`,
    ...
    if (accountIntelligence) {
      const intel = accountIntelligence as any;
      if (intel.businessSummary) contextParts.push(`\nBusiness Summary: ${intel.businessSummary}`);
      if (intel.keyPainPoints?.length) contextParts.push(`Pain Points: ${intel.keyPainPoints.join(', ')}`);
      if (intel.recentNews?.length) contextParts.push(`Recent News: ${intel.recentNews.slice(0, 2).join('; ')}`);
    }
    ...
    const emailTypeDescriptions: Record<string, string> = {
      cold_outreach: 'First outreach to a new prospect - should be attention-grabbing and problem-focused',
      follow_up: 'Follow-up after initial contact - should reference previous interaction',
      meeting_request: 'Request for a call or demo - should be concise with clear value',
      nurture: 'Value-add content sharing - educational and helpful',
      breakup: 'Final outreach attempt - create urgency without being pushy',
    };

    const emailTypeDesc = emailTypeDescriptions[emailType] || emailTypeDescriptions.cold_outreach;

    // Generate email using Vertex AI
    const prompt = `You are an expert B2B demand generation copywriter. Generate a personalized ${emailType?.replace(/_/g, ' ') || 'cold outreach'} email.

Context:
${context}

Email Type: ${emailTypeDesc}

IMPORTANT: The email MUST be personalized to ${account.name}${contact ? ` and ${contact.fullName || contact.firstName}` : ''}. Reference specific details about their business.

Generate a complete email with:
1. Subject line (compelling, under 60 chars)
2. Preheader (preview text, under 100 chars)
3. Hero title (bold headline)
4. Hero subtitle (supporting text)
5. Intro paragraph (2-3 sentences, personalized to the account)
6. 3 value bullets (specific to their industry/needs)
7. CTA label (action button text)
8. Closing line
...
Return ONLY valid JSON:
{
  ...
  "closingLine": "..."
    ...
    const emailContent = await generateJSON<any>(prompt, {
      temperature: 0.7,
      maxTokens: 2000,
    });

    // Build HTML template
    const { buildBrandedEmailHtml } = await import("../../client/src/components/email-builder/ai-email-template");
    
    const html = buildBrandedEmailHtml({
      copy: {
        subject: emailContent.subject || 'Your Subject Here',
        preheader: emailContent.preheader,
        heroTitle: emailContent.heroTitle || 'Welcome',
        heroSubtitle: emailContent.heroSubtitle || '',
        intro: emailContent.intro || '',
        valueBullets: emailContent.valueBullets || ['Benefit 1', 'Benefit 2', 'Benefit 3'],
        ctaLabel: emailContent.ctaLabel || 'Learn More',
        closingLine: emailContent.closingLine || 'Best regards',
      },
    ...
    res.json({
      ...
      contact: contact ? { id: contact.id, name: contact.fullName || contact.firstName } : null,
    });

  } catch (error: any) {
    console.error("[Preview Studio] Email generation error:", error);
