...
async function verifyEmailsBulk(
  ...
  options: {
    ...
    onCircuitOpen?: () => Promise<void>; // Callback when circuit breaker opens
  } = {}
  ...
  for (let miniBatchIndex = 0; miniBatchIndex < totalMiniBatches; miniBatchIndex++) {
    const miniBatchStart = miniBatchIndex * MINI_BATCH_SIZE;
    const miniBatchEnd = Math.min(miniBatchStart + MINI_BATCH_SIZE, emails.length);
    const miniBatch = emails.slice(miniBatchStart, miniBatchEnd);
    
    // Pool health check before processing mini-batch
    const { poolMetrics } = await import("../db");
    const poolStats = poolMetrics.getStats();
    
    // Circuit breaker: if pool is under pressure, pause and notify
    if (poolMetrics.isCircuitOpen()) {
      console.warn(`[verifyEmailsBulk] Circuit breaker OPEN - Pool under pressure:`, poolStats);
      ...
      const backoffDelay = Math.min(5000 * Math.pow(2, Math.min(miniBatchIndex, 3)), 30000);
      console.warn(`[verifyEmailsBulk] Pausing for ${backoffDelay}ms to allow pool recovery...`);
    ...
    for (let i = 0; i < miniBatch.length; i++) {
      const email = miniBatch[i];
      const globalIndex = miniBatchStart + i;
      
      try {
        const validation = await validateEmail3Layer(email);
        
        const layer1 = validation.layer1;
        const layer2 = validation.layer2;
        
        results.set(email, {
          email,
          status: validation.status,
          details: {
            syntax: layer1?.syntaxValid || false,
            domain: layer1?.hasMx || false,
            smtp: false,
            catch_all: layer2?.kickboxAcceptAll || false,
            disposable: layer2?.kickboxDisposable || layer1?.isDisposable || false,
            free: layer2?.kickboxFree || layer1?.isFree || false,
            role: layer2?.kickboxRole || layer1?.isRole || false,
          },
          reason: layer2?.kickboxReason || validation.eligibilityReason || validation.status,
          provider: validation.provider || 'kickbox',
          rawResponse: layer2?.rawResponse || layer1 || validation,
          checkedAt: new Date(),
          kickboxResult: layer2?.kickboxResult,
          kickboxReason: layer2?.kickboxReason,
          kickboxScore: layer2?.kickboxScore,
          kickboxAcceptAll: layer2?.kickboxAcceptAll,
          kickboxDisposable: layer2?.kickboxDisposable,
          kickboxFree: layer2?.kickboxFree,
          kickboxRole: layer2?.kickboxRole,
          kickboxDidYouMean: layer2?.kickboxDidYouMean,
        ...
        if (onProgress) {
          onProgress(globalIndex + 1, emails.length, email);
        }

        if (delayMs > 0 && i < miniBatch.length - 1) {
    ...
    if (miniBatchIndex < totalMiniBatches - 1) {
  ...
  try {
    ...
    const queueItems = await db.execute(sql`
      ...
      SELECT 
        ...
        a.hq_city,
        a.hq_country,
        a.main_phone as account_main_phone,
    ...
    // Enrich contacts with priority tier information for UI badges
    const enrichedData = queueItems.rows.map((row: any) => {
...
router.post("/api/verification-campaigns/:campaignId/contacts/run-email-validation", requireAuth, async (req, res) => {
  try {
    ...
    const byDomain = new Map<string, typeof contactsToProcess>();
  ...
  try {
    ...
    for (let i = 0; i < contactIds.length; i++) {
      ...
      try {
        ...
        // Inclusive email validation: exclude only explicitly invalid emails
        // Includes: null/unchecked, unknown, unverified, valid, safe_to_send, and any other status
        if (contact.emailStatus === 'invalid') {
          console.log(`[BULK ENRICHMENT] Skipping contact ${contactId} - email status is invalid`);
          skipped++;
...
export async function processEmailValidationJob(jobId: string) {
  ...
  try {
    ...
    // Reduced batch size from 500 to 150 to prevent pool exhaustion
    // Architect guidance: Smaller batches allow better pool management and checkpointing
    ...
    console.log(`[EMAIL VALIDATION JOB] Job ${jobId}: Processing ${allContactIds.length} contacts in ${totalBatches} batches (BATCH_SIZE=${BATCH_SIZE})`);
      ...
      for (let i = 0; i < emailsToVerify.length; i += CACHE_QUERY_CHUNK_SIZE) {
        ...
        const cachedValidationsChunk = await db
          .select({
            ...
            // Include all Kickbox fields for proper caching
            kickbox_result: verificationEmailValidations.kickboxResult,
            kickbox_reason: verificationEmailValidations.kickboxReason,
            kickbox_score: verificationEmailValidations.kickboxScore,
            kickbox_accept_all: verificationEmailValidations.kickboxAcceptAll,
            kickbox_disposable: verificationEmailValidations.kickboxDisposable,
            kickbox_free: verificationEmailValidations.kickboxFree,
            kickbox_role: verificationEmailValidations.kickboxRole,
            risk_level: verificationEmailValidations.riskLevel,
            is_business_email: verificationEmailValidations.isBusinessEmail,
            email_eligible: verificationEmailValidations.emailEligible,
            eligibility_reason: verificationEmailValidations.eligibilityReason,
            deep_verified_at: verificationEmailValidations.deepVerifiedAt,
          })
          .from(verificationEmailValidations)
          .where(
            and(
