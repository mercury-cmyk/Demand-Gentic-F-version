...
router.get(
  ...
  async (_req: Request, res: Response) => {
    try {
      ...
      const transcriptStats = await db.execute<{
        status: string;
        count: string;
      }>(sql`
      ...
      const dispositionStats = await db.execute<{
        status: string;
        count: string;
      }>(sql`
      ...
      const lastRun = await db.execute<{ created_at: string }>(sql`
      ...
      // Review queue count
      const reviewQueueResult = await db.execute<{ count: string }>(sql`
      ...
      const retranscriptionQueueResult = await db.execute<{ count: string }>(sql`
        SELECT count(*)::text as count
        FROM transcript_quality_assessments
      ...
      // Build response
      const tStats: Record<string, number> = { missing: 0, partial: 0, complete: 0, failed: 0 };
      for (const row of (transcriptStats.rows || [])) {
        tStats[row.status] = parseInt(row.count, 10);
      }

      const dStats: Record<string, number> = { pending: 0, validated: 0, mismatch: 0, auto_corrected: 0, reviewed: 0 };
