...
const providerActive: Record<string, number> = { openai: 0, google: 0, kimi: 0 };
const providerQueues: Record<string, Array<{ resolve: () => void; reject: (err: Error) => void; timer: ReturnType<typeof setTimeout> }>> = {
  openai: [], google: [], kimi: [],
};
...
function acquire(provider?: AiProvider): Promise<void> {
  ...
  if (activeCount < MAX_CONCURRENT_AI_CALLS) {
    activeCount++;
    if (provider) providerActive[provider]++;
  ...
  if (provider) {
    ...
    if (providerQueue.length > 0) {
      ...
      if (activeCount < MAX_CONCURRENT_AI_CALLS) {
        ...
        // Put it in the global queue instead
        waitQueue.push({ resolve: () => { providerActive[provider]++; next.resolve(); }, reject: next.reject, timer: next.timer });
      }
      return;
    }
 ...
 */
export async function withAiConcurrency<T>(fn: () => Promise<T>, label?: string, provider?: AiProvider): Promise<T> {
