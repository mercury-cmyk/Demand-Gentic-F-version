 ...
 */
export function parseCSVWithAutoDelimiter<T = any>(
  csvData: string,
  options: Omit<CSVParseOptions, 'delimiter'> = {}
): CSVParseResult<T> {
  const delimiters = [',', '\t', '|', ';'];
  let bestResult: Papa.ParseResult<T> | null = null;
  let bestScore = -1;
  ...
  return bestResult as CSVParseResult<T>;
}
...
export function parseCSV<T = any>(
  ...
  if (options.delimiter === 'auto' || !options.delimiter) {
    return parseCSVWithAutoDelimiter<T>(csvData, options);
  }
 ...
 */
export function parseCSVStream<T = any>(
  csvContent: string,
 ...
 */
export async function processCSVRows<TInput, TOutput>(
  rows: TInput[],
  processor: (row: TInput, index: number) => TOutput | null | Promise<TOutput | null>,
  options: {
    ...
    try {
      ...
      if (result === null) {
        ...
        if (options.collectErrors) {
          errors.push(`Row ${i + 1}: Invalid or incomplete data`);
        }
      } else {
        results.push(result);
      }
