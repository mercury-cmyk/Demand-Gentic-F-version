...
let workflowQueue: Queue<VerificationWorkflowJob> | null = null;
let workflowWorker: Worker<VerificationWorkflowJob> | null = null;

export function getVerificationWorkflowQueue(): Queue<VerificationWorkflowJob> | null {
  if (!isQueueAvailable()) {
  ...
  if (!workflowQueue) {
    workflowQueue = createQueue<VerificationWorkflowJob>(QUEUE_NAME, {
      attempts: 3,
      backoff: {
        type: 'exponential',
        delay: 5000
...
export function startVerificationWorkflowWorker() {
  ...
  workflowWorker = createWorker<VerificationWorkflowJob>(
    QUEUE_NAME,
...
async function processVerificationWorkflow(job: any): Promise<void> {
...
async function runEligibilityCheck(campaignId: string, workflowId: string): Promise<WorkflowStage> {
...
async function runEmailValidation(campaignId: string, workflowId: string): Promise<WorkflowStage> {
...
async function runAddressEnrichment(campaignId: string, workflowId: string): Promise<WorkflowStage> {
...
async function runPhoneEnrichment(campaignId: string, workflowId: string): Promise<WorkflowStage> {
...
async function acquireCampaignLock(campaignId: string): Promise<boolean> {
