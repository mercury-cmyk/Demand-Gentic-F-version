...
export class SignatureService {
  ...
  async getSignatureById(userId: string, signatureId: string): Promise<EmailSignature | null> {
    const [signature] = await db
      .select()
      .from(emailSignatures)
      .where(and(
  ...
  async setDefaultSignature(userId: string, signatureId: string): Promise<EmailSignature | null> {
    await db
      .update(emailSignatures)
      .set({ isDefault: false, updatedAt: new Date() })
      .where(eq(emailSignatures.userId, userId));
