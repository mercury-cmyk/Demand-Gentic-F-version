...
export function getVariantForContact(
  ...
  return position < splitPercentage ? 'A' : 'B';
}
...
export async function trackABTestMetric(
  ...
  const variantMetrics = metrics[`variant${variant}`];

  // Increment counter
  if (metric === 'open') variantMetrics.opened++;
  if (metric === 'click') variantMetrics.clicked++;
  if (metric === 'bounce') variantMetrics.bounced++;
  ...
  await calculateSignificance(testId);
}
...
export async function calculateSignificance(testId: string) {
  ...
  const metrics = test.metrics as any;
  const metricA = metrics.variantA;
  const metricB = metrics.variantB;

  // Chi-square formula: Σ((observed - expected)² / expected)
  const totalSent = metricA.sent + metricB.sent;
  const totalOpened = metricA.opened + metricB.opened;

  if (totalSent === 0) return;

  // Expected values under null hypothesis
  const expectedOpenRateA = (totalOpened / totalSent) * metricA.sent;
  const expectedOpenRateB = (totalOpened / totalSent) * metricB.sent;

  // Chi-square calculation
  const chiSquare =
    Math.pow(metricA.opened - expectedOpenRateA, 2) / expectedOpenRateA +
    Math.pow(metricB.opened - expectedOpenRateB, 2) / expectedOpenRateB;

  // Convert to p-value (simplified for 1 degree of freedom)
  // Chi-square critical value at 0.05 significance = 3.841
  // Chi-square critical value at 0.01 significance = 6.635
  const pValue = chiSquare > 3.841 ? 1 - (chiSquare / 10) : 0;
...
export async function exportResults(testId: string): Promise<string> {
  const test = await db.query.abTests.findFirst({
    where: eq(abTests.id, testId),
  });
