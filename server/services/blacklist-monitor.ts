 ...
 */
export async function checkIpBlacklists(ip: string): Promise<BlacklistSummary> {
 ...
 */
export async function checkDomainBlacklists(domain: string): Promise<BlacklistSummary> {
...
export async function comprehensiveBlacklistCheck(domain: string): Promise<{
  domain: BlacklistSummary;
  mxIps?: BlacklistSummary[];
}> {
...
export async function getMonitorsDueForCheck(
  limit: number = 100
): Promise<typeof blacklistMonitors.$inferSelect[]> {
  const now = new Date();
...
export async function getDomainBlacklistSummary(domainAuthId: number): Promise<{
  ...
  cleanMonitors: number;
  listedMonitors: number;
