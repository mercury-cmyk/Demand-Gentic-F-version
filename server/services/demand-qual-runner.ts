...
async function analyzeTranscriptForBANT(
  ...
  bantScore: Omit<BANTScore, 'overall'>;
  bantEvidence: BANTEvidence;
  objectionsDetected: ObjectionDetected[];
  escalationTriggers: EscalationTrigger[];
 ...
 */
export async function quickBANTScore(transcript: string): Promise<BANTScore> {
 ...
 */
export async function hasEscalationTriggers(transcript: string): Promise<boolean> {
  const result = await processQualificationFromCall({
    transcript,
  });
 ...
 */
export async function extractObjections(transcript: string): Promise<ObjectionDetected[]> {
  const result = await processQualificationFromCall({
    transcript,
  });
