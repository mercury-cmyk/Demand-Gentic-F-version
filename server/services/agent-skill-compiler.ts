...
export interface SkillCompilationInput {
  agentName: string;
  skillId: string;
  skillInputValues: Record<string, any>; // User-provided values for skill inputs
...
- **Brand Trust Index:** Low opt-out rates (<2%), positive prospect feedback (>8/10)
  ...
  for (const input of skill.requiredInputs) {
    ...
    if (providedInputs[input.key] && input.validation) {
      ...
      if (input.validation.minLength && value.length < input.validation.minLength) {
        errors.push(`${input.label} must be at least ${input.validation.minLength} characters`);
      }
...
export async function previewCompiledPrompt(
  input: SkillCompilationInput
): Promise<{ preview: string; wordCount: number; estimatedTokens: number }> {
