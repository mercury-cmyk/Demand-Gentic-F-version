 ...
 */
export async function getKnowledgeBlockById(id: number): Promise<KnowledgeBlock | null> {
 ...
 */
export async function getKnowledgeBlockBySlug(slug: string): Promise<KnowledgeBlock | null> {
  const [block] = await db
    .select()
    .from(knowledgeBlocks)
 ...
 */
export async function deleteKnowledgeBlock(id: number): Promise<boolean> {
 ...
 */
export const DEFAULT_KNOWLEDGE_BLOCKS: Omit<InsertKnowledgeBlock, 'createdBy'>[] = [
 ...
 */
export async function areKnowledgeBlocksInitialized(): Promise<boolean> {
