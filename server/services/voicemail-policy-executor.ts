...
export class VoicemailPolicyExecutor {
  ...
  async executeMachinePolicy(
  ...
  ): Promise<VoicemailExecutionResult> {
    ...
    if (policy.vm_cooldown_hours) {
      ...
      if (lastVm) {
        ...
        if (hoursSinceLastVm < policy.vm_cooldown_hours) {
          console.log(`[VoicemailPolicy] Contact ${contactId} in cooldown period (${hoursSinceLastVm.toFixed(1)}h < ${policy.vm_cooldown_hours}h)`);
          return { action: 'skipped', reason: 'cooldown_period_active' };
        }
      }
    }

    // Check local time window if specified
    if (policy.vm_local_time_window) {
      const contact = await storage.getContact(contactId);
      if (contact && !this.isWithinLocalTimeWindow(contact.timezone || undefined, policy.vm_local_time_window)) {
        console.log(`[VoicemailPolicy] Contact ${contactId} outside local time window`);
        return { action: 'skipped', reason: 'outside_local_time_window' };
      }
    }
    ...
    switch (policy.action) {
      case 'leave_voicemail':
        return await this.leaveVoicemail(contactId, campaignId, callAttemptId, policy, queue);
      
      case 'schedule_callback':
        return await this.scheduleCallback(contactId, campaignId, callAttemptId, policy);
      
      case 'drop_silent':
        return this.dropSilent(callAttemptId);
      
      default:
        return { action: 'skipped', reason: 'unknown_action' };
    }
  }

  /**
   * Leave voicemail message (TTS or audio file)
   */
   ...
   */
  private async getContactVmCount(contactId: string, campaignId: string): Promise<number> {
   ...
   */
  private async getLastVoicemail(contactId: string, campaignId: string): Promise<Date | null> {
    const tracking = await db.query.contactVoicemailTracking.findFirst({
      where: and(
        eq(contactVoicemailTracking.contactId, contactId),
        eq(contactVoicemailTracking.campaignId, campaignId)
   ...
   */
  private async getCampaignDailyVmCount(campaignId: string): Promise<number> {
  ...
  private isWithinLocalTimeWindow(
    ...
    try {
      ...
      console.error('[VoicemailPolicy] Error checking time window:', error);
   ...
   */
  private substituteTokens(template: string, context: VoicemailTokenContext): string {
    let result = template;

    // Simple token substitution using regex
    const tokenPattern = /\{\{([^}]+)\}\}/g;
    
    result = result.replace(tokenPattern, (match, path) => {
