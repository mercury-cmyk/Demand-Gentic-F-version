...
export class DealConversationService {
  private tenantMailboxes: Set<string> = new Set();
  ...
  private async getTenantMailboxAddresses(): Promise<Set<string>> {
    const now = Date.now();
    if (this.tenantMailboxes.size > 0 && (now - this.lastMailboxSync) < this.MAILBOX_CACHE_TTL) {
      return this.tenantMailboxes;
    }

    const allMailboxes = await storage.getAllMailboxAccounts();
    this.tenantMailboxes = new Set(
      allMailboxes
        .map(m => m.mailboxEmail)
        .filter(Boolean)
  ...
  private async filterExternalParticipants(emails: string[]): Promise<string[]> {
  ...
  async findMatchingOpportunities(participantEmails: string[]): Promise<PipelineOpportunity[]> {
  ...
  async processEmailSync(context: EmailSyncContext): Promise<{ conversationsCreated: number; messagesCreated: number }> {
