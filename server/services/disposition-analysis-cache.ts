 ...
 * - Keys: disposition:<callSessionId> (direct mapping)
 * - TTL: 14 days (analysis results are stable)
 *
 * Performance Impact:
 * - Cache hits: <10ms (vs 1-2.5s for AI call)
 * - Expected hit rate: 80%+ on page refreshes
 * - Memory savings: 7.5MB → <100KB per batch
 */

import type Redis from "ioredis";
import { createHash } from "crypto";
import { isRedisConfigured, getRedisUrl, getRedisConnectionOptions } from "../lib/redis-config";

const LOG_PREFIX = "[DispositionCache]";
...
export class DispositionAnalysisCache {
  ...
  private inMemoryCache: Map<string, { createdAt: number; value: DeepAnalysisOutput }> = new Map();
  ...
  async getMany(callSessionIds: string[]): Promise<Map<string, DeepAnalysisOutput>> {
    const result = new Map<string, DeepAnalysisOutput>();
    ...
    if (this.isConnected && this.redis) {
      try {
        ...
        if (pipelineResults) {
          for (let i = 0; i < callSessionIds.length; i++) {
            const [err, val] = pipelineResults[i] as [Error | null, string | null];
            if (!err && val) {
              try {
                const parsed = JSON.parse(val);
                result.set(callSessionIds[i], parsed);
                // Also populate in-memory
                this.inMemoryCache.set(callSessionIds[i], { createdAt: Date.now(), value: parsed });
              } catch {
   ...
   */
  async invalidateMany(callSessionIds: string[]): Promise<void> {
  ...
  private pruneInMemoryCache(): void {
    if (this.inMemoryCache.size <= IN_MEMORY_MAX_SIZE) return;

    const sorted = Array.from(this.inMemoryCache.entries())
      .sort((a, b) => a[1].createdAt - b[1].createdAt);

    const toDelete = this.inMemoryCache.size - IN_MEMORY_MAX_SIZE;
    for (let i = 0; i < toDelete; i++) {
      this.inMemoryCache.delete(sorted[i][0]);
    }
  }
   ...
   */
  getStats(): { memoryEntriesCount: number; isRedisConnected: boolean } {
    return {
      memoryEntriesCount: this.inMemoryCache.size,
      isRedisConnected: this.isConnected,
    };
  }

  /**
   * Shutdown cache service (for graceful shutdown)
   */
