...
export class EmailSequenceProcessor {
  private queue: ReturnType<typeof createQueue<EmailSendJobData>>;
  private worker: ReturnType<typeof createWorker<EmailSendJobData>>;

  constructor() {
    this.queue = createQueue<EmailSendJobData>("email-sequence-sends", {
      attempts: 3,
      backoff: {
        type: "exponential",
    ...
    this.worker = createWorker<EmailSendJobData>(
   ...
   * Supports: {{firstName}}, {{lastName}}, {{fullName}}, {{email}}, {{company}}, etc.
   */
  private personalizeText(text: string, context: PersonalizationContext): string {
   ...
   */
  private async processEmailSend(job: any): Promise<void> {
   ...
   */
  async scheduleReadyEmails(): Promise<number> {
    if (!this.queue) {
