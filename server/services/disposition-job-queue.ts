 ...
 *
 * Performance Impact:
 * - User gets response in <100ms (job ID returned)
 * - Can process 100+ concurrent jobs
...
import {
  ...
  type DeepReanalysisSummary,
} from "./disposition-deep-reanalyzer";

const LOG_PREFIX = "[DispositionJobQueue]";

export interface DispositionAnalysisJobData {
...
export interface DispositionJobProgress {
...
export interface DispositionJobResult {
  jobId: string;
  status: "completed" | "failed";
  result?: DeepReanalysisSummary;
  error?: string;
  completedAt: string;
}

let queue: Queue<DispositionAnalysisJobData> | null = null;
let worker: Worker<DispositionAnalysisJobData> | null = null;
 ...
 */
export async function initializeDispositionQueue(): Promise<boolean> {
  ...
  try {
    ...
    // Create queue
    queue = new Queue<DispositionAnalysisJobData>("disposition-analysis", {
    ...
    worker = new Worker<DispositionAnalysisJobData>(
 ...
 */
export async function getJobStatus(jobId: string): Promise<DispositionJobProgress | null> {
 ...
 * Get job result
 */
export async function getJobResult(jobId: string): Promise<DispositionJobResult | null> {
  if (!queue) return null;
...
function estimateAnalysisDuration(limit: number): number {
  ...
  if (limit <= 10) return 15; // Small batch: 15 seconds
  if (limit <= 50) return 40 + Math.ceil((limit - 10) * 0.6); // 40s + additional
  if (limit <= 100) return 70 + Math.ceil((limit - 50) * 0.5);
  if (limit <= 200) return 100 + Math.ceil((limit - 100) * 0.4);
  return 150 + Math.ceil((limit - 200) * 0.3);
}
