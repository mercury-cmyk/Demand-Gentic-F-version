 ...
 */
async function processEngagementTriggers(): Promise<number> {
  const now = new Date();
 ...
 */
async function processPipelineActions(): Promise<number> {
  const now = new Date();
 ...
 */
async function executePipelineAction(action: typeof unifiedPipelineActions.$inferSelect): Promise<void> {
  const aiCtx = action.aiGeneratedContext as Record<string, any> | null;
