...
const SCORING_PARAMS = {
  // Answer rate: baseline 40%, target 70%
  ANSWER_RATE_BASELINE: 0.40,
  ANSWER_RATE_TARGET: 0.70,
  
  // Duration: baseline 10s, target 120s
  DURATION_BASELINE_SEC: 10,
  DURATION_TARGET_SEC: 120,
  
  // Short call: acceptable < 5%, concerning > 15%
  SHORT_CALL_GOOD: 0.05,
  SHORT_CALL_BAD: 0.15,
  
  // Hangup: acceptable < 3%, concerning > 10%
  HANGUP_GOOD: 0.03,
  HANGUP_BAD: 0.10,
  
  // Voicemail: acceptable < 20%, concerning > 40%
  VOICEMAIL_GOOD: 0.20,
  VOICEMAIL_BAD: 0.40,
  
  // Failure: acceptable < 2%, concerning > 8%
  FAILURE_GOOD: 0.02,
  FAILURE_BAD: 0.08,
};
...
export async function calculateReputation(
  numberId: string
): Promise<ReputationScoreDetails> {
...
export async function recalculateAllReputations(): Promise<{
  processed: number;
  failed: number;
  bandCounts: Record<ReputationBand, number>;
}> {
  ...
  const bandCounts: Record<ReputationBand, number> = {
    excellent: 0,
    healthy: 0,
    warning: 0,
    risk: 0,
...
async function getWindowMetrics(
  ...
  const [result] = await db
    .select({
      ...
      totalDurationSec: sql<number>`COALESCE(SUM(duration_sec), 0)`.as('total_duration'),
      shortCalls: sql<number>`COUNT(CASE WHEN is_short_call = true THEN 1 END)`.as('short_calls'),
      immediateHangups: sql<number>`COUNT(CASE WHEN is_immediate_hangup = true THEN 1 END)`.as('hangups'),
      voicemails: sql<number>`COUNT(CASE WHEN is_voicemail = true THEN 1 END)`.as('voicemails'),
      failures: sql<number>`COUNT(CASE WHEN is_failed = true THEN 1 END)`.as('failures'),
    })
...
function scaleScore(value: number, baseline: number, target: number): number {
  ...
  return Math.round(((value - baseline) / (target - baseline)) * 100);
}

/**
 * Scale a value to 0-100 penalty (higher value = higher penalty)
 */
function scalePenalty(value: number, good: number, bad: number): number {
