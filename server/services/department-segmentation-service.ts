 ...
 * Each segment filters contacts where department = <department name>.
 */
export async function seedDepartmentSegments(ownerId: string): Promise<{
  created: string[];
  existing: string[];
  segments: Array<{ id: string; name: string; department: string }>;
}> {
  const created: string[] = [];
  const existing: string[] = [];
  const result: Array<{ id: string; name: string; department: string }> = [];
...
export async function autoClassifyContacts(options?: {
  ...
  // Classify in batches
  const updates: Array<{ id: string; department: string }> = [];
  ...
  // ---- PHASE 2: Role taxonomy fallback ----
  if (remainingIds.length > 0) {
    ...
    for (let i = 0; i < remainingIds.length; i += chunkSize) {
      ...
      const taxonomyMatches = await db
        .select({
          contactId: contactIntelligence.contactId,
          jobFunction: jobRoleTaxonomy.jobFunction,
        })
        .from(contactIntelligence)
        .innerJoin(
          jobRoleTaxonomy,
          eq(contactIntelligence.normalizedRoleId, jobRoleTaxonomy.id)
        )
        .where(inArray(contactIntelligence.contactId, chunk));

      for (const match of taxonomyMatches) {
        const dept = JOB_FUNCTION_TO_DEPARTMENT[match.jobFunction];
        if (dept) {
          await db
            .update(contacts)
            .set({ department: dept })
      ...
      // Count remaining unclassifiable from this chunk
      const classifiedInChunk = taxonomyMatches.filter(
        (m) => JOB_FUNCTION_TO_DEPARTMENT[m.jobFunction]
      ).length;
