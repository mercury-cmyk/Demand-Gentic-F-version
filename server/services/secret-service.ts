...
export type SecretSummary = Omit<SecretStoreRecord, 'encryptedValue'>;
...
export async function listSecrets(options: SecretListOptions = {}): Promise<SecretSummary[]> {
...
export async function getSecretById(
  ...
  const value = decryptJson<unknown>(record.encryptedValue, ensureMasterKey());
...
export async function getSecretSummary(id: string): Promise<SecretSummary | null> {
  const record = await fetchSecretRecord(id);
  if (!record) {
    return null;
  }
