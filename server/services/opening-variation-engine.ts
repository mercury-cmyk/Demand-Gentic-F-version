...
function addMicroVariations(template: string, config: OpeningConfig): string {
  ...
  if (Math.random() < 0.3) {
    ...
    result = result.replace(/^(Hello|Hi|Hey)/i, randomGreeting);
  }

  if (Math.random() < 0.2) {
    // Add filler word (20% chance)
    const fillers = ['uh, ', 'um, ', 'so, '];
    const randomFiller = fillers[Math.floor(Math.random() * fillers.length)];
    result = result.replace(/^(Hello|Hi|Hey),?\s*/i, `$1, ${randomFiller}`);
  }
...
/**
 * Apply disfluency pattern to text
 */
function applyDisfluency(text: string, pattern: DisfluencyPattern): string {
  if (!pattern.useFiller || !pattern.filler) return text;

  switch (pattern.fillerPosition) {
    case 'start':
      return `${pattern.filler}—${text}`;
    case 'middle':
...
function generateSSML(text: string, variation: OpeningVariation): string {
  ...
  if (pauseBeforeMs > 0) {
    ssml += `<break time="${pauseBeforeMs}ms"/>`;
  }

  // Add breath if specified
  if (disfluencyPattern.breathPauseMs > 0) {
    ssml += `<break time="${disfluencyPattern.breathPauseMs}ms"/>`;
  }
