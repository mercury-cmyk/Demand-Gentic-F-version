...
interface ModelVerdict {
  ...
  intentSignals: Array<{ signal: string; strength: "strong" | "moderate" | "weak"; source: string }>;
  engagementIndicators: Array<{ indicator: string; positive: boolean }>;
...
## RESPOND WITH THIS EXACT JSON STRUCTURE:
{
  "verdict": "high_potential" | "likely_potential" | "review" | "not_potential",
  "confidence": <0-100>,
  "intentScore": <0-100 based on genuine buying/engagement intent>,
  "campaignFitScore": <0-100 how well this lead fits THIS campaign's objective>,
  "reasoning": "<2-3 sentence explanation>",
  "signals": ["<key signal 1>", "<key signal 2>", ...],
  "missingDataImpact": "none" | "low" | "medium" | "high",
  "suggestedDisposition": "<what the disposition SHOULD be>",
  "overrideDisposition": <true if current disposition is wrong>,
  "recommendedAction": "engage" | "nurture" | "review" | "skip",
  "intentSignals": [{"signal": "<description>", "strength": "strong"|"moderate"|"weak", "source": "transcript|behavior|metadata"}],
  "engagementIndicators": [{"indicator": "<description>", "positive": true|false}],
...
async function runKimiAnalysis(ctx: CallContext): Promise<ModelVerdict | null> {
  if (!isKimiConfigured()) {
  ...
  try {
    ...
    const result = await kimiGenerateJSON<ModelVerdict>(prompt, {
...
async function runDeepSeekAnalysis(ctx: CallContext): Promise<ModelVerdict | null> {
  ...
  try {
    const prompt = buildPrecisionPrompt(ctx);
    const result = await deepSeekJSON<ModelVerdict>(prompt, {
...
function computeConsensus(
  ...
  overrideDisposition: boolean;
  suggestedDisposition: string;
  intentSignals: Array<{ signal: string; strength: string; source: string }>;
  engagementIndicators: Array<{ indicator: string; positive: boolean }>;
  missingFields: string[];
  dataCompleteness: number;
} {
...
function dedupeSignals(signals: Array<{ signal: string; strength: string; source: string }>) {
...
function dedupeIndicators(indicators: Array<{ indicator: string; positive: boolean }>) {
  const seen = new Set<string>();
...
export async function analyzePrecisionLead(callSessionId: string): Promise<PrecisionAnalysisResult> {
  ...
  if (!row.transcript || row.transcript.length < 50) {
    return { success: false, verdict: "not_potential", consensusConfidence: 0, consensusIntentScore: 0, consensusCampaignFit: 0, recommendedAction: "skip", error: "No meaningful transcript" };
  }

  const contactName = [row.contactFirst, row.contactLast].filter(Boolean).join(" ") || "Unknown Contact";
  const dedupKey = buildDedupKey(row.contactId, row.campaignId, row.toNumber);

  // Check if already analyzed (dedup)
  const [existing] = await db
    .select({ id: precisionLeadAnalyses.id, verdict: precisionLeadAnalyses.verdict })
    .from(precisionLeadAnalyses)
    .where(eq(precisionLeadAnalyses.dedupKey, dedupKey))
    .limit(1);

  if (existing) {
    console.log(`${LOG} Dedup hit for ${dedupKey}, skipping re-analysis`);
    return {
      success: true,
      analysisId: existing.id,
      verdict: existing.verdict,
      consensusConfidence: 0,
      consensusIntentScore: 0,
      consensusCampaignFit: 0,
  ...
  const ctx: CallContext = {
    ...
    contactPhone: row.toNumber,
    companyName: row.companyName,
    campaignName: row.campaignName,
    campaignObjective: row.campaignObjective,
    successCriteria: row.successCriteria,
    talkingPoints: row.talkingPoints,
    qaParameters: row.qaParameters,
    lqaIntentStrength: row.lqaIntentStrength,
    lqaOutcomeCategory: row.lqaOutcomeCategory,
    lqaCampaignFitScore: row.lqaCampaignFitScore ? Number(row.lqaCampaignFitScore) : null,
    lqaShouldCreateLead: row.lqaShouldCreateLead,
    cqrEngagementLevel: row.cqrEngagementLevel,
    cqrQualificationMet: row.cqrQualificationMet,
    cqrDispositionAccurate: row.cqrDispositionAccurate,
  };

  // Run both models in parallel
  console.log(`${LOG} Running dual-model analysis for ${callSessionId}...`);
  const [kimiResult, deepseekResult] = await Promise.all([
    runKimiAnalysis(ctx),
    runDeepSeekAnalysis(ctx),
  ]);

  const consensus = computeConsensus(kimiResult, deepseekResult);
  const durationMs = Date.now() - startMs;

  // Compute priority rank based on consensus
  let priorityRank = 99;
  if (consensus.verdict === "high_potential") priorityRank = 1;
  else if (consensus.verdict === "likely_potential") priorityRank = 2;
  else if (consensus.verdict === "review") priorityRank = 3;
  else priorityRank = 4;
  // Refine within tier by intent score
  if (consensus.intentScore >= 80) priorityRank = Math.max(1, priorityRank - 1);
...
}): Promise<{ processed: number; highPotential: number; likelyPotential: number; review: number; notPotential: number; errors: number; batchId: string }> {
