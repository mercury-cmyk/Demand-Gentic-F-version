...
export async function getActiveVariantForCampaign(
  ...
  accountId?: string,
  agentId?: string
): Promise<typeof promptVariants.$inferSelect | null> {
 ...
 */
export async function recordVariantTest(result: VariantTestResult): Promise<typeof promptVariantTests.$inferSelect> {
  const [test] = await db
...
): Promise<typeof variantSelectionHistory.$inferSelect> {
 ...
 */
export async function compareCampaignVariants(campaignId: string): Promise<VariantComparison[]> {
  const variants = await db
    .select()
    .from(promptVariants)
    .where(eq(promptVariants.campaignId, campaignId));
 ...
 */
export async function compareAccountVariants(accountId: string): Promise<VariantComparison[]> {
  const variants = await db
    .select()
    .from(promptVariants)
    .where(eq(promptVariants.accountId, accountId));
...
export async function updateVariant(
  variantId: string,
  updates: Partial<Omit<PromptVariantInput, "context">> & { context?: Record<string, any> }
) {
  const [updated] = await db
    .update(promptVariants)
    .set({
