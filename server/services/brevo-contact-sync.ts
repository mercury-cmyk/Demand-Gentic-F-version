...
async function brevoFetch(path: string, options: RequestInit = {}): Promise<Response> {
...
export interface BrevoEmailCampaign {
  ...
  statistics?: {
    ...
    listStats?: Record<string, BrevoGlobalStats>;
  };
...
export async function getBrevoListContacts(listId: number, limit = 500, offset = 0): Promise<{ contacts: BrevoContact[]; count: number }> {
  ...
  return res.json() as Promise<{ contacts: BrevoContact[]; count: number }>;
}

/** Get a single Brevo contact by email */
export async function getBrevoContact(email: string): Promise<BrevoContact | null> {
...
export async function upsertBrevoContact(email: string, attributes: Record<string, unknown>, listIds?: number[]): Promise<void> {
...
export async function getBrevoBlocklist(limit = 500, offset = 0): Promise<{ contacts: { email: string; reason: Record<string, unknown> }[]; count: number }> {
...
export async function bulkPushContactsToBrevo(
  ...
  for (let i = 0; i < validContacts.length; i += BATCH_SIZE) {
    ...
    const jsonBody = batch.map(c => {
      const email = (c.emailNormalized || c.email)!;
      const attrs: Record<string, unknown> = { CRM_CONTACT_ID: c.id };
  ...
  while (true) {
    ...
    for (const bc of page.contacts) {
      ...
      try {
        ...
        const attrs = (bc.attributes || {}) as Record<string, string | undefined>;
 ...
 */
export async function syncBrevoBlacklist(): Promise<SyncResult> {
  const result: SyncResult = { created: 0, updated: 0, skipped: 0, errors: [] };
 ...
 */
export async function pushSegmentToBrevo(segmentId: string, brevoListId: number): Promise<SyncResult> {
 ...
 */
export async function pushListToBrevo(listId: string, brevoListId: number): Promise<SyncResult> {
 ...
 */
export async function createBrevoList(name: string, folderId: number = 1): Promise<{ id: number; name: string }> {
...
export async function listBrevoCampaigns(options: { status?: string; limit?: number; offset?: number } = {}): Promise<{ campaigns: BrevoEmailCampaign[]; count: number }> {
  ...
  return res.json() as Promise<{ campaigns: BrevoEmailCampaign[]; count: number }>;
}

/** Get a single Brevo campaign with full stats */
export async function getBrevoCampaign(campaignId: number): Promise<BrevoEmailCampaign> {
  ...
  return res.json() as Promise<BrevoEmailCampaign>;
}
 ...
 */
export async function getBrevoCampaignEngagement(campaignId: number): Promise<BrevoEngagementReport> {
 ...
 */
export async function getAllBrevoEngagementReports(limit = 50): Promise<BrevoEngagementReport[]> {
 ...
 */
export async function getBrevoStatsByDay(days = 30): Promise<Array<{
  date: string;
  delivered: number;
  opens: number;
  clicks: number;
 ...
 */
export async function addToBrevoBlocklist(email: string): Promise<void> {
 ...
 */
export async function removeFromBrevoBlocklist(email: string): Promise<void> {
  const res = await brevoFetch(`/contacts/${encodeURIComponent(email)}`, {
    method: 'PUT',
 ...
 */
export async function getUnifiedBlocklist(limit = 200, offset = 0): Promise<{ entries: BlocklistEntry[]; total: number }> {
  ...
  const entryMap = new Map<string, BlocklistEntry>();
 ...
 */
export async function blockEmailEverywhere(email: string, reason: 'hard_bounce' | 'unsubscribe' | 'spam_complaint' | 'manual' = 'manual'): Promise<void> {
 ...
 */
export async function unblockEmailEverywhere(email: string): Promise<void> {
  const normalized = email.toLowerCase().trim();
...
export async function getCombinedCampaignEngagement(crmCampaignId: string, brevoCampaignId?: number): Promise<{
  ...
  brevo: BrevoEngagementReport | null;
}> {
