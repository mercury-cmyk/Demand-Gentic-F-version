...
export const DEFAULT_PERSONAS: Record<string, SimulatedHumanProfile> = {
  "friendly_dm": {
...
export class SimulationEngine {
  ...
  /**
   * Create a new simulation session - NO TELEPHONY
   */
  async createSession(config: SimulationSessionConfig): Promise<SimulationSession> {
...
Respond ONLY with the human's spoken words. No stage directions or descriptions.`;
   ...
   */
  async runTurn(session: SimulationSession, humanInput?: string): Promise<SimulationTurn> {
   ...
   */
  async runFullSimulation(session: SimulationSession): Promise<SimulationSession> {
    const context = await this.buildAgentContext(session);
   ...
   */
  async evaluateSimulation(session: SimulationSession): Promise<SimulationEvaluation> {
    ...
    if (metrics.qualificationQuestions < 2) {
      recommendations.push("Ask more discovery questions to understand prospect needs");
    }
    if (!metrics.valuePropositionDelivered) {
      recommendations.push("Clearly articulate the value proposition and benefits");
    }
    if (!metrics.callToActionDelivered) {
      recommendations.push("Include a clear call to action or next step");
    }
  ...
  async getSession(sessionId: string): Promise<SimulationSession | null> {
    ...
    const metadata = (dbSession.metadata || {}) as Record<string, unknown>;
