...
const COMMUNITY_INDUSTRY_KEYWORDS: Record<string, string[]> = {
  finance: [
    "financial services",
    "banking",
    "insurance",
...
const DEPARTMENT_COMMUNITY_MAP: Record<string, string> = {
  finance: "finance",
  accounting: "finance",
  treasury: "finance",
...
async function buildCandidateCommunities(params: {
  ...
  Array<{ community: string; source: ProductMatchResult extends { matched: true } ? ProductMatchResult["matchSource"] : string; priority: number }>
> {
  const candidates: Array<{
    community: string;
    source: "department" | "industry" | "intent" | "fallback";
    priority: number;
  }> = [];
...
async function loadCandidateEvents(params: {
  ...
  Array<{
    ...
    ctaLink: string | null;
    learnBullets: string[] | null;
    source: "campaign_link" | "external_event" | "resources_centre";
  }>
> {
  const candidateEvents: Array<any> = [];
...
export function formatProductContextForPrompt(
  match: Extract<ProductMatchResult, { matched: true }>
): string {
