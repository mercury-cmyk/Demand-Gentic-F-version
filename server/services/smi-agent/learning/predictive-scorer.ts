...
function calculateTimingScore(contact: any): number {
  ...
  if (contact.lastCallAttemptAt) {
    ...
    if (daysSinceLastCall < 7) score -= 0.2; // Too recent
  }
...
function buildScoreFactors(
  ...
  return {
    roleFactors: [
      {
        factor: intel?.normalizedRole?.name || 'Unknown role',
        impact: roleScore - 0.5,
        direction: roleScore > 0.5 ? 'positive' : roleScore < 0.5 ? 'negative' : 'neutral',
      },
      {
        factor: `Decision authority: ${intel?.decisionAuthority || 'unknown'}`,
        impact: intel?.decisionAuthority === 'decision_maker' ? 0.3 : 0,
        direction: intel?.decisionAuthority === 'decision_maker' ? 'positive' : 'neutral',
      },
    ],
    industryFactors: [
      {
        factor: account?.industryStandardized || 'Unknown industry',
        impact: industryScore - 0.5,
        direction: industryScore > 0.5 ? 'positive' : 'neutral',
      },
    ],
    problemFactors: [
      {
    ...
    historicalFactors: [
      {
        ...
        direction: historicalPatternScore > 0.5 ? 'positive' : historicalPatternScore < 0.5 ? 'negative' : 'neutral',
      },
    ],
  };
}

function determineApproach(intel: any, patterns: any[]): SmiApproach {
  // Use intel recommendation if available
  if (intel?.bestApproach) return intel.bestApproach;

  // Check patterns for successful approaches
  for (const pattern of patterns) {
    if (pattern.recommendations.approachModifications.length > 0) {
...
async function cacheScore(score: PredictiveScore): Promise<void> {
...
export class PredictiveScorer implements IPredictiveScorer {
  async generatePredictiveScore(request: PredictiveScoreRequest): Promise<PredictiveScore> {
