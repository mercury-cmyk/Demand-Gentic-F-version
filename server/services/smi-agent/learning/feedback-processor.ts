...
async function enrichOutcomeData(
  contactId?: string,
  accountId?: string
): Promise<{ roleId: number | null; industryId: number | null }> {
...
async function updateTitleMappingFromOutcome(
  ...
  const confidenceAdjustment = roleMatchConfidence > 0.7 ? 0.02 : roleMatchConfidence < 0.3 ? -0.02 : 0;
  const newConfidence = Math.max(0.1, Math.min(0.99, currentConfidence + confidenceAdjustment));

  await db
    .update(jobTitleMappings)
    .set({
    ...
    .where(eq(jobTitleMappings.id, mapping[0].id));
}

/**
 * Update contact intelligence based on outcome
 */
async function updateContactIntelligenceFromOutcome(
  input: CallOutcomeLearningInput
): Promise<void> {
...
function isSignificantOutcome(input: CallOutcomeLearningInput): boolean {
  ...
  if (input.engagementSignals.interestLevel < 0.2) return true;
  if (input.roleSignals?.decisionAuthorityConfirmed) return true;

  return false;
}

/**
 * Invalidate predictive scores for a contact
 */
async function invalidatePredictiveScores(
  contactId?: string,
  campaignId?: string
): Promise<void> {
  if (!contactId) return;
 ...
 */
export async function batchProcessLearningRecords(limit: number = 100): Promise<number> {
...
export class FeedbackProcessor {
  async recordOutcome(input: CallOutcomeLearningInput): Promise<void> {
    return recordLearningOutcome(input);
  }

  async processForSMI(input: CallOutcomeLearningInput): Promise<void> {
    return processCallOutcomeForSMI(input);
  }

  async batchProcess(limit?: number): Promise<number> {
