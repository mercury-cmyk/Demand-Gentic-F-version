...
export async function expandRolesForCampaign(
  ...
  const allMatchedTitles = new Map<number, string[]>();
  ...
  for (const match of synonymMatches) {
    if (!processedRoleIds.has(match.role.id) && expandedRoles.length < maxExpansion) {
      processedRoleIds.add(match.role.id);
      expandedRoles.push({
        role: match.role,
        expansionReason: 'synonym',
  ...
  // 3. Find adjacent roles
  if (includeAdjacent) {
    const adjacentMatches = await findAdjacentRoles(
      Array.from(processedRoleIds),
      seniorityFilter
    );
    for (const match of adjacentMatches) {
      if (!processedRoleIds.has(match.role.id) && expandedRoles.length < maxExpansion) {
        processedRoleIds.add(match.role.id);
        expandedRoles.push({
          role: match.role,
          expansionReason: 'adjacent',
          relevanceScore: match.relevanceScore,
          matchedTitles: match.matchedTitles,
        });
        allMatchedTitles.set(match.role.id, match.matchedTitles);
  ...
  // 4. AI-recommended expansions based on learning patterns
  if (industryId && expandedRoles.length < maxExpansion) {
    const aiRecommendations = await getAIRecommendedRoles(
      specifiedRoles,
      industryId,
      processedRoleIds,
      maxExpansion - expandedRoles.length
    );
    for (const match of aiRecommendations) {
      if (!processedRoleIds.has(match.role.id)) {
        processedRoleIds.add(match.role.id);
        expandedRoles.push({
          role: match.role,
          expansionReason: 'ai_recommended',
          relevanceScore: match.relevanceScore,
          matchedTitles: match.matchedTitles,
        });
        allMatchedTitles.set(match.role.id, match.matchedTitles);
  ...
  const filteredRoles = seniorityFilter
    ? expandedRoles.filter(r => seniorityFilter.includes(r.role.seniority))
    : expandedRoles;
 ...
 */
async function findSpecifiedRoles(specifiedRoles: string[]): Promise<Array<{
  role: NormalizedRole;
  matchedTitles: string[];
}>> {
  const results: Array<{ role: NormalizedRole; matchedTitles: string[] }> = [];
...
async function getAIRecommendedRoles(
  ...
  const recommendedRoleIds = new Set<number>();
 ...
 */
async function getTitlesForRole(roleId: number): Promise<string[]> {
...
export class RoleExpansionService {
  async expandRoles(request: RoleExpansionRequest): Promise<RoleExpansionResult> {
    return expandRolesForCampaign(request);
  }

  async getSpecifiedRoles(roleNames: string[]): Promise<NormalizedRole[]> {
