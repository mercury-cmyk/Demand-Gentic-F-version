...
export interface ContactIntelligenceResult {
  ...
  painPointSensitivity: Record<string, number>; // pain point -> sensitivity score
...
export interface CallOutcomeLearningInput {
  ...
  adjustmentsApplied?: Record<string, any>;
  // Metadata
...
export interface MessagingSignals {
  ...
  proofPointResonance: Record<string, 'resonated' | 'neutral' | 'rejected'>;
...
export interface ITitleMappingService {
  mapTitle(rawTitle: string): Promise<TitleMappingResult>;
  mapTitlesBatch(rawTitles: string[]): Promise<TitleMappingResult[]>;
  getAdjacentRoles(roleId: number): Promise<AdjacentRole[]>;
  expandCampaignRolesToTitles(request: RoleExpansionRequest): Promise<RoleExpansionResult>;
  getRoleTaxonomy(filters?: { function?: string; seniority?: string; category?: string }): Promise<NormalizedRole[]>;
}

export interface IIndustryMappingService {
  classifyIndustry(input: { rawIndustry?: string; sicCode?: string; naicsCode?: string }): Promise<IndustryClassificationResult>;
  getIndustryIntelligence(industryId: number): Promise<IndustryIntelligence>;
  getIndustryDepartmentPainPoints(industryId: number, department: string): Promise<IndustryDepartmentPainPoint[]>;
  getIndustryTaxonomy(filters?: { level?: string; parentId?: number }): Promise<IndustryTaxonomy[]>;
}

export interface IPerspectiveEngine {
  generateMultiPerspectiveIntelligence(request: PerspectiveGenerationRequest): Promise<MultiPerspectiveIntelligence>;
  getCachedPerspectiveAnalysis(accountId: string): Promise<PerspectiveAnalysis[]>;
  invalidatePerspectiveCache(accountId: string): Promise<void>;
}

export interface IContactIntelligenceService {
  generateContactIntelligence(contactId: string, campaignId?: string, forceRefresh?: boolean): Promise<ContactIntelligenceResult>;
  getContactIntelligence(contactId: string): Promise<ContactIntelligenceResult | null>;
  invalidateContactIntelligence(contactId: string): Promise<void>;
}

export interface ISolutionMappingService {
  mapSolutionToProblemsAndRoles(request: SolutionMappingRequest): Promise<SolutionMapping>;
  getRecommendedTargets(campaignId: string): Promise<RoleRecommendation[]>;
}

export interface ILearningAggregator {
  recordLearningOutcome(outcome: CallOutcomeLearningInput): Promise<void>;
  aggregateLearnings(request: LearningAggregationRequest): Promise<LearningPattern[]>;
  getLearningInsights(scope: InsightScope, scopeId?: string, type?: InsightType): Promise<LearningPattern[]>;
}

export interface IPredictiveScorer {
  generatePredictiveScore(request: PredictiveScoreRequest): Promise<PredictiveScore>;
  generateCampaignPredictiveScores(request: BatchPredictiveScoreRequest): Promise<PredictiveScore[]>;
  getContactPredictiveScore(contactId: string, campaignId: string): Promise<PredictiveScore | null>;
}

export interface IGovernanceService {
  validateOutput(outputType: string, output: any): Promise<SmiValidationResult>;
  logAudit(entry: SmiAuditEntry): Promise<void>;
  getAuditLog(filters: {
    startDate?: Date;
    endDate?: Date;
    operationType?: string;
