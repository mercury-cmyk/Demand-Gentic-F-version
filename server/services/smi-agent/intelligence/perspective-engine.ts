...
): Promise<MultiPerspectiveIntelligence> {
 ...
 */
export async function invalidatePerspectiveCache(accountId: string): Promise<void> {
  await db
 ...
 */
async function collectAccountSignals(accountData: any): Promise<AccountSignals> {
...
{
  "keyConsiderations": ["<3-5 key things to consider>"],
  "valueDrivers": ["<2-4 value drivers that would resonate>"],
  "potentialConcerns": ["<2-3 potential concerns or objections>"],
  "recommendedApproach": "direct|consultative|educational|peer-based",
  "messagingAngles": ["<2-3 messaging angles>"],
  "questionsToAsk": ["<3-4 discovery questions>"],
  "proofPointsNeeded": ["<2-3 types of proof points needed>"],
  "confidence": <0-1>
}`,
        },
        {
...
export class PerspectiveEngine implements IPerspectiveEngine {
  ...
  async getCachedPerspectiveAnalysis(accountId: string): Promise<PerspectiveAnalysis[]> {
    return getCachedPerspectiveAnalysis(accountId);
  }

  async invalidatePerspectiveCache(accountId: string): Promise<void> {
