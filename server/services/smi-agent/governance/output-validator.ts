...
function validateTitleMapping(output: TitleMappingResult): SmiValidationResult {
  ...
  if (output.confidence !== undefined) {
    ...
    if (output.confidence < 0.5 && output.normalizedRole) {
      warnings.push({ field: 'confidence', message: 'Low confidence mapping may need review', severity: 'medium' });
    }
  }

  // Role validation
  if (output.normalizedRole) {
    if (!output.normalizedRole.id) {
      errors.push({ field: 'normalizedRole.id', message: 'Role ID is required', code: 'REQUIRED_FIELD' });
    }
    if (!output.normalizedRole.name) {
      errors.push({ field: 'normalizedRole.name', message: 'Role name is required', code: 'REQUIRED_FIELD' });
    }
  }

  // Suggestions
  if (output.mappingSource === 'none') {
    suggestions.push('Consider adding this title to the mapping database');
  }
  if (output.alternativeRoles && output.alternativeRoles.length > 0) {
...
function validateIndustryClassification(output: IndustryClassificationResult): SmiValidationResult {
  ...
  if (output.confidence < 0.3) {
    warnings.push({ field: 'confidence', message: 'Very low confidence classification', severity: 'high' });
    suggestions.push('Manual review recommended for this industry classification');
  }

  // SIC/NAICS validation
  if (output.sicCodes.length === 0 && output.naicsCodes.length === 0) {
    warnings.push({ field: 'sicCodes', message: 'No SIC/NAICS codes mapped', severity: 'low' });
  }
...
/**
 * Validate multi-perspective intelligence
 */
function validateMultiPerspective(output: MultiPerspectiveIntelligence): SmiValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationWarning[] = [];
  const suggestions: string[] = [];
  ...
  // Required field validation
  if (!output.accountId) {
    errors.push({ field: 'accountId', message: 'Account ID is required', code: 'REQUIRED_FIELD' });
  }

  if (!output.perspectives || output.perspectives.length === 0) {
    errors.push({ field: 'perspectives', message: 'At least one perspective analysis required', code: 'REQUIRED_FIELD' });
  }

  // Validate each perspective
  for (const perspective of output.perspectives || []) {
    if (!perspective.perspectiveCode) {
      errors.push({ field: 'perspective.code', message: 'Perspective code is required', code: 'REQUIRED_FIELD' });
    }

    if (perspective.confidence < 0.4) {
      warnings.push({
        field: `perspective.${perspective.perspectiveCode}`,
        message: `Low confidence for ${perspective.perspectiveName} analysis`,
        severity: 'medium',
      });
    }

    // Validate analysis content
    const analysis = perspective.analysis;
    if (!analysis.keyConsiderations || analysis.keyConsiderations.length === 0) {
      warnings.push({
        field: `perspective.${perspective.perspectiveCode}.keyConsiderations`,
        message: 'Missing key considerations',
        severity: 'medium',
      });
    }
    ...
    if (!analysis.recommendedApproach) {
      warnings.push({
        field: `perspective.${perspective.perspectiveCode}.recommendedApproach`,
        message: 'Missing recommended approach',
        severity: 'low',
      });
    }
  }

  // Validate synthesized recommendation
  if (!output.synthesizedRecommendation?.primaryAngle) {
    warnings.push({ field: 'synthesizedRecommendation.primaryAngle', message: 'Missing primary angle', severity: 'low' });
  }
...
/**
 * Validate contact intelligence result
 */
function validateContactIntelligence(output: ContactIntelligenceResult): SmiValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationWarning[] = [];
  const suggestions: string[] = [];
  ...
  if (output.normalizedRole && output.roleConfidence < 0.5) {
    warnings.push({ field: 'roleConfidence', message: 'Low role mapping confidence', severity: 'medium' });
    suggestions.push('Verify job title to role mapping');
  }

  // Decision authority validation
  const validAuthorities = ['decision_maker', 'influencer', 'user', 'gatekeeper'];
  if (output.decisionAuthority && !validAuthorities.includes(output.decisionAuthority)) {
    errors.push({ field: 'decisionAuthority', message: 'Invalid decision authority value', code: 'INVALID_VALUE' });
  }

  // Propensity score validation
  if (output.engagementPropensity < 0 || output.engagementPropensity > 1) {
    errors.push({ field: 'engagementPropensity', message: 'Score must be between 0 and 1', code: 'INVALID_RANGE' });
  }
  if (output.qualificationPropensity < 0 || output.qualificationPropensity > 1) {
...
function validatePredictiveScore(output: PredictiveScore): SmiValidationResult {
  ...
  // Priority validation
  if (output.callPriority < 1 || output.callPriority > 100) {
...
function validateSolutionMapping(output: SolutionMapping): SmiValidationResult {
  ...
  if (!output.problemMappings || output.problemMappings.length === 0) {
    ...
    for (const pm of output.problemMappings) {
      ...
      if (pm.solutionFit < 0 || pm.solutionFit > 1) {
  ...
  if (!output.roleRecommendations || output.roleRecommendations.length === 0) {
    ...
    for (const rr of output.roleRecommendations) {
      ...
      if (rr.fitScore < 0 || rr.fitScore > 1) {
...
export class OutputValidator {
  async validate(outputType: string, output: any): Promise<SmiValidationResult> {
