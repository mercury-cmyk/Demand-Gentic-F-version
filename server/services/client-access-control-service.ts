...
export const FEATURE_CATEGORIES: Record<string, { label: string; features: ClientFeatureFlag[] }> = {
  crm_pipeline: {
    label: 'CRM & Pipeline',
...
export let PRESETS: Record<string, { label: string; description: string; features: ClientFeatureFlag[] }> = {
  basic_viewer: {
    label: 'Basic Viewer',
...
export async function checkClientFeature(
  ...
  if (grant.expiresAt && new Date(grant.expiresAt) < new Date()) {
    return { allowed: false, reason: `Feature '${feature}' grant has expired` };
  }

  // Check scope restrictions
  if (grant.scopeType === 'campaign' && context?.campaignId && grant.scopeValue) {
    const allowedCampaigns = (grant.scopeValue as any)?.campaignIds || [];
    if (allowedCampaigns.length > 0 && !allowedCampaigns.includes(context.campaignId)) {
...
export async function getClientAccessSummary(clientAccountId: string) {
  ...
  return {
    ...
    hasExpiringGrants: activeGrants.some(g => g.expiresAt && new Date(g.expiresAt) < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000)),
  };
}

/**
 * Grant a feature to a client
 */
export async function grantFeature(input: GrantFeatureInput) {
  const { clientAccountId, feature, scopeType, scopeValue, config, expiresAt, notes, grantedBy } = input;

  // Upsert: if a grant already exists for this feature, update it
  const [existing] = await db
    .select({ id: clientPermissionGrants.id, isEnabled: clientPermissionGrants.isEnabled })
    .from(clientPermissionGrants)
    .where(
      and(
        eq(clientPermissionGrants.clientAccountId, clientAccountId),
