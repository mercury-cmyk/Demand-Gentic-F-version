...
const activeCostSessions = new Map<string, CostTrackerSession>();
...
export function getActiveCostSummary(): {
  ...
  calls: Array<{ callId: string; durationSeconds: number; currentCost: number }>;
} {
  const calls: Array<{ callId: string; durationSeconds: number; currentCost: number }> = [];
  let totalCost = 0;
