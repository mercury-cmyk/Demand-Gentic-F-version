 ...
 */
async function loadCampaignCriteria(campaignId: string): Promise<CampaignSuccessCriteria> {
  try {
    ...
    if (campaign.qaParams && typeof campaign.qaParams === "object") {
      const qaParams = campaign.qaParams as Record<string, unknown>;
      ...
      if (qaParams.scoring_weights && typeof qaParams.scoring_weights === "object") {
        criteria.scoringWeights = {
          ...criteria.scoringWeights,
          ...(qaParams.scoring_weights as Partial<CampaignSuccessCriteria["scoringWeights"]>),
        };
      }
    ...
    if (campaign.aiAgentSettings && typeof campaign.aiAgentSettings === "object") {
      const aiSettings = campaign.aiAgentSettings as Record<string, unknown>;

      if (aiSettings.qualification && typeof aiSettings.qualification === "object") {
        const qual = aiSettings.qualification as Record<string, unknown>;
...
function identifyBreakdown(
  ...
  if (callDurationSeconds < 15) {
    return {
      phase: "opening",
      breakdownType: "early_hangup",
      description: "Call ended very quickly, likely before engagement",
      atSeconds: callDurationSeconds,
  ...
  if (
    ...
    lowerTranscript.includes("in a meeting") ||
    lowerTranscript.includes("can't transfer")
  ) {
    return {
      phase: "gatekeeper",
      breakdownType: "gatekeeper_block",
      description: "Call was blocked at gatekeeper level",
      atSeconds: Math.min(30, callDurationSeconds),
  ...
  if (!hasAnyPositive && callDurationSeconds < 60) {
    return {
      phase: "pitch",
      breakdownType: "no_engagement",
      description: "Prospect showed no engagement or interest signals",
      atSeconds: callDurationSeconds,
      transcript: transcript.substring(Math.max(0, transcript.length - 300)),
    };
  }
...
/**
 * Calculate comprehensive quality score for a call
 */
function calculateQualityScore(
  ...
  const analysis = analyzeTranscript(transcript, criteria);

  // Call duration score
  let durationScore = 0;
  let durationReason = "";
  if (callDurationSeconds < criteria.minimumCallDurationSeconds) {
    durationScore = 20;
    durationReason = `Call too short (${callDurationSeconds}s < ${criteria.minimumCallDurationSeconds}s minimum)`;
  } else if (
    callDurationSeconds >= criteria.optimalCallDurationRange.min &&
    callDurationSeconds <= criteria.optimalCallDurationRange.max
  ) {
    durationScore = 100;
    durationReason = "Optimal call duration for meaningful conversation";
  } else if (callDurationSeconds > criteria.optimalCallDurationRange.max) {
    durationScore = 70;
  ...
  if (callDurationSeconds < criteria.minimumCallDurationSeconds) {
    issues.push({
      type: "warning",
      message: `Call duration (${callDurationSeconds}s) below minimum (${criteria.minimumCallDurationSeconds}s)`,
    });
  }
  if (analysis.objections.length > 0 && objectionScore < 50) {
    issues.push({
      type: "critical",
      message: `Unaddressed objections: ${analysis.objections.join(", ")}`,
    });
  }
  if (engagementScore < 30) {
    issues.push({
      type: "warning",
      message: "Very low prospect engagement detected",
    });
  }
  if (!hasClosing) {
    issues.push({
      type: "info",
      message: "No clear next steps or closing identified",
    });
  }

  // Generate recommendations
  const recommendations: string[] = [];
  if (objectionScore < 50) {
    recommendations.push("Review objection handling scripts and training");
  }
  if (engagementScore < 50) {
    recommendations.push("Consider adjusting opening pitch or value proposition");
  }
  if (!hasClosing) {
    recommendations.push("Ensure all calls include clear call-to-action");
  }
  if (durationScore < 60) {
    recommendations.push("Work on building rapport to extend meaningful conversations");
  }
  ...
  return {
    ...
    complianceScore,
    breakdown: {
      callDuration: { score: durationScore, reason: durationReason },
      prospectEngagement: { score: engagementScore, reason: engagementReason },
      questionsCovered: { score: questionScore, reason: questionReason },
      objectionHandling: { score: objectionScore, reason: objectionReason },
      closingStrength: { score: closingScore, reason: closingReason },
      dataCapture: { score: dataScore, reason: dataReason },
    },
    qualificationStatus,
...
/**
 * Perform comprehensive analysis on a call
 */
...
function generateInsights(
  ...
  } else if (qualityScore.overallScore < 40) {
    insights.push("This call shows significant room for improvement in engagement.");
  }

  // Engagement insights
  if (analysis.engagementLevel >= 70) {
    insights.push("High prospect engagement suggests good value proposition alignment.");
  } else if (analysis.engagementLevel < 30) {
    insights.push("Low engagement may indicate misaligned targeting or messaging.");
  }

  // Objection insights
  if (analysis.objections.length > 0 && qualityScore.breakdown.objectionHandling.score < 50) {
    insights.push(
      "Objection handling training could improve conversion on similar calls."
    );
  }

  // Breakdown insights
  if (breakdown) {
    switch (breakdown.breakdownType) {
      case "early_hangup":
        insights.push("Consider optimizing opening lines to reduce early hangups.");
        break;
      case "gatekeeper_block":
        insights.push("Gatekeeper navigation techniques may need improvement.");
        break;
      case "objection_unhandled":
        insights.push("Develop specific responses for common objections.");
        break;
      case "no_engagement":
        insights.push("Review targeting criteria to improve prospect fit.");
        break;
    }
  }

  // Closing insights
  if (qualityScore.breakdown.closingStrength.score < 50) {
    insights.push("Strengthen closing techniques to establish clear next steps.");
  }
...
/**
 * Analyze multiple calls for a campaign
 */
export async function analyzeCampaignCalls(
  campaignId: string,
  options: {
    limit?: number;
    startDate?: Date;
