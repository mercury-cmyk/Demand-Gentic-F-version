...
export class TelephonyProviderFactory {
   ...
   */
  static async createAndInitialize(config: TelephonyProviderConfig): Promise<ITelephonyProvider> {
...
export class TelephonyProviderRegistry implements ITelephonyProviderRegistry {
  ...
  registerProvider(provider: ITelephonyProvider): void {
    ...
    if (!this.primaryProviderId || provider.config.priority < (this.getPrimaryProvider()?.config.priority ?? Infinity)) {
      this.primaryProviderId = provider.providerId;
      console.log(`[TelephonyProviderRegistry] Primary provider set to: ${provider.providerId}`);
    }
  }
  ...
  getProviderByType(type: TelephonyProviderConfig['type']): ITelephonyProvider | undefined {
    for (const provider of this.providers.values()) {
      if (provider.providerType === type && provider.isReady()) {
        return provider;
      }
    }
    return undefined;
  ...
  getPrimaryProvider(): ITelephonyProvider | undefined {
    if (!this.primaryProviderId) return undefined;
    return this.providers.get(this.primaryProviderId);
  }
  
  /**
   * Select the best provider for a call based on various criteria
   */
  selectProvider(destination: string, options?: ProviderSelectionOptions): ITelephonyProvider | undefined {
    ...
    if (options?.preferredProviderId) {
      ...
      if (preferred && preferred.isReady() && this.isProviderAllowed(preferred, destination, options)) {
        return preferred;
      }
    }
    
    // Get all eligible providers sorted by priority
    const eligibleProviders = this.getEligibleProviders(destination, options);
    
    if (eligibleProviders.length === 0) {
      console.warn(`[TelephonyProviderRegistry] No eligible providers for destination: ${destination}`);
      return undefined;
    }
  ...
  removeProvider(providerId: string): boolean {
    ...
    if (removed && this.primaryProviderId === providerId) {
      ...
      for (const provider of this.providers.values()) {
        if (!newPrimary || provider.config.priority < newPrimary.config.priority) {
          newPrimary = provider;
        }
      }
      this.primaryProviderId = newPrimary?.providerId ?? null;
    }
  ...
  /**
   * Set the primary provider explicitly
   */
  setPrimaryProvider(providerId: string): boolean {
   ...
   */
  async getProviderWithFailover(destination: string, options?: ProviderSelectionOptions): Promise<ITelephonyProvider | undefined> {
    const eligibleProviders = this.getEligibleProviders(destination, options);
    eligibleProviders.sort((a, b) => a.config.priority - b.config.priority);
 ...
 */
export async function initializeTelephonyProviders(configs: TelephonyProviderConfig[]): Promise<TelephonyProviderRegistry> {
