...
const activeSessions = new Map<string, CampaignCreationSession>();
...
export function updateSession(sessionId: string, updates: Partial<CampaignCreationSession>): CampaignCreationSession | undefined {
...
Response Schema:
{
  ...
  try {
    ...
    const generatedContext: Partial<StructuredCampaignContext> = {
      version: '1.0',
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      status: 'draft',
...
export function validateCampaignContext(
  context: Partial<StructuredCampaignContext>
): CampaignValidationRequirements {
...
export async function getConversationalGuidance(
  ...
  updatedContext: Partial<StructuredCampaignContext>;
  nextQuestion?: string;
}> {
  const model = genAI.getGenerativeModel({ 
    model: 'gemini-2.0-flash',
