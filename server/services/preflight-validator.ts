...
export type PreflightData = z.infer<typeof PreflightSchema>;
 ...
 */
export function validatePreflight(data: Record<string, any>): PreflightValidationResult {
  try {
 ...
 */
function flattenErrors(errors: z.ZodError["errors"]): Record<string, string> {
 ...
 */
export function generatePreflightReport(data: Record<string, any>): string {
  const lines = [
