...
export class MercuryEmailService {
   ...
   */
  async sendDirect(request: MercurySendRequest): Promise<MercurySendResult> {
  ...
  async queueEmail(params: {
    ...
    scheduledAt?: Date;
  }): Promise<{ outboxId: string; skipped: boolean }> {
  ...
  async processOutbox(batchSize: number = MERCURY_DEFAULTS.batchSize): Promise<{
    ...
    for (const entry of queued) {
      ...
      if (entry.scheduledAt && new Date(entry.scheduledAt) > new Date()) {
        continue;
      }
      ...
      if (queued.indexOf(entry) < queued.length - 1) {
   ...
   */
  async renderTemplate(templateKey: string, variables: Record<string, string>, includeFooter = true): Promise<TemplateRenderResult | null> {
    ...
    const varDefs = (template.variables || []) as Array<{ name: string; required: boolean; defaultValue?: string; exampleValue?: string }>;
   ...
   */
  async previewTemplate(templateKey: string, sampleVarsOverride?: Record<string, string>): Promise<{
   ...
   */
  async getDefaultProvider(): Promise<SmtpProvider | null> {
   ...
   */
  async retryOutboxEntry(outboxId: string): Promise<MercurySendResult> {
