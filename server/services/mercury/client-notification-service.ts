 ...
 */
async function generateEmailTemplate(params: GenerateTemplateParams): Promise<GeneratedTemplate> {
...
<body style="margin:0; padding:0; background-color:#f0f2f5; -webkit-font-smoothing:antialiased;">
  <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#f0f2f5;">
    <tr><td align="center" style="padding: 32px 16px;">
      <table role="presentation" class="main-table" width="600" border="0" cellspacing="0" cellpadding="0" style="background-color:#ffffff; border-radius:16px; overflow:hidden;">
        <tr>
          <td class="mobile-header" style="background-color:#1e293b; padding: 44px 40px; text-align:center;">
            <h1 style="color:#ffffff; margin:0; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:24px; font-weight:700;">${typeLabel}</h1>
            <p style="color:#94a3b8; margin:8px 0 0; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:14px;">Pivotal B2B &middot; DemandGentic.ai</p>
          </td>
        </tr>
        <tr>
          <td class="mobile-padding" style="padding: 40px;">
            <h2 style="margin:0 0 12px; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:20px; color:#0f172a; font-weight:600;">Hi ${params.clientName},</h2>
            <p style="margin:0 0 24px; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:15px; line-height:1.7; color:#475569;">
              We have an update for you${params.campaignName ? ` regarding <strong style="color:#0f172a;">${params.campaignName}</strong>` : ''}.
            </p>
            ${params.customPrompt ? `<p style="margin:0 0 24px; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:15px; line-height:1.7; color:#475569;">${params.customPrompt}</p>` : ''}
            <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
              <tr><td align="center" style="padding: 8px 0 32px;">
                <a href="#" style="font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:16px; font-weight:600; color:#ffffff; text-decoration:none; padding:14px 40px; border-radius:10px; display:inline-block; background-color:#2563eb;">
                  View in Portal
                </a>
              </td></tr>
            </table>
        ...
        <tr>
          <td style="padding: 20px 40px; border-top: 1px solid #e5e7eb; text-align:center;">
            <p style="margin:0; font-family:'Segoe UI',Helvetica,Arial,sans-serif; font-size:12px; color:#6b7280;">
...
export class ClientNotificationService {
  ...
  async listNotifications(params: {
    ...
    limit?: number;
    offset?: number;
  }): Promise<{ notifications: ClientNotification[]; total: number }> {
   ...
   */
  async getNotification(id: string): Promise<ClientNotification | null> {
   ...
   */
  async resolveClientRecipients(clientAccountId: string): Promise<Array<{ email: string; name: string }>> {
