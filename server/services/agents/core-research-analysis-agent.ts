...
export class CoreResearchAnalysisAgent extends BaseAgent {
  ...
  private scoringConfigs: Map<string, ScoringModelConfiguration> = new Map();
  ...
  async analyzeLeadQuality(
    ...
    try {
      ...
      if (!leadData) {
        return this.buildErrorResult<LeadQualityResult>(
          'lead_quality',
      ...
      const rawResult = parseJSONResponse<any>(content);
  ...
  async analyzeEmailQuality(
    ...
    try {
      ...
      const content = response.choices[0]?.message?.content;
      if (!content) {
        return this.buildErrorResult<EmailQualityResult>(
          'email_quality',
          'AI returned empty response',
          startTime
        );
  ...
  private normalizeEmailQualityResult(
    ...
    return {
      ...
      spamTriggers: ensureArray<string>(raw.spamTriggers),
      scores: {
        overall: overallScore,
        tier: determineScoreTier(overallScore),
        components: [
  ...
  async analyzeCallQuality(
    ...
    try {
      ...
      if (!callData) {
        return this.buildErrorResult<CallQualityResult>(
          'call_quality',
  ...
  async analyzeEngagement(
    ...
    try {
      ...
      if (!contactData) {
        return this.buildErrorResult<EngagementAnalysisResult>(
          'engagement',
  ...
  private normalizeEngagementResult(
    ...
    return {
      ...
      intentScore,
      intentSignals: ensureArray<string>(raw.intentSignals),
      momentumScore,
  ...
  async scoreAccountHealth(
    ...
    try {
      ...
      if (!accountData) {
        return this.buildErrorResult<AccountHealthScore>(
          'account_health',
  ...
  private normalizeAccountHealthResult(
    ...
    return {
      ...
      riskFactors: ensureArray<string>(raw.riskFactors),
  ...
  ): Promise<NextBestActionRecommendations> {
    const startTime = Date.now();
    const model = process.env.RESEARCH_ANALYSIS_MODEL || 'deepseek-chat';
  ...
  private normalizeNBAResult(
    ...
    const actions: NextBestAction[] = ensureArray<any>(raw.actions).map((action: any, index: number) => ({
      ...
      contributingFactors: ensureArray<string>(action.contributingFactors),
    }));
  ...
  async analyzeCommunicationQuality(
  ...
  ): Promise<CommunicationQualityResult> {
    const startTime = Date.now();
    const model = process.env.RESEARCH_ANALYSIS_MODEL || 'deepseek-chat';
    ...
    try {
      ...
      if (!contactData) {
        return this.buildErrorResult<CommunicationQualityResult>(
  ...
  private normalizeCommunicationResult(
    ...
    return {
      ...
      issuesIdentified: ensureArray<string>(raw.issuesIdentified),
      riskAreas: ensureArray<string>(raw.riskAreas),
      scores: {
  ...
  private buildErrorResult<T extends AnalysisOutput>(
