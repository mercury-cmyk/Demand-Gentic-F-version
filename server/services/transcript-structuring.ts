...
export function mergeTranscriptTurns(turns: StructuredTranscriptTurn[]): StructuredTranscriptTurn[] {
  ...
  const merged: StructuredTranscriptTurn[] = [];

  for (const turn of turns) {
    const text = normalizeTurnText(turn.text);
    if (!text) continue;

    const normalizedTurn: StructuredTranscriptTurn = {
      ...turn,
      text,
    };

    const previous = merged[merged.length - 1];
    if (!previous || previous.role !== normalizedTurn.role) {
      merged.push(normalizedTurn);
      continue;
    }

    const similarity = computeJaccardSimilarity(previous.text, normalizedTurn.text);
    if (similarity >= 0.8) {
