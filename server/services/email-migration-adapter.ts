...
}): Promise<EmailGenerationResponse | null> {
  if (!USE_UNIFIED_ROUTER) {
    return null;
  }
...
function parseHtmlToClientFormat(html: string, subject: string): {
  ...
  // Extract hero title (usually h1 or large heading)
  const heroTitleMatch = html.match(/<h1[^>]*>([^<]+)<\/h1>/i);
  const heroTitle = heroTitleMatch ? heroTitleMatch[1].trim() : '';

  // Extract hero subtitle (usually h2 or smaller heading after h1)
  const heroSubtitleMatch = html.match(/<h2[^>]*>([^<]+)<\/h2>/i);
  ...
  const ctaMatch = html.match(/<a[^>]*class="[^"]*button[^"]*"[^>]*>([^<]+)<\/a>/i) ||
  ...
  const closingMatches = html.match(/<p[^>]*>([^<]{20,})<\/p>/gi);
