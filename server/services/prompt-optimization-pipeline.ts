...
type PromptContract = {
  ...
  constraints: string[];
  tool_policy: Record<string, boolean> | string[] | null;
  final_system_prompt: string;
  final_first_message: string | null;
  version: string;
  approved_by: string | null;
  audit_log: Array<{ timestamp: string; action: string; details?: string }>;
};
...
const DEFAULT_AGENT_TYPE_DEFAULTS: Record<PromptAgentType, string[]> = {
  voice: [
...
const REDACTION_PATTERNS: Array<{ label: string; regex: RegExp }> = [
...
function formatToolPolicy(toolPolicy: Record<string, boolean> | string[] | null): string[] {
...
function composeFinalSystemPrompt(input: {
  immutableContext: Awaited<ReturnType<typeof resolveImmutableContext>>;
  userPrompt: string;
  agentType: PromptAgentType;
  toolPolicy: Record<string, boolean> | string[] | null;
}): string {
...
function parseReviewOutput(raw: string): z.infer<typeof REVIEW_OUTPUT_SCHEMA> | null {
  if (!raw) return null;
...
function mergeRiskAssessments(
  lintFindings: LintFinding[],
  llmRisk: RiskAssessment | null,
  ...
  const originalLower = originalUserPrompt.toLowerCase();
  const optimizedLower = optimizedUserPrompt.toLowerCase();
...
export async function optimizePromptPackage(
  ...
  let reviewOutput: z.infer<typeof REVIEW_OUTPUT_SCHEMA> | null = null;
