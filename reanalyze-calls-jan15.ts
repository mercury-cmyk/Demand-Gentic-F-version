...
async function ensureTranscript(attempt: any): Promise<string | null> {
...
async function classifyWithGemini(transcript: string, context: { disposition?: string; campaignName?: string; contactName?: string; duration?: number; }): Promise<GeminiResult> {
...
Return strict JSON:
{
  ...
  return await generateJSON<GeminiResult>(prompt, { temperature: 0.2 });
}
