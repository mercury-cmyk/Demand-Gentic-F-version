...
function analyzeTranscript(transcript: string): {
  ...
  // Check for low unique content (repetitive/stuck)
    ...
    rejectionReason = 'Repetitive content - likely voicemail or stuck loop';
  }
  // Check for actual intent signals
  else if (highIntentMatches.length === 0 && consideringMatches.length === 0) {
    rejectionReason = 'No intent signals detected';
  }
  // Check minimum quality
  else if (qualificationScore < 30) {
    rejectionReason = 'Qualification score too low';
  }
  // Check minimum conversation length with good variety
  else if (wordCount < 50 || (wordCount < 100 && uniqueRatio < 0.4)) {
    rejectionReason = 'Conversation too short or low quality';
  }
  // Passed all checks
  else {
    isQualified = true;
