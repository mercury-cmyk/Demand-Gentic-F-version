...
async function findActualHumanResponses() {
    ...
    if (matches.length > 0) {
      ...
      if (isVoicemail) {
        ...
        if (vmMatch) {
          ...
          if (humanIndex < vmIndex && humanIndex < 50) {
            humanResponses.push(call);
          } else {
            // Voicemail with coincidental matching text
            uncertain.push(call);
          }
        } else {
          humanResponses.push(call);
        ...
        // No voicemail indicators, definitely human
        humanResponses.push(call);
      }
    } else if (isVoicemail) {
  ...
  console.log(`🎯 CONFIRMED HUMAN RESPONSES: ${humanResponses.length}`);
  console.log(`📫 Voicemails: ${voicemails.length}`);
  console.log(`❓ Uncertain: ${uncertain.length}\n`);

  // Show ALL human responses
  console.log('========================================');
  console.log('🎯 ALL CONFIRMED HUMAN RESPONSES');
  console.log('========================================\n');

  if (humanResponses.length === 0) {
    console.log('❌ NO confirmed human responses found in these calls.');
    console.log('\nThis means either:');
    console.log('  1. All calls went to voicemail');
    console.log('  2. People answered but said nothing recognizable');
    console.log('  3. Transcription missed the human speech\n');
  } else {
    humanResponses.forEach((call, i) => {
