...
async function checkAllCallsStatus() {
  ...
  const leadsResult = await pool.query(`
    SELECT 
      ...
      COUNT(CASE WHEN (transcript IS NULL OR LENGTH(transcript) < 50) AND recording_url IS NOT NULL THEN 1 END) AS needs_transcription,
      COUNT(CASE WHEN (transcript IS NULL OR LENGTH(transcript) < 50) AND recording_url IS NULL THEN 1 END) AS no_source
    FROM leads
  ...
  const sampleSessions = await pool.query(`
    ...
    LIMIT 10
  `);
  console.log(JSON.stringify(sampleSessions.rows, null, 2));
  console.log("");

  console.log("========================================");
  console.log("SAMPLE: leads needing transcription");
  console.log("========================================");
  const sampleLeads = await pool.query(`
    ...
    LIMIT 10
  `);
  console.log(JSON.stringify(sampleLeads.rows, null, 2));
  console.log("");

  process.exit(0);
}

checkAllCallsStatus().catch((e) => {
