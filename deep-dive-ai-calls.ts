  ...
  const byRecDuration = await pool.query(`
    SELECT 
      CASE 
        ...
        WHEN recording_duration_sec < 15 THEN '1-15s'
        WHEN recording_duration_sec < 30 THEN '15-30s'
        WHEN recording_duration_sec < 60 THEN '30-60s'
        WHEN recording_duration_sec < 120 THEN '60-120s'
        ELSE '120s+'
      END as rec_bucket,
      ai_disposition,
      COUNT(*) as count
