...
### The Problem:
1. **Blocking AMD Detection**: The `` block was configured to run BEFORE the `` connection
...
- [server/routes/texml.ts](server/routes/texml.ts)
  - `/ai-call` endpoint: Removed blocking `` block
  - `/incoming` endpoint: Removed blocking `` block
