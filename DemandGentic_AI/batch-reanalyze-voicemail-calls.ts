 ...
 */
async function checkForExistingLead(callAttemptId: string): Promise {
...
async function analyzeCallWithDeepSeek(
  ...
  if (!transcription || transcription.trim().length === 0) {
    return {
      isVoicemail: callDurationSeconds !== null && callDurationSeconds  lowerTranscript.includes(kw));
...
function generateReport() {
  ...
  const confidenceRanges = {
    '0.0-0.3': ANALYSIS_RESULTS.filter(r => r.analysisResult.confidence  r.analysisResult.confidence > 0.3 && r.analysisResult.confidence  r.analysisResult.confidence > 0.6 && r.analysisResult.confidence  r.analysisResult.confidence > 0.9).length
  };
...
async function main() {
  try {
    ...
    for (let i = 1; i  setTimeout(resolve, 2000));
      }
    }
