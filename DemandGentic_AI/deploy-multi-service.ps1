...
if ([string]::IsNullOrWhiteSpace($PROJECT_ID)) {
  throw "No GCP project configured. Set GCP_PROJECT_ID or run 'gcloud config set project '."
}
...
$GCS_BUCKET = if ([string]::IsNullOrWhiteSpace($env:GCS_BUCKET)) { "demandgentic-prod-storage-2026" } else { $env:GCS_BUCKET }
