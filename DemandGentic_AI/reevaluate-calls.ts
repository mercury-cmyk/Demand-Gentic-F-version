...
async function evaluateTranscript(transcript: string, contactName: string, company: string): Promise {
...
async function reevaluateCalls() {
  ...
  for (let i = 0; i  ${evaluation.disposition} (confidence: ${evaluation.confidence.toFixed(2)}) - ${evaluation.reason.substring(0, 60)}...`);
