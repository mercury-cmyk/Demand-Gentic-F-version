...
function parseArgs(argv: string[]): CliOptions {
  const getNum = (flag: string, fallback: number): number => {
    ...
    if (idx  0 ? Math.floor(value) : fallback;
  };
...
async function callQualityRecordExists(callSessionId: string): Promise {
  const result = await pool.query(
...
async function insertQualityRecord(row: Candidate, analysis: ConversationQualityAnalysis): Promise {
  const result = await pool.query(
    `
...
async function processOne(row: Candidate, options: CliOptions): Promise {
  try {
