...
function linearToUlawSample(sample: number): number {
  ...
  if (sample >= 256) {
    let temp = sample;
    exponent = 1;
    while (temp >= 512 && exponent >= 1; exponent++; }
  ...
  return ((sign | (exponent > 4) & 0x07;
    const man = b & 0x0f;
    ULAW_TO_LINEAR[i] = sign * (((man > 4) & 0x07;
    const man = b & 0x0f;
    ALAW_TO_LINEAR[i] = sign * (exp === 0 ? (man ();
...
function handleIncomingRtp(session: BridgeSession, buf: Buffer): void {
  ...
  if (session.openingProtectionUntil > 0 && Date.now() >> 0, 4);
...
function detectIdentityConfirmation(text: string, expectedName?: string): boolean {
  ...
  if (!normalized || normalized.length ", "my name is ", " speaking"
  if (expectedName) {
...
function maybeInjectStateReinforcement(session: BridgeSession): void {
  ...
  if (session.contactTurnsSinceReinforcement  {
  if (!GCP_PROJECT) {
      ...
      try {
        ...
        if (serverContent?.outputTranscription?.text) {
          ...
          if (agentText) {
            ...
            if (last && last.role === 'agent' && (Date.now() - last.ts)  {
...
async function connectToOpenAI(session: BridgeSession): Promise {
    ...
    ws.on('message', (data: Buffer) => {
      try {
        ...
        if (type === 'response.audio.delta' && msg.delta) {
          ...
          const PACKET_SIZE = 160;
          for (let i = 0; i  0) session.rtpSendQueue.push(chunk);
          }
          startRtpPacing(session);
        ...
        if (type === 'response.audio_transcript.delta' && msg.delta) {
          const last = session.transcript[session.transcript.length - 1];
          if (last && last.role === 'agent' && (Date.now() - last.ts)  0) log(`OpenAI VAD interrupt — flushed ${flushed} queued packets for ${session.callId}`);
          return;
        }
...
async function handleToolCall(session: BridgeSession, call: any): Promise {
  ...
  if (call.name === 'submit_disposition') {
    ...
    if (disposition === 'voicemail' && callDurationSeconds  destroySession(session.callId), 2000);
  }
...
async function callbackToCloudRun(callId: string, action: string, data: any): Promise {
  if (!CLOUD_RUN_API) {
