...
async function fetchRecordingUrl(callControlId: string): Promise {
  try {
...
async function main() {
  ...
  const breakdown = await db.select({
    disposition: dialerCallAttempts.disposition,
    count: sql`count(*)::int`,
    avgDur: sql`coalesce(avg(call_duration_seconds),0)::int`,
    connected: sql`sum(case when connected then 1 else 0 end)::int`,
    hasTranscript: sql`sum(case when full_transcript is not null and full_transcript != '' then 1 else 0 end)::int`,
    hasRecording: sql`sum(case when recording_url is not null then 1 else 0 end)::int`,
  })
  .from(dialerCallAttempts)
