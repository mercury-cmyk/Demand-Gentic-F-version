...
export interface UnifiedWebRTCActions {
  ...
  disconnect: () => void;
  // Calling
  makeCall: (destination: string, options?: Partial) => Promise;
  answerCall: () => Promise;
  hangup: () => void;
  // Controls
  toggleMute: () => void;
  ...
  // Device management
  setAudioInput: (deviceId: string) => Promise;
...
export function useUnifiedWebRTC(config: UnifiedWebRTCConfig): [UnifiedWebRTCState, UnifiedWebRTCActions] {
  ...
  const [bridgeMode, setBridgeMode] = useState('human');
  ...
  // Make outbound call
  const makeCall = useCallback(async (destination: string, options?: Partial) => {
