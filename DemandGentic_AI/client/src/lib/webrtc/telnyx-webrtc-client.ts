...
export class TelnyxWebRTCClient {
  ...
  async connect(): Promise {
    try {
      ...
      // NOTE: The Telnyx SDK handles signaling internally
      // Do NOT set host/port/wss - the SDK connects to the correct endpoints automatically
      ...
      const clientOptions: any = {
        ...
        // Custom ICE servers if provided (for TURN/STUN in restrictive networks)
   ...
   */
  async call(options: TelnyxOutboundCallOptions): Promise {
   ...
   */
  async replaceLocalAudioTrack(newTrack: MediaStreamTrack): Promise {
    if (!this.currentCall) {
   ...
   */
  async restoreLocalMicrophoneTrack(): Promise {
    if (!this.localStream) {
   ...
   * Update audio device
   */
  async setAudioInputDevice(deviceId: string): Promise {
    if (this.client) {
   ...
   */
  async setAudioOutputDevice(deviceId: string): Promise {
    if (this.client) {
