...
export class OpenAIRealtimeWebRTCClient {
   ...
   */
  async connect(inputAudioTrack?: MediaStreamTrack): Promise {
   ...
   */
  private async fetchEphemeralToken(): Promise {
   ...
   */
  private async exchangeSDP(token: string, offerSdp: string): Promise {
