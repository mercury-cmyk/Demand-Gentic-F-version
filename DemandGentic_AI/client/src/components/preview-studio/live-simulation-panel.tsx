...
async function parseJsonResponse(response: Response, context: string): Promise {
...
async function parseErrorMessage(response: Response, context: string): Promise {
  const contentType = response.headers.get('content-type') || '';
  if (contentType.includes('application/json')) {
  ...
  const text = await response.text();
  const preview = text.replace(/\s+/g, ' ').slice(0, 200);
  const hint = preview.startsWith('('voice');
  ...
  const [eagerness, setEagerness] = useState("medium");
  ...
  const [textSimStatus, setTextSimStatus] = useState('idle');
  const [selectedPersona, setSelectedPersona] = useState('neutral_dm');
  const [textSimResult, setTextSimResult] = useState(null);
  ...
  const voiceSimStatusRef = useRef('idle');
  const [voiceSimTranscripts, setVoiceSimTranscripts] = useState([]);
  const [isMuted, setIsMuted] = useState(false);
  const voiceSimWsRef = useRef(null);
  const voiceSimDurationRef = useRef(0);
  const [voiceSimDuration, setVoiceSimDuration] = useState(0);
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const timerRef = useRef(null);
  const voiceSimTimerRef = useRef(null);
  ...
  const { data: voicesData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return parseJsonResponse(res, 'Voice list');
    },
  ...
  useEffect(() => {
    const autoLoadPrompt = async () => {
      ...
      try {
        ...
        const data = await parseJsonResponse(response, 'Auto-load prompt');
  ...
  const handleLoadPrompt = async () => {
    ...
    try {
      ...
      const response = await apiRequest('GET', `/api/preview-studio/assembled-prompt?${params.toString()}`);
      const data = await parseJsonResponse(response, 'Load prompt');
  ...
  const phoneTestMutation = useMutation({
    mutationFn: async () => {
      ...
      return parseJsonResponse(response, 'Start phone test');
    },
    onMutate: () => {
  ...
  const textSimulationMutation = useMutation({
    mutationFn: async () => {
      ...
      return parseJsonResponse(res, 'Simulation');
    },
    onMutate: () => {
  ...
  const handleStartVoiceSimulation = async () => {
    try {
      ...
      const data = await parseJsonResponse(res, 'Voice simulation');
  ...
  const hangupMutation = useMutation({
    mutationFn: async (sessionId: string) => {
      ...
      return parseJsonResponse(response, 'End call');
    },
    onSuccess: () => {
  ...
  return (
      ...
      {/* Mode Tabs */}
       setMode(v as SimulationMode)} className="w-full">
          ...
          {transcripts.length > 0 && (
              ...
              {/* Metrics Card */}
                    ...
                    Call Metrics
                      ...
                      Messages
                      {transcripts.filter(t => t.role !== 'system').length}
