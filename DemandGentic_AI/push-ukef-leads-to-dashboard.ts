...
async function pushUKEFLeadsToDashboard() {
  ...
  for (const campaign of ukefCampaigns) {
    ...
    const [counts] = await db.select({
      total: sql`count(*)::int`,
      new: sql`COUNT(CASE WHEN ${leads.qaStatus} = 'new' THEN 1 END)::int`,
      underReview: sql`COUNT(CASE WHEN ${leads.qaStatus} = 'under_review' THEN 1 END)::int`,
      approved: sql`COUNT(CASE WHEN ${leads.qaStatus} = 'approved' THEN 1 END)::int`,
      published: sql`COUNT(CASE WHEN ${leads.qaStatus} = 'published' THEN 1 END)::int`,
      publishedNotPushed: sql`COUNT(CASE WHEN ${leads.qaStatus} = 'published' AND ${leads.submittedToClient} = false THEN 1 END)::int`,
      alreadyPushed: sql`COUNT(CASE WHEN ${leads.submittedToClient} = true THEN 1 END)::int`,
      rejected: sql`COUNT(CASE WHEN ${leads.qaStatus} = 'rejected' THEN 1 END)::int`,
    })
    .from(leads)
    .where(and(
      eq(leads.campaignId, campaign.id),
    ...
    const [totalCounts] = await db.select({
      total: sql`count(*)::int`,
      pre2026: sql`COUNT(CASE WHEN ${leads.createdAt} `COUNT(CASE WHEN ${leads.createdAt} >= ${CUTOFF_DATE} THEN 1 END)::int`,
    })
    .from(leads)
    .where(and(
      eq(leads.campaignId, campaign.id),
    ...
    const [finalCounts] = await db.select({
      total: sql`count(*)::int`,
      onDashboard: sql`COUNT(CASE WHEN ${leads.submittedToClient} = true THEN 1 END)::int`,
      onDashboard2026: sql`COUNT(CASE WHEN ${leads.submittedToClient} = true AND ${leads.createdAt} >= ${CUTOFF_DATE} THEN 1 END)::int`,
    })
    .from(leads)
    .where(and(
      eq(leads.campaignId, campaign.id),
