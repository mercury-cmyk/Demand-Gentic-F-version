 ...
 */
async function generateOrderNumber(): Promise {
    ...
    const statsResults = await Promise.allSettled([
      db
        .select({
          ...
          total: sql`COUNT(DISTINCT CASE WHEN ${campaignQueue.status} <> 'removed' THEN COALESCE(${campaignQueue.contactId}::text, ${campaignQueue.dialedNumber}, ${campaignQueue.id}::text) ELSE NULL END)::int`,
          pending: sql`COUNT(DISTINCT CASE WHEN ${campaignQueue.status} IN ('queued', 'pending') THEN COALESCE(${campaignQueue.contactId}::text, ${campaignQueue.dialedNumber}, ${campaignQueue.id}::text) ELSE NULL END)::int`,
        })
        .from(campaignQueue)
      ...
      db
        .select({
          campaignId: dialerCallAttempts.campaignId,
          callsMade: sql`count(*)::int`,
          connected: sql`sum(case when ${dialerCallAttempts.connected} = true then 1 else 0 end)::int`,
          qualified: sql`sum(case when ${dialerCallAttempts.disposition} = 'qualified_lead' then 1 else 0 end)::int`,
          voicemail: sql`sum(case when ${dialerCallAttempts.disposition} = 'voicemail' then 1 else 0 end)::int`,
          noAnswer: sql`sum(case when ${dialerCallAttempts.disposition} = 'no_answer' then 1 else 0 end)::int`,
          invalid: sql`sum(case when ${dialerCallAttempts.disposition} = 'invalid_data' then 1 else 0 end)::int`,
        })
        .from(dialerCallAttempts)
        .where(inArray(dialerCallAttempts.campaignId, campaignIdList))
        .groupBy(dialerCallAttempts.campaignId),
...
router.post('/quick-create', campaignUpload.array('files', 10), async (req: Request, res: Response) => {
  try {
    ...
    if (data.argyleFlow && (!data.targetLeadCount || data.targetLeadCount  | null;
    ...
    if (linkedDraft) {
      const existingDraftFields = (linkedDraft.draftFields || {}) as Record;
      await db
        .update(workOrderDrafts)
        .set({
          status: 'submitted',
...
async function verifyCampaignAccess(campaignId: string, clientAccountId: string): Promise {
  const [campaign] = await db
    .select({ id: campaigns.id, name: campaigns.name, clientAccountId: campaigns.clientAccountId })
    .from(campaigns)
    .where(eq(campaigns.id, campaignId))
  ...
  try {
    ...
    const [
      ...
      pool.query(`
        ...
        WHERE cq.campaign_id = $1
          AND cq.status = 'queued'
          AND (cq.next_attempt_at IS NULL OR cq.next_attempt_at >'ai_priority_score', '')), '')::int DESC NULLS LAST,
          cq.created_at ASC
        LIMIT 15
      `, [campaignId]),
...
router.get('/:id/promo-submissions', async (req: Request, res: Response) => {
  try {
    ...
    const mapped = submissions.map((s) => {
      const formData = s.formData as Record | null;
