...
router.post("/batch-start", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    const phoneToItemMap = new Map>(); // phone -> Set of queueItemIds
    ...
    for (const item of eligibleItems) {
      try {
        ...
        try {
          ...
          if (callsInitiated  setTimeout(resolve, delayBetweenCalls));
          }
  ...
  try {
    ...
    const stats = {
      ...
      handoffs: aiCallSessions.filter((s) => {
        const analysis = s.aiAnalysis as Record | null;
...
const activeContinuousSessions = new Map();
