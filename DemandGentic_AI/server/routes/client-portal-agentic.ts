...
function buildClientPortalBodyHtml(args: { copy: GeneratedEmailContent }) {
  ...
  return `
    ...
    ${heroSubtitle ? `${escapeHtml(heroSubtitle)}` : ""}
    ${intro ? `${nl2br(intro)}` : ""}
    ${bulletHtml}
    ${ctaHtml}
...
function buildPlainTextHtml(copy: GeneratedEmailContent, companyName?: string): string {
  ...
  ${escapeHtml(copy.subject || 'Email')}
    ...
    ${bulletList}
      ...
      ${escapeHtml(copy.ctaLabel || 'Learn more')}
    
    
    ${escapeHtml(copy.closingLine || 'Best regards')}
...
function buildNewsletterHtml(copy: GeneratedEmailContent, companyName?: string, companyAddress?: string): string {
  ...
  ${escapeHtml(copy.subject || 'Email')}
      ...
      ${companyName ? `${escapeHtml(companyName)} DIGEST` : ''}
      ${escapeHtml(copy.heroTitle || 'This Week\'s Insights')}
      ...
      ${escapeHtml(copy.closingLine || 'See you next week!')}
...
router.post("/orders/create", async (req: Request, res: Response) => {
  try {
    ...
    if (!volumeRequested || volumeRequested  100) {
...
router.post("/emails/generate", async (req: Request, res: Response) => {
  try {
    ...
    const emails = [];
    for (let i = 0; i 
        I wanted to reach out and follow up on our previous conversation.
        We help businesses like yours streamline operations and improve efficiency.
        Would you have 15 minutes this week for a quick call?
        Looking forward to hearing from you.
      `,
      bodyHtml: `I wanted to reach out and follow up on our previous conversation.
        We help businesses like yours streamline operations and improve efficiency.
        Would you have 15 minutes this week for a quick call?
        Looking forward to hearing from you.`,
  ...
  try {
    ...
    const [reportRecord] = await db
      ...
      .values({
        ...
        reportData: result.data as unknown as Record,
    ...
    if (regularCampaignIds.length > 0) {
      ...
      if (publishedRegularCampaignIds.length > 0) {
        ...
        const accountCounts = await db
          .select({
            campaignId: leads.campaignId,
            count: sql`count(DISTINCT ${leads.accountName})::int`,
          })
          .from(leads)
          .where(
            and(
...
const DEFAULT_CAMPAIGN_PRICING: Record = {
