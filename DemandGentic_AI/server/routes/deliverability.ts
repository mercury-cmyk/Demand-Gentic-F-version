...
router.get('/dashboard', async (req: Request, res: Response) => {
  try {
    ...
    const warningDomains = domainHealthData.filter(
      (d) => d.healthScore >= 60 && d.healthScore  d.healthScore  d.blacklistListings > 0).length;
  ...
  try {
    ...
    let query = db
      .select({
        ...
        delivered: sql`SUM(${perDomainStats.delivered})`,
        bouncesHard: sql`SUM(${perDomainStats.bouncesHard})`,
        bouncesSoft: sql`SUM(${perDomainStats.bouncesSoft})`,
        complaints: sql`SUM(${perDomainStats.complaints})`,
        opens: sql`SUM(${perDomainStats.opens})`,
        clicks: sql`SUM(${perDomainStats.clicks})`,
      })
      .from(perDomainStats)
