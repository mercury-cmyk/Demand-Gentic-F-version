 ...
 */
async function getClientCampaignIds(clientAccountId: string): Promise {
  ...
  try {
    ...
    const [summary] = await db
      .select({
        ...
        totalDuration: sql`COALESCE(SUM(${callSessions.durationSec}), 0)::int`,
        avgDuration: sql`COALESCE(AVG(${callSessions.durationSec}), 0)::int`,
      })
      .from(callSessions)
      .where(
        and(
    ...
    const dispositions = await db
      .select({
        disposition: sql`COALESCE(${callSessions.aiDisposition}, 'unknown')`,
        count: sql`COUNT(*)::int`,
      })
      .from(callSessions)
      .where(
    ...
    const campaignBreakdown = await db
      .select({
        ...
        totalCalls: sql`COUNT(*)::int`,
        qualified: sql`COUNT(CASE WHEN ${callSessions.aiDisposition} IN ('qualified', 'qualified_lead', 'converted_qualified') THEN 1 END)::int`,
        notInterested: sql`COUNT(CASE WHEN ${callSessions.aiDisposition} = 'not_interested' THEN 1 END)::int`,
        voicemail: sql`COUNT(CASE WHEN ${callSessions.aiDisposition} = 'voicemail' THEN 1 END)::int`,
        noAnswer: sql`COUNT(CASE WHEN ${callSessions.aiDisposition} = 'no_answer' THEN 1 END)::int`,
        dncRequest: sql`COUNT(CASE WHEN ${callSessions.aiDisposition} IN ('dnc_request', 'do_not_call', 'dnc_added') THEN 1 END)::int`,
      })
      .from(callSessions)
...
router.get('/recordings', async (req: Request, res: Response) => {
  try {
    ...
    const visibility = (clientAccount?.visibilitySettings || {}) as Record;
    ...
    const recordings = await db
      .select({
        ...
        campaignName: campaigns.name,
        contactName: sql`COALESCE(CONCAT(${contacts.firstName}, ' ', ${contacts.lastName}), 'Unknown')`,
        accountName: sql`COALESCE(${accounts.name}, 'Unknown')`,
...
router.get('/analytics/engagement', async (req: Request, res: Response) => {
  try {
    ...
    const [leadStats] = await db
      .select({
        qualified: sql`COUNT(CASE WHEN ${leads.qaStatus} = 'approved' THEN 1 END)::int`,
      })
      .from(leads)
    ...
    const timeline = await db
      .select({
        date: sql`TO_CHAR(${callSessions.createdAt}, 'YYYY-MM-DD')`,
        calls: sql`COUNT(*)::int`,
        qualified: sql`COUNT(CASE WHEN ${callSessions.aiDisposition} IN ('qualified', 'qualified_lead', 'converted_qualified') THEN 1 END)::int`,
      })
      .from(callSessions)
    ...
    const emailTimeline = await db
      .select({
        date: sql`TO_CHAR(${emailSends.sentAt}, 'YYYY-MM-DD')`,
        emails: sql`COUNT(*)::int`,
      })
      .from(emailSends)
      .where(
        and(
    ...
    const sampleDispositions = await db
      .select({
        disposition: sql`COALESCE(${clientMockCalls.disposition}, 'sample_call')`,
        count: sql`COUNT(*)::int`,
      })
      .from(clientMockCalls)
      .innerJoin(qaGatedContent, and(
    ...
    const [agentBehavior] = await db
      .select({
        ...
        overall: sql`COALESCE(ROUND(AVG(${callQualityRecords.overallQualityScore})), 0)::int`,
        clarity: sql`COALESCE(ROUND(AVG(${callQualityRecords.clarityScore})), 0)::int`,
        engagement: sql`COALESCE(ROUND(AVG(${callQualityRecords.engagementScore})), 0)::int`,
        empathy: sql`COALESCE(ROUND(AVG(${callQualityRecords.empathyScore})), 0)::int`,
        objectionHandling: sql`COALESCE(ROUND(AVG(${callQualityRecords.objectionHandlingScore})), 0)::int`,
        qualification: sql`COALESCE(ROUND(AVG(${callQualityRecords.qualificationScore})), 0)::int`,
        closing: sql`COALESCE(ROUND(AVG(${callQualityRecords.closingScore})), 0)::int`,
        flowCompliance: sql`COALESCE(ROUND(AVG(${callQualityRecords.flowComplianceScore})), 0)::int`,
      })
    ...
    const [sampleBehavior] = await db
      .select({
        sampleSize: sql`COUNT(*)::int`,
        avgScore: sql`COALESCE(ROUND(AVG(${clientMockCalls.aiScore})), 0)::int`,
      })
      .from(clientMockCalls)
      .innerJoin(qaGatedContent, and(
        eq(qaGatedContent.id, clientMockCalls.qaContentId),
    ...
    const sampleRecentCalls = await db
      .select({
        ...
        campaignName: campaigns.name,
        contactName: sql`COALESCE(${clientMockCalls.callName}, 'Sample Call')`,
        accountName: sql`'Sample'`,
        disposition: sql`COALESCE(${clientMockCalls.disposition}, 'sample_call')`,
  ...
  try {
    ...
    const rawConversations = await db
      .select({
        ...
        // Recording info
        hasRecording: sql`(${callSessions.recordingS3Key} IS NOT NULL)`,
        recordingS3Key: callSessions.recordingS3Key,
      })
      .from(callSessions)
      .innerJoin(campaigns, eq(callSessions.campaignId, campaigns.id))
    ...
    const enriched = await Promise.all(
      emailCampaigns.map(async (campaign) => {
        ...
        const [sendStats] = await db
          .select({
            totalRecipients: sql`COUNT(DISTINCT ${emailSends.contactId})::int`,
            sent: sql`COUNT(*)::int`,
            delivered: sql`COUNT(CASE WHEN ${emailSends.status} = 'delivered' THEN 1 END)::int`,
...
router.get('/disposition-intelligence', requireClientFeature('disposition_overview'), async (req: Request, res: Response) => {
  try {
    ...
    const [totals] = await db
      .select({ totalCalls: sql`COUNT(*)::int` })
      .from(callSessions)
      .where(and(inArray(callSessions.campaignId, targetIds), eq(callSessions.status, 'completed')));
...
router.get('/cost-tracking', requireClientFeature('billing_cost_tracking'), async (req: Request, res: Response) => {
  try {
    ...
    const [emailCosts] = await db
      .select({ totalEmails: sql`COUNT(*)::int` })
      .from(emailSends)
    ...
    const [leadCosts] = await db
      .select({
        ...
        qualifiedLeads: sql`COUNT(CASE WHEN ${leads.qaStatus} IN ('approved', 'published') THEN 1 END)::int`,
      })
      .from(leads)
    ...
    const campaignBreakdown = await Promise.all(
      campaignIds.slice(0, 20).map(async (cId) => {
        ...
        const [cCalls] = await db.select({ count: sql`COUNT(*)::int` }).from(callSessions).where(and(eq(callSessions.campaignId, cId), eq(callSessions.status, 'completed')));
        const [cEmails] = await db.select({ count: sql`COUNT(*)::int` }).from(emailSends).where(eq(emailSends.campaignId, cId));
        const [cLeads] = await db.select({ count: sql`COUNT(*)::int` }).from(leads).where(and(eq(leads.campaignId, cId), inArray(leads.qaStatus, ['approved', 'published'])));
        return {
          campaignId: cId,
