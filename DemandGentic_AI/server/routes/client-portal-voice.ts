...
async function parseVoiceCommand(transcript: string): Promise {
  try {
...
function executeNavigation(action: string, entities: Record): CommandResult {
    ...
    switch (action) {
      ...
      case 'get_total_spend': {
        ...
        const [result] = await db
          .select({
            total: sql`COALESCE(SUM(total_cost), 0)::text`,
          })
      ...
      case 'get_lead_count': {
        const [result] = await db
          .select({
            count: sql`COALESCE(SUM(quantity), 0)::int`,
          })
          .from(clientActivityCosts)
          .where(
            and(
  ...
  try {
    switch (action) {
      ...
      case 'billing_summary': {
        ...
        const [invoices] = await db
          .select({
            pending: sql`count(*) FILTER (WHERE status = 'sent')::int`,
            overdue: sql`count(*) FILTER (WHERE status = 'overdue')::int`,
          })
          .from(clientInvoices)
...
async function generateAudioResponse(text: string, voice: string = 'nova'): Promise {
  try {
...
router.post('/command', async (req: Request, res: Response) => {
  ...
  try {
    ...
    await db.insert(clientVoiceCommands).values({
      ...
      actionResult: result.data as Record,
