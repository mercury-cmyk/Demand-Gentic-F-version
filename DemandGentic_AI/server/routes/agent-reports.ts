  ...
  try {
    ...
    const agentStats = await db
      .select({
        ...
        totalCalls: sql`COUNT(DISTINCT ${leads.id})`.as('total_calls'),
        qualifiedLeads: sql`COUNT(DISTINCT CASE WHEN ${leads.qaData}->>'ai_result' = 'Qualified' THEN ${leads.id} END)`.as('qualified_leads'),
        acceptedLeads: sql`COUNT(DISTINCT CASE WHEN ${leads.qaStatus} IN ('approved', 'pending_pm_review', 'published', 'Accepted') THEN ${leads.id} END)`.as('accepted_leads'),
        rejectedLeads: sql`COUNT(DISTINCT CASE WHEN ${leads.qaStatus} IN ('rejected', 'returned', 'Rejected') THEN ${leads.id} END)`.as('rejected_leads'),
        pendingReview: sql`COUNT(DISTINCT CASE WHEN ${leads.qaStatus} IN ('new', 'under_review', 'Pending Review') THEN ${leads.id} END)`.as('pending_review'),
        avgCallDuration: sql`AVG(${leads.callDuration})`.as('avg_call_duration'),
      })
      .from(leads)
      .where(
        and(
