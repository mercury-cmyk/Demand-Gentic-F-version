 ...
 */
const DISPOSITION_DISPLAY_MAP: Record = {
  ...
  try {
    ...
    const queueStats = await db
      .select({
        ...
        count: sql`COUNT(DISTINCT ${agentQueue.contactId})::int`,
      })
      .from(agentQueue)
    ...
    const campaignBreakdown = await db
      .select({
        ...
        total: sql`COUNT(DISTINCT ${agentQueue.contactId})::int`,
        queued: sql`COUNT(DISTINCT CASE WHEN ${agentQueue.queueState} = 'queued' THEN ${agentQueue.contactId} END)::int`,
        inProgress: sql`COUNT(DISTINCT CASE WHEN ${agentQueue.queueState} = 'in_progress' THEN ${agentQueue.contactId} END)::int`,
        completed: sql`COUNT(DISTINCT CASE WHEN ${agentQueue.queueState} = 'completed' THEN ${agentQueue.contactId} END)::int`,
      })
      .from(agentQueue)
  ...
  try {
    ...
    const legacyDispositionStats = await db
      .select({
        ...
        count: sql`COUNT(DISTINCT ${callSessions.id})::int`,
        totalDuration: sql`SUM(COALESCE(${callSessions.durationSec}, 0))::int`,
      })
      .from(callSessions)
    ...
    const dialerDispositionStats = await db
      .select({
        ...
        count: sql`COUNT(*)::int`,
        totalDuration: sql`SUM(COALESCE(${dialerCallAttempts.callDurationSeconds}, 0))::int`,
        connected: sql`COUNT(CASE WHEN ${dialerCallAttempts.connected} = true THEN 1 END)::int`,
      })
    ...
    const legacyCampaignStats = await db
      .select({
        campaignId: campaigns.id,
        campaignName: campaigns.name,
        totalCalls: sql`COUNT(DISTINCT ${callSessions.id})::int`,
        qualified: sql`COUNT(DISTINCT CASE WHEN ${dispositions.systemAction} = 'converted_qualified' THEN ${callSessions.id} END)::int`,
        avgDuration: sql`AVG(COALESCE(${callSessions.durationSec}, 0))::int`,
      })
      .from(callSessions)
      .innerJoin(callJobs, eq(callSessions.callJobId, callJobs.id))
    ...
    const dialerCampaignStats = await db
      .select({
        ...
        connected: sql`COUNT(CASE WHEN ${dialerCallAttempts.connected} = true THEN 1 END)::int`,
        avgDuration: sql`AVG(COALESCE(${dialerCallAttempts.callDurationSeconds}, 0))::int`,
      })
      .from(dialerCallAttempts)
    ...
    const qaStats = await db
      .select({
        ...
        count: sql`COUNT(DISTINCT ${leads.id})::int`,
      })
      .from(leads)
 ...
 */
const DISPLAY_TO_CANONICAL_MAP: Record = {
  ...
  try {
    ...
    const legacyQuery = db
      .select({
        ...
        contactName: sql`CONCAT(${contacts.firstName}, ' ', ${contacts.lastName})`,
    ...
    const dialerQuery = db
      .select({
        ...
        campaignName: campaigns.name,
        agentId: dialerCallAttempts.humanAgentId,
        agentName: sql`COALESCE(CONCAT(${users.firstName}, ' ', ${users.lastName}), '🤖 ' || ${virtualAgents.name}, 'AI Agent')`,
        contactId: contacts.id,
        contactName: sql`CONCAT(${contacts.firstName}, ' ', ${contacts.lastName})`,
        contactEmail: contacts.email,
        contactPhone: contacts.directPhoneE164,
