...
function parseLimit(value: unknown, fallback = 100, max = 500): number {
  ...
  if (!Number.isFinite(parsed) || parsed `COUNT(*)::int`,
    })
    .from(emailSends)
...
async function getCampaignEmailStats(campaignId: string) {
  const [sendStats] = await db
    .select({
      ...
      failed: sql`COUNT(CASE WHEN ${emailSends.status} = 'failed' THEN 1 END)::int`,
    })
    .from(emailSends)
    .where(eq(emailSends.campaignId, campaignId));
  ...
  const [eventStats] = await db
    .select({
      totalOpens: sql`COUNT(CASE WHEN ${emailEvents.type} = 'opened' THEN 1 END)::int`,
      uniqueOpens: sql`COUNT(DISTINCT CASE WHEN ${emailEvents.type} = 'opened' THEN ${emailEvents.recipient} END)::int`,
      totalClicks: sql`COUNT(CASE WHEN ${emailEvents.type} = 'clicked' THEN 1 END)::int`,
      uniqueClicks: sql`COUNT(DISTINCT CASE WHEN ${emailEvents.type} = 'clicked' THEN ${emailEvents.recipient} END)::int`,
      unsubscribed: sql`COUNT(CASE WHEN ${emailEvents.type} = 'unsubscribed' THEN 1 END)::int`,
      spamComplaints: sql`COUNT(CASE WHEN ${emailEvents.type} = 'complained' THEN 1 END)::int`,
      hardBounces: sql`COUNT(CASE WHEN ${emailEvents.type} = 'bounced' AND ${emailEvents.bounceType} = 'hard' THEN 1 END)::int`,
      softBounces: sql`COUNT(CASE WHEN ${emailEvents.type} = 'bounced' AND ${emailEvents.bounceType} = 'soft' THEN 1 END)::int`,
    })
    .from(emailEvents)
