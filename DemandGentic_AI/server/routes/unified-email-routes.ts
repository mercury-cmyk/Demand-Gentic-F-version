  ...
  try {
    ...
    const stats = await db
      .select({
        ...
        successfulRequests: sql`COUNT(*) FILTER (WHERE status = 'completed')`,
        failedRequests: sql`COUNT(*) FILTER (WHERE status = 'failed')`,
        cachedRequests: sql`COUNT(*) FILTER (WHERE status = 'cached')`,
        avgLatencyMs: sql`AVG(latency_ms)`,
        fallbacksUsed: sql`COUNT(*) FILTER (WHERE fallback_used = true)`,
      })
      .from(emailGenerationLogs)
    ...
    const providerStats = await db
      .select({
        ...
        count: sql`COUNT(*)`,
        successRate: sql`ROUND(COUNT(*) FILTER (WHERE status = 'completed') * 100.0 / COUNT(*), 2)`,
        avgLatencyMs: sql`AVG(latency_ms)`,
      })
      .from(emailGenerationLogs)
      .where(gte(emailGenerationLogs.requestedAt, startDate))
