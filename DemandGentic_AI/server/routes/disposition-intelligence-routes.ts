  ...
  try {
    ...
    const distributionRows = await db
      .select({
        ...
        count: sql`count(*)::int`,
        avgDuration: sql`avg(${dialerCallAttempts.callDurationSeconds})::int`,
      })
    ...
    const qualityByDisposition = await db
      .select({
        ...
        avgScore: sql`avg(${callQualityRecords.overallQualityScore})::int`,
        totalRecords: sql`count(*)::int`,
        accurateCount: sql`sum(case when ${callQualityRecords.dispositionAccurate} = true then 1 else 0 end)::int`,
        inaccurateCount: sql`sum(case when ${callQualityRecords.dispositionAccurate} = false then 1 else 0 end)::int`,
      })
    ...
    const timeSeriesRows = await db
      .select({
        date: sql`to_char(${dialerCallAttempts.createdAt}, 'YYYY-MM-DD')`,
        disposition: dialerCallAttempts.disposition,
        count: sql`count(*)::int`,
      })
      .from(dialerCallAttempts)
    ...
    const campaignMap: Record; total: number; qualified: number }> = {};
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        disposition: sql`coalesce(${callSessions.aiDisposition}, ${dialerCallAttempts.disposition}::text)`,
        transcript: sql`coalesce(${callSessions.aiTranscript}, ${dialerCallAttempts.fullTranscript}, ${dialerCallAttempts.aiTranscript})`,
      })
      .from(callSessions)
    ...
    const detectionByCallId: Record = {};
...
router.get("/deep-dive", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const rows = await db
      .select({
        callSessionId: callSessions.id,
        contactName: sql`coalesce(${contacts.fullName}, concat(${contacts.firstName}, ' ', ${contacts.lastName}), 'Unknown')`,
        companyName: sql`coalesce(${accounts.name}, 'Unknown')`,
        campaignName: sql`coalesce(${campaigns.name}, 'Unknown')`,
    ...
    const callSessionIds = rows.map(r => r.callSessionId);
    let voicemailMap: Record = {};
    let attemptMap: Record = {};
    ...
    const voicemailPatterns: Array = [];
    if (disposition === 'voicemail') {
      const phraseCount: Record = {};
...
router.get("/agent-performance", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const [avgScores] = await db
      .select({
        avgOverall: sql`avg(${callQualityRecords.overallQualityScore})::int`,
        avgEngagement: sql`avg(${callQualityRecords.engagementScore})::int`,
        avgClarity: sql`avg(${callQualityRecords.clarityScore})::int`,
        avgEmpathy: sql`avg(${callQualityRecords.empathyScore})::int`,
        avgObjectionHandling: sql`avg(${callQualityRecords.objectionHandlingScore})::int`,
        avgQualification: sql`avg(${callQualityRecords.qualificationScore})::int`,
        avgClosing: sql`avg(${callQualityRecords.closingScore})::int`,
        avgFlowCompliance: sql`avg(${callQualityRecords.flowComplianceScore})::int`,
        totalAnalyzed: sql`count(*)::int`,
      })
      .from(callQualityRecords)
      .where(whereClause);
    ...
    const openingIssues: Record = {};
    const closingIssues: Record = {};
    const interruptionPatterns: Record = {};
  ...
  try {
    ...
    const [qualityMetrics] = await db
      .select({
        ...
        avgQuality: sql`avg(${callQualityRecords.overallQualityScore})::int`,
        avgAlignment: sql`avg(${callQualityRecords.campaignAlignmentScore})::int`,
        avgTalkingPoints: sql`avg(${callQualityRecords.talkingPointsCoverageScore})::int`,
        avgFlowCompliance: sql`avg(${callQualityRecords.flowComplianceScore})::int`,
        qualificationMet: sql`sum(case when ${callQualityRecords.qualificationMet} = true then 1 else 0 end)::int`,
      })
      .from(callQualityRecords)
    ...
    try {
      const [withIntel] = await db
        .select({
          count: sql`count(distinct ${callQualityRecords.id})::int`,
          avgQuality: sql`avg(${callQualityRecords.overallQualityScore})::int`,
        })
        .from(callQualityRecords)
    ...
    const trendRows = await db
      .select({
        date: sql`to_char(${callQualityRecords.createdAt}, 'YYYY-MM-DD')`,
        totalCalls: sql`count(*)::int`,
        avgQuality: sql`avg(${callQualityRecords.overallQualityScore})::int`,
        qualifiedCount: sql`sum(case when ${callQualityRecords.qualificationMet} = true then 1 else 0 end)::int`,
      })
      .from(callQualityRecords)
      .where(and(eq(callQualityRecords.campaignId, campaignId as string), ...dateConditions))
...
router.post("/generate-coaching", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    let sampledIds: Set;
    if (records.length  r.callSessionId));
    } else {
      sampledIds = new Set();
...
async function notifyQualityAnalysts(
  ...
  try {
    ...
    const html = `
        ...
        Potential Lead Disposition Override
        A disposition was overridden from the Potential Leads review page.
