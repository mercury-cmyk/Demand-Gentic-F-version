...
async function resolveListToContactIds(
  ...
  if (list.entityType === 'account') {
    ...
    for (let i = 0; i  contactIds.push(c.id));
    }
    ...
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        const releaseConditions = [
          ...
          sql`(${agentQueue.scheduledFor} IS NULL OR ${agentQueue.scheduledFor}  0) {
          ...
          let filterContactIds: Set | null = null;
          if (hasFilterGroup) {
            filterContactIds = new Set();
          ...
          let sourceContactIds: Set | null = null;

          if (hasLists) {
            sourceContactIds = sourceContactIds || new Set();
        ...
        for (let i = 0; i  [row.contacts.id, row]));
        ...
        if (!allow_sharing) {
          ...
          const activeStates: Array = ['queued', 'locked', 'in_progress'];
          
          // Batch to avoid parameter limits
          const availableContactIds = availableContacts.map(c => c.id);
          const existingAssignments: any[] = [];
          for (let i = 0; i  a.contactId));
    ...
    try {
      ...
      for (const listId of allListIds) {
        const [list] = await db.select({
          ...
          recordCount: sql`array_length(${lists.recordIds}, 1)`,
