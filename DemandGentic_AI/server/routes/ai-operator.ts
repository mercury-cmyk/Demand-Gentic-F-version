...
function convertToGeminiFunctionDeclarations() {
  return AVAILABLE_TOOLS.map(tool => {
    ...
    if (fn.parameters.properties) {
      for (const [key, prop] of Object.entries(fn.parameters.properties as Record)) {
...
function normalizeToolArgs(toolName: T, rawArgs: any) {
...
function combineFilters(...filters: Array) {
  const defined = filters.filter(Boolean) as SQL[];
  return defined.length ? and(...defined) : undefined;
}

// Execute tool calls
async function executeTool(toolName: string, args: any, context: ToolExecutionContext): Promise {
  ...
  try {
    ...
    switch (toolName) {
      case 'count_records': {
        const parsedArgs = normalized as z.infer;
...
async function executeWithVertexAI(
  ...
  while (!isComplete && iterations  p.functionCall);
