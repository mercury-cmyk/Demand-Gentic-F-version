...
router.get("/api/content-promotion/pages", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (shouldIncludeStudioDraftProjects) {
      const studioProjectsById = new Map();
...
router.get("/api/content-promotion/pages/:id/analytics", requireAuth, async (req, res) => {
  try {
    ...
    const [viewStats] = await db
      .select({
        totalViews: sql`count(*)::int`,
        uniqueViews: sql`count(distinct ${contentPromotionPageViews.visitorEmail})::int`,
        submissions: sql`count(*) filter (where ${contentPromotionPageViews.eventType} = 'form_submit')::int`,
      })
      ...
      try {
        ...
        if (provider) {
          ...
          const html = `
              ...
              ` : ""}
              ${emailConfig.footerText ? `${emailConfig.footerText}` : ""}
              ${companyName ? `Sent by ${companyName}` : ""}
