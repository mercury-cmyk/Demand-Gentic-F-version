 ...
 */
async function checkClientCampaignAccess(clientAccountId: string, campaignId: string): Promise {
...
const PROSPECT_PERSONAS: Record = {
  'friendly_dm': {
...
router.post('/start', async (req: Request, res: Response) => {
  try {
    ...
    if (aiSettings?.persona) {
      ...
      agentInfo.voice = persona.voice || aiSettings.voice || agentInfo.voice;
    }
...
router.post('/chat', async (req: Request, res: Response) => {
  try {
    ...
    if (!session && campaignId) {
      ...
      const aiSettings2 = campaign.aiAgentSettings as any;
      if (aiSettings2?.persona) {
        const persona = aiSettings2.persona;
        agentInfo.name = persona.name || agentInfo.name;
        agentInfo.companyName = persona.companyName || agentInfo.companyName;
        agentInfo.role = persona.role || agentInfo.role;
        agentInfo.voice = persona.voice || aiSettings2.voice || agentInfo.voice;
      }
 ...
 */
function getAvailableVariables(): Array {
  return [
...
function sanitizeHtmlEmail(html: string): string {
  ...
  clean = clean.replace(/]*stylesheet[^>]*>/gi, '');
...
Best regards,{{senderName}}{{senderTitle}}
...
router.post('/generate-email', async (req: Request, res: Response) => {
  try {
    ...
    const emailTypeToVariant: Record = {
 ...
 */
async function resolveClientVirtualAgentId(campaignId: string): Promise {
  const [assignment] = await db
    .select()
    .from(campaignAgentAssignments)
    .where(eq(campaignAgentAssignments.campaignId, campaignId))
...
type ClientPostCallResult = {
  ...
  postCallAnalysis: Record;
};

function buildTranscriptText(transcripts: Array): string {
  return transcripts
    .map((entry) => {
...
function inferDispositionFromTranscript(transcriptText: string, metadata: Record): string {
...
async function finalizePhoneTestPostCall(sessionId: string): Promise {
  ...
  const metadata = ((session.metadata as Record) || {}) as Record;
  ...
  if (metadata.finalDisposition && metadata.postCallAnalysis && existingTurnCount === turnCount) {
    return {
      ...
      postCallAnalysis: metadata.postCallAnalysis as Record,
    };
  }

  // First check if the voice-dialer's full post-call analysis pipeline has already
  ...
  let voiceDialerAnalysis: Record | null = null;
  ...
  if (callSessionId || callControlId) {
    try {
      ...
      console.warn('[Client Phone Test] callSessions lookup failed:', lookupErr);
    }
  }

  const finalDisposition = voiceDialerDisposition || inferDispositionFromTranscript(transcriptText, metadata);

  let conversationQuality: Record | null = null;
...
router.post('/phone-test/start', async (req: Request, res: Response) => {
  try {
    ...
    // Resolve virtual agent for this campaign
    const resolvedAgentId = await resolveClientVirtualAgentId(campaignId);
    ...
    try {
      const callerIdResult = await getCallerIdForCall({
        ...
        virtualAgentId: resolvedAgentId || undefined,
    ...
    if (process.env.NODE_ENV !== 'production' && process.env.PUBLIC_WEBHOOK_HOST) {
      ...
      console.log(`[Client Phone Test] Using ngrok tunnel host: ${webhookHost}`);
    } else {
