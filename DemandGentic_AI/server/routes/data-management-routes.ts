...
router.get('/overview', async (_req: Request, res: Response) => {
  try {
    ...
    const [requestStats] = await db.select({
      total: sql`count(*)::int`,
      pending: sql`count(case when ${dataRequests.status} = 'requested' then 1 end)::int`,
      inProgress: sql`count(case when ${dataRequests.status} = 'in_progress' then 1 end)::int`,
    }).from(dataRequests);

    // Get recent uploads count
    const [uploadStats] = await db.select({
      total: sql`count(*)::int`,
      processing: sql`count(case when ${dataUploads.status} in ('pending', 'validating', 'processing', 'intelligence_running') then 1 end)::int`,
      completed: sql`count(case when ${dataUploads.status} = 'completed' then 1 end)::int`,
    }).from(dataUploads);

    // Get open quality issues
    const [issueStats] = await db.select({
      total: sql`count(*)::int`,
      critical: sql`count(case when ${dataQualityIssues.severity} = 'critical' then 1 end)::int`,
      high: sql`count(case when ${dataQualityIssues.severity} = 'high' then 1 end)::int`,
      unresolved: sql`count(case when ${dataQualityIssues.status} = 'open' then 1 end)::int`,
        ...
        SELECT
          COALESCE(
            ...
            CASE
              ...
              WHEN annual_revenue >'accountType'), ''),
...
router.get('/requests', async (req: Request, res: Response) => {
  try {
    ...
    const [{ total }] = await db.select({ total: sql`count(*)::int` }).from(dataRequests);
...
router.get('/uploads', async (req: Request, res: Response) => {
  try {
    ...
    const [{ total }] = await db.select({ total: sql`count(*)::int` }).from(dataUploads);
...
router.get('/enrichment/gaps', async (_req: Request, res: Response) => {
  try {
    const [contactStats] = await db.select({
      total: sql`count(*)::int`,
      missingEmail: sql`count(case when email IS NULL OR email = '' then 1 end)::int`,
      missingPhone: sql`count(case when direct_phone IS NULL OR direct_phone = '' then 1 end)::int`,
      missingMobile: sql`count(case when mobile_phone IS NULL OR mobile_phone = '' then 1 end)::int`,
      missingTitle: sql`count(case when job_title IS NULL OR job_title = '' then 1 end)::int`,
      missingSeniority: sql`count(case when seniority_level IS NULL OR seniority_level = '' then 1 end)::int`,
      missingDepartment: sql`count(case when department IS NULL OR department = '' then 1 end)::int`,
      missingLinkedIn: sql`count(case when linkedin_url IS NULL OR linkedin_url = '' then 1 end)::int`,
      missingCountry: sql`count(case when country IS NULL OR country = '' then 1 end)::int`,
      missingState: sql`count(case when state IS NULL OR state = '' then 1 end)::int`,
      missingCity: sql`count(case when city IS NULL OR city = '' then 1 end)::int`,
      missingAccount: sql`count(case when account_id IS NULL then 1 end)::int`,
    }).from(contacts).where(isNull(contacts.deletedAt));

    const [accountStats] = await db.select({
      total: sql`count(*)::int`,
      missingIndustry: sql`count(case when industry_standardized IS NULL OR industry_standardized = '' then 1 end)::int`,
      missingRevenue: sql`count(case when annual_revenue IS NULL then 1 end)::int`,
      missingEmployees: sql`count(case when staff_count IS NULL AND (employees_size_range IS NULL OR employees_size_range = '') then 1 end)::int`,
      missingDomain: sql`count(case when domain IS NULL OR domain = '' then 1 end)::int`,
      missingCountry: sql`count(case when hq_country IS NULL OR hq_country = '' then 1 end)::int`,
      missingPhone: sql`count(case when main_phone IS NULL OR main_phone = '' then 1 end)::int`,
      missingLinkedIn: sql`count(case when linkedin_url IS NULL OR linkedin_url = '' then 1 end)::int`,
    }).from(accounts);
...
router.get('/analytics/field-coverage', async (_req: Request, res: Response) => {
  try {
    const [contactCoverage] = await db.select({
      total: sql`count(*)::int`,
      email: sql`count(case when ${contacts.email} is not null then 1 end)::int`,
      jobTitle: sql`count(case when ${contacts.jobTitle} is not null then 1 end)::int`,
      directPhone: sql`count(case when ${contacts.directPhone} is not null then 1 end)::int`,
      mobilePhone: sql`count(case when ${contacts.mobilePhone} is not null then 1 end)::int`,
      seniorityLevel: sql`count(case when ${contacts.seniorityLevel} is not null then 1 end)::int`,
      department: sql`count(case when ${contacts.department} is not null then 1 end)::int`,
      country: sql`count(case when ${contacts.country} is not null then 1 end)::int`,
      state: sql`count(case when ${contacts.state} is not null then 1 end)::int`,
      city: sql`count(case when ${contacts.city} is not null then 1 end)::int`,
      linkedinUrl: sql`count(case when ${contacts.linkedinUrl} is not null then 1 end)::int`,
    }).from(contacts);

    const [acctCoverage] = await db.select({
      total: sql`count(*)::int`,
      industry: sql`count(case when ${accounts.industryStandardized} is not null then 1 end)::int`,
      revenue: sql`count(case when ${accounts.annualRevenue} is not null then 1 end)::int`,
      employees: sql`count(case when ${accounts.staffCount} is not null or ${accounts.employeesSizeRange} is not null then 1 end)::int`,
      domain: sql`count(case when ${accounts.domain} is not null then 1 end)::int`,
      hqCountry: sql`count(case when ${accounts.hqCountry} is not null then 1 end)::int`,
      phone: sql`count(case when ${accounts.mainPhone} is not null then 1 end)::int`,
    }).from(accounts);
...
const DEPARTMENT_ALIASES: Record = {
...
router.post('/intelligence/gather', async (req: Request, res: Response) => {
  try {
    ...
    const resolvedAccountIds = new Set(explicitAccountIds);
    const resolutionSummary = {
      explicitAccountIds: explicitAccountIds.length,
      fromLists: 0,
      fromCampaigns: 0,
      listBreakdown: [] as Array,
...
function dedupeBy(items: T[], getKey: (item: T) => string): T[] {
  const seen = new Set();
  const out: T[] = [];
...
async function getAccountNameMap(accountIds: string[]): Promise> {
  const map = new Map();
