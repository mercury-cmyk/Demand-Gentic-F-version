...
function uniqueOrdered(values: Array): string[] {
  const seen = new Set();
  const output: string[] = [];
...
async function fetchOrgIntelligenceContext(): Promise {
...
function extractIcpPersonas(profile: OrganizationProfile | null): Array {
  if (!profile?.icp) return [];
  const icp = profile.icp as any;
...
function extractOutreachObjectionHandlers(profile: OrganizationProfile | null): Array {
  if (!profile?.outreach) return [];
  const out = profile.outreach as any;
