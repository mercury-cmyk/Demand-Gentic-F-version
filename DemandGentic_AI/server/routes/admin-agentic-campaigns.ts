  ...
  try {
    ...
    let query = db
      .select({
        ...
        assignedPmName: sql`CONCAT(${users.firstName}, ' ', ${users.lastName})`.as('assigned_pm_name'),
...
router.post('/agentic-campaign/:id/chat', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (extractedData && Object.keys(extractedData).length > 0) {
      ...
      const existingConfig = (session[stepConfigKey] as Record) || {};
...
Extract and return JSON with:
{
  ...
  "confidence": 0.0-1.0
    ...
    const contextConfig = (session.contextConfig as Record) || {};
...
async function extractConfigurationFromResponse(response: string, currentStep: string): Promise> {
  try {
...
async function generateStepConfiguration(session: AgenticCampaignSession, step: string): Promise> {
  const contextConfig = (session.contextConfig as any) || {};
  const audienceConfig = (session.audienceConfig as any) || {};
  const conversationHistory = session.conversationHistory || [];
