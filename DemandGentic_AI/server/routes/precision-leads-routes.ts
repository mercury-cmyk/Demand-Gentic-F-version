  ...
  try {
    const verdictCounts = await db
      .select({
        verdict: precisionLeadAnalyses.verdict,
        count: sql`count(*)`,
        avgConfidence: sql`round(avg(${precisionLeadAnalyses.consensusConfidence}))`,
        avgIntent: sql`round(avg(${precisionLeadAnalyses.consensusIntentScore}))`,
      })
      .from(precisionLeadAnalyses)
    ...
    const overrideCounts = await db
      .select({
        overridden: sql`count(*) filter (where ${precisionLeadAnalyses.overrideDisposition} = true)`,
        total: sql`count(*)`,
      })
      .from(precisionLeadAnalyses);
    ...
    const [recentRow] = await db
      .select({
        lastProcessed: sql`max(${precisionLeadAnalyses.processedAt})`,
        avgDuration: sql`round(avg(${precisionLeadAnalyses.processingDurationMs}))`,
      })
      .from(precisionLeadAnalyses);
