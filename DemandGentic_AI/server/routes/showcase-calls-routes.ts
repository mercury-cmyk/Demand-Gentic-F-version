...
async function enforceShowcaseDurationCap(): Promise {
  ...
  const rows = ((updated as any)?.rows ?? []) as Array;
  return rows.length;
}
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        transcriptExcerpt: sql`LEFT(${callQualityRecords.fullTranscript}, 200)`,
  ...
  try {
    ...
    const [showcaseStats] = await db
      .select({
        ...
        avgOverall: sql`ROUND(AVG(${callQualityRecords.overallQualityScore}))`,
        avgEngagement: sql`ROUND(AVG(${callQualityRecords.engagementScore}))`,
        avgClarity: sql`ROUND(AVG(${callQualityRecords.clarityScore}))`,
        avgEmpathy: sql`ROUND(AVG(${callQualityRecords.empathyScore}))`,
        avgObjectionHandling: sql`ROUND(AVG(${callQualityRecords.objectionHandlingScore}))`,
        avgFlowCompliance: sql`ROUND(AVG(${callQualityRecords.flowComplianceScore}))`,
      })
      .from(callQualityRecords)
      .innerJoin(callSessions, eq(callQualityRecords.callSessionId, callSessions.id))
      .leftJoin(campaigns, eq(callQualityRecords.campaignId, campaigns.id))
    ...
    const [conversationStats] = await db
      .select({
        ...
        avgScore: sql`ROUND(AVG(${callQualityRecords.overallQualityScore}))`,
        highPerformers: sql`COUNT(CASE WHEN (${agentPerformanceScoreSql()}) >= ${HIGH_PERFORMER_APS_THRESHOLD} THEN 1 END)`,
      })
      .from(callQualityRecords)
      .innerJoin(callSessions, eq(callQualityRecords.callSessionId, callSessions.id))
      .leftJoin(campaigns, eq(callQualityRecords.campaignId, campaigns.id))
...
// Bypass strict header check for this route to support  with query token
router.get("/:callSessionId/stream", async (req: Request, res: Response) => {
  try {
    ...
    if (!authenticatedUserId) {
      ...
      // headers to . We still attempt normal recording resolution.
