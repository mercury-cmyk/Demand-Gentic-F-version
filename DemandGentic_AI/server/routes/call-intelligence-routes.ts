...
let gcsDirectStorage: InstanceType | null = null;
try {
 ...
 */
async function downloadGcsAudioAsBuffer(gcsKey: string): Promise {
...
function extractGcsKeyFromRecordingUrl(recordingUrl: string | null | undefined): string | null {
  ...
  const payload = aiAnalysis as Record;
...
router.get("/unified", requireAuth, requireRole('admin', 'manager', 'qa_analyst'), async (req, res) => {
  try {
    ...
    if (endDate) {
      dialerConditions.push(sql`COALESCE(${dialerCallAttempts.callStartedAt}, ${dialerCallAttempts.createdAt})  0 ? and(...dialerConditions) : undefined;
...
router.get("/transcription-health", requireAuth, requireRole('admin', 'manager', 'qa_analyst'), async (req, res) => {
  try {
    ...
    const durationFilter = maxDuration
      ? sql`AND COALESCE(duration_sec, 0) > ${minDuration} AND COALESCE(duration_sec, 0)  ${minDuration}`;
    const dialerDurationFilter = maxDuration
      ? sql`AND COALESCE(call_duration_seconds, 0) > ${minDuration} AND COALESCE(call_duration_seconds, 0)  ${minDuration}`;
    ...
    const sessionsDaily = await db.execute(sql`
      SELECT
        ...
        count(CASE WHEN ai_transcript IS NULL OR length(ai_transcript) = ${startDate}
        AND started_at = 20)
                     OR (ai_transcript IS NOT NULL AND length(ai_transcript) >= 20) THEN 1 END) AS with_transcript,
        count(CASE WHEN (full_transcript IS NULL OR length(full_transcript) = ${startDate}
        AND call_started_at ();
    ...
    const last7 = daily.filter(d => (Date.now() - new Date(d.day).getTime()) / 86400000  s + d.totalRecordings, 0),
    ...
    const sessionsByCampaign = await db.execute(sql`
      SELECT
        ...
        count(CASE WHEN cs.ai_transcript IS NULL OR length(cs.ai_transcript) = ${startDate}
        AND cs.started_at = 20)
                     OR (dca.ai_transcript IS NOT NULL AND length(dca.ai_transcript) >= 20) THEN 1 END) AS with_transcript,
        count(CASE WHEN (dca.full_transcript IS NULL OR length(dca.full_transcript) = ${startDate}
        AND dca.call_started_at ();
...
router.get("/transcription-gaps", requireAuth, requireRole('admin', 'manager', 'qa_analyst'), async (req, res) => {
  try {
    ...
    if (gapType === 'transcript') {
      gapFilter = sql`(cs.ai_transcript IS NULL OR length(cs.ai_transcript)  ${minDuration} AND COALESCE(cs.duration_sec, 0)  ${minDuration}`;
    const dcaDurationFilter = maxDuration
      ? sql`AND COALESCE(dca.call_duration_seconds, 0) > ${minDuration} AND COALESCE(dca.call_duration_seconds, 0)  ${minDuration}`;
    ...
    const countResult = await db.execute(sql`
      ...
      WHERE cs.started_at >= ${since}
        AND cs.started_at = 20 THEN 'completed' ELSE 'missing' END AS transcript_status,
...
router.get("/transcription-calls", requireAuth, requireRole('admin', 'manager', 'qa_analyst'), async (req, res) => {
  try {
    ...
    const csDurationFilter = maxDuration
      ? sql`AND COALESCE(cs.duration_sec, 0) > ${minDuration} AND COALESCE(cs.duration_sec, 0)  ${minDuration}`;
    const dcaDurationFilter = maxDuration
      ? sql`AND COALESCE(dca.call_duration_seconds, 0) > ${minDuration} AND COALESCE(dca.call_duration_seconds, 0)  ${minDuration}`;
    ...
    if (transcriptionStatus === 'transcribed') {
      ...
      dcaTranscriptFilter = sql`(
        (dca.full_transcript IS NULL OR length(dca.full_transcript) = 20) AS transcribed,
             count(*) FILTER (WHERE cs.ai_transcript IS NULL OR length(cs.ai_transcript) = ${since}
        AND cs.started_at = 20)
                                 OR (dca.ai_transcript IS NOT NULL AND length(dca.ai_transcript) >= 20)) AS transcribed,
             count(*) FILTER (WHERE (dca.full_transcript IS NULL OR length(dca.full_transcript) = ${since}
        AND dca.call_started_at = 20 THEN 'completed' ELSE 'missing' END AS transcript_status,
        CASE WHEN EXISTS (SELECT 1 FROM call_quality_records cqr WHERE cqr.call_session_id = cs.id) THEN 'completed' ELSE 'missing' END AS analysis_status
      FROM call_sessions cs
      LEFT JOIN campaigns c ON cs.campaign_id = c.id
      WHERE cs.started_at >= ${since}
...
router.post("/regeneration/worker/config", requireAuth, requireRole('admin'), async (req, res) => {
  try {
    ...
    // Validate inputs
    if (concurrency !== undefined && (concurrency  10)) {
    ...
    if (batchDelayMs !== undefined && batchDelayMs  {
  try {
...
router.post("/backfill-analysis", async (req, res) => {
  try {
    ...
    for (let idx = 0; idx  setTimeout(r, 200));
      } catch (err: any) {
        failed++;
  ...
  try {
    ...
    const missing = await db.execute(sql`
      ...
      WHERE (cs.recording_url IS NOT NULL OR cs.recording_s3_key IS NOT NULL)
        AND (cs.ai_transcript IS NULL OR length(cs.ai_transcript)  ${minDuration}
        AND COALESCE(cs.duration_sec, 0)  ({
          id: r.id,
          durationSec: r.duration_sec,
