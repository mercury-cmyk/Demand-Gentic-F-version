    ...
    const projectsWithStats = await Promise.all(
      projects.map(async (project) => {
        ...
        const [costSum] = await db
          .select({ total: sql`COALESCE(SUM(total_cost), 0)::text` })
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const [costSummary] = await db
      .select({
        totalCost: sql`COALESCE(SUM(total_cost), 0)::text`,
        activityCount: sql`count(*)::int`,
      })
      .from(clientActivityCosts)
  ...
  try {
    ...
    const summary = await db
      .select({
        ...
        totalCost: sql`SUM(total_cost)::text`,
        count: sql`count(*)::int`,
      })
      .from(clientActivityCosts)
      .where(eq(clientActivityCosts.projectId, projectId))
