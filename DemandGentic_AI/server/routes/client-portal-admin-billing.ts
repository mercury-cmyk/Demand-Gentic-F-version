...
router.get('/invoices/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (businessProfile?.addressLine1) {
      ...
      const addr = billingConfig.billingAddress as Record;
  ...
  try {
    ...
    const costs = await db
      .select({
        ...
        count: sql`count(*)::int`,
        minDate: sql`MIN(activity_date)::text`,
        maxDate: sql`MAX(activity_date)::text`,
      })
      .from(clientActivityCosts)
      .where(
        and(
  ...
  try {
    ...
    const html = `
        ...
        ${invoiceLabel}
          ...
          Wire Transfer Details (USD)
          
            SWIFT/BIC:CHFGUS44021
            ABA Routing:091311229
            Bank:Choice Financial Group
            Account:202556901478
            Beneficiary:Pivotal B2B LLC
