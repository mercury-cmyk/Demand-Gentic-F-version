  ...
  try {
    ...
    if (!telnyxResponse.ok) {
      ...
      if (callInfo.isInternational && diagnosticHints.length === 0) {
        ...
        diagnosticHints.push("Short-duration failures ( 0
...
router.get("/:campaignId/test-calls-summary", requireDualAuth, requireRole("admin", "campaign_manager", "client", "client_user"), async (req, res) => {
  try {
    ...
    const stats = await db
      .select({
        total: sql`count(*)`,
        completed: sql`count(*) filter (where ${campaignTestCalls.status} = 'completed')`,
        successful: sql`count(*) filter (where ${campaignTestCalls.testResult} = 'success')`,
        needsImprovement: sql`count(*) filter (where ${campaignTestCalls.testResult} = 'needs_improvement')`,
        failed: sql`count(*) filter (where ${campaignTestCalls.testResult} = 'failed')`,
        avgDuration: sql`avg(${campaignTestCalls.durationSeconds})`,
      })
      .from(campaignTestCalls)
      .where(eq(campaignTestCalls.campaignId, campaignId));
