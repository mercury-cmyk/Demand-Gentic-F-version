...
async function processBatchJob(
  ...
  try {
    ...
    for (let i = 0; i  processOneCall(call, skipAnalysis))
      );
...
function buildFilterConditions(filters: {
  ...
  if (filters.includeFailedTranscriptions) {
    conditions.push(
      sql`(
        (${dialerCallAttempts.fullTranscript} IS NULL OR LENGTH(${dialerCallAttempts.fullTranscript}) = ${normalizeFilterDate(filters.dateFrom, "start")}`
    );
  }

  if (filters.dateTo) {
    conditions.push(
      sql`COALESCE(${dialerCallAttempts.callStartedAt}, ${dialerCallAttempts.createdAt})  {
  ...
  const rows = await db
    .select({
      ...
      hasAttemptRecordingUrl: sql`${dialerCallAttempts.recordingUrl} IS NOT NULL`,
      hasSessionRecordingUrl: sql`${callSessions.recordingUrl} IS NOT NULL`,
      hasS3Key: sql`${callSessions.recordingS3Key} IS NOT NULL`,
      createdAt: sql`COALESCE(${dialerCallAttempts.callStartedAt}, ${dialerCallAttempts.createdAt})`,
    })
