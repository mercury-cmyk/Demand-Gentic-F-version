...
async function ensureStore(agentVersion: string, promptHash: string | null): Promise {
  try {
...
function computeLineDiff(oldText: string, newText: string): { type: 'same' | 'add' | 'remove'; line: string }[] {
  ...
  let oi = 0, ni = 0;
  while (oi = oldLines.length || newLines[ni] !== oldLines[oi])) {
...
async function persistStore(store: VoiceTrainingStore): Promise {
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        contactName: sql`COALESCE(${contacts.firstName} || ' ' || ${contacts.lastName}, 'Unknown')`,
...
function scoreLiveTestTranscript(transcript: LiveTestTranscriptTurn[]): { scores: LiveTestScores; evaluation: { recommendations: string[]; conversationStages: string[] } } {
  ...
  const objectionScore = objectionHandled ? 85 : (userTurns.length = 2 ? 33 : qualificationQuestions >= 1 ? 20 : 0) +
    (valueDelivered ? 34 : 0) +
    (ctaDelivered ? 33 : 0)
  ));
  ...
  if (qualificationQuestions  1) recommendations.push('Acknowledge and handle objections more explicitly.');
