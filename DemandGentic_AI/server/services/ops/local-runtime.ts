...
async function commandExists(command: string, args: string[] = ["--version"]): Promise {
  try {
...
async function ensureJobsFile(): Promise {
...
async function writeJobs(jobs: OpsDeploymentJob[]): Promise {
  await ensureJobsFile();
  await fsp.writeFile(JOBS_FILE_PATH, JSON.stringify(jobs, null, 2), "utf8");
}

async function upsertJob(jobId: string, updater: (job: OpsDeploymentJob) => OpsDeploymentJob): Promise {
  const jobs = await readJobs();
  const nextJobs = jobs.map((job) => (job.id === jobId ? updater(job) : job));
  await writeJobs(nextJobs);
}

async function appendJob(job: OpsDeploymentJob): Promise {
  const jobs = await readJobs();
  jobs.unshift(job);
...
async function getGitValue(args: string[]): Promise {
  try {
...
async function parseComposeMetadata(): Promise {
  try {
...
async function getDockerRows(): Promise>> {
...
export async function getLocalOverview(): Promise {
  return {
...
export async function listLocalWorkspaceDirectory(relativePath = ""): Promise {
...
export async function readLocalWorkspaceFile(relativePath: string): Promise {
  const resolvedPath = resolveWorkspacePath(relativePath);
  const stats = await fsp.stat(resolvedPath);
  if (!stats.isFile()) {
...
export async function writeLocalWorkspaceFile(relativePath: string, content: string): Promise {
  const resolvedPath = resolveWorkspacePath(relativePath);
...
export async function createLocalWorkspaceFolder(relativePath: string): Promise {
  const resolvedPath = resolveWorkspacePath(relativePath);
  await fsp.mkdir(resolvedPath, { recursive: true });
  return { path: normalizeRelativePath(relativePath), created: true };
}

export async function deleteLocalWorkspaceEntry(relativePath: string): Promise {
  const resolvedPath = resolveWorkspacePath(relativePath);
  const stats = await fsp.stat(resolvedPath);
  if (stats.isDirectory()) {
...
export async function getLocalDeploymentStatus(): Promise {
...
export async function runLocalRestartAction(service: string): Promise {
  return startJob(
    "restart",
    "docker",
