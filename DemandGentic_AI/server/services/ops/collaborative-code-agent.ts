...
function parseJsonPayload(content: string): T | null {
  try {
  ...
  try {
    ...
    const payload =
      options.responseFormat === "json"
        ? parseJsonPayload(response.content)
...
async function runSharedCollaborativeStages(
  ...
  const architectureStage = await runStage(
    request,
    "architecture",
...
export async function runCollaborativeSingleFileEdit(
  ...
  const implementationStage = await runStage(
    request,
    "implementation",
