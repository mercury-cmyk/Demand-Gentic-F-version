...
export async function queueBulkEmails(options: BulkEmailOptions): Promise {
  const {
    campaignId,
    senderProfileId,
    from,
 ...
 */
export async function sendCampaignEmails(campaignId: string): Promise {
  ...
  console.log(`[sendCampaignEmails] Sending from ${fromName ? `${fromName} ` : fromEmail}`);
