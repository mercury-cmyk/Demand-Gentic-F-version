...
async function startRun(runId: string): Promise {
...
async function continueRun(runId: string, interruptResponse: InterruptResponse): Promise {
...
async function isRunPaused(runId: string): Promise {
...
async function getRunPlan(runId: string): Promise {
...
async function completeRun(runId: string): Promise {
  await db.update(agentCommandRuns)
    .set({
      status: 'completed',
...
async function failRun(runId: string, errorCode: string, errorMessage: string): Promise {
  await db.update(agentCommandRuns)
    .set({
      status: 'failed',
      errorCode,
...
async function executeUnderstandPhase(run: AgentCommandRun): Promise {
...
async function executePlanPhase(run: AgentCommandRun): Promise {
  let plan: PlanStep[] = [];
...
async function executeVerifyPhase(runId: string): Promise {
  // Update status
  await db.update(agentCommandRuns)
...
async function executeSummarizePhase(runId: string): Promise {
  // Get all artifacts
  const artifacts = await db.select()
    .from(agentCommandArtifacts)
    .where(eq(agentCommandArtifacts.runId, runId));
...
export async function getRunSteps(runId: string): Promise {
  return db.select()
    .from(agentCommandSteps)
    .where(eq(agentCommandSteps.runId, runId))
    .orderBy(agentCommandSteps.idx);
...
export async function getRunInterrupts(runId: string): Promise {
  return db.select()
    .from(agentCommandInterrupts)
...
export async function getPendingInterrupt(runId: string): Promise {
  const [interrupt] = await db.select()
    .from(agentCommandInterrupts)
    .where(and(
      eq(agentCommandInterrupts.runId, runId),
