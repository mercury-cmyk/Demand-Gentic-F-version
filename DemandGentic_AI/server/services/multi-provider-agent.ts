...
export class MultiProviderOrchestrator {
  ...
  private providerMetrics: Map = new Map();
  ...
  private async callCodex(request: AgentRequest): Promise {
  ...
  private async callClaude(request: AgentRequest): Promise {
    const startTime = Date.now();
    const profile = normalizeOptimizationProfile(request.optimizationProfile);
  ...
  private async callGemini(request: AgentRequest): Promise {
    const startTime = Date.now();
    const profile = normalizeOptimizationProfile(request.optimizationProfile);
  ...
  private async callKimi(request: AgentRequest): Promise {
    const startTime = Date.now();
    const profile = normalizeOptimizationProfile(request.optimizationProfile);
  ...
  private async callDeepSeek(request: AgentRequest): Promise {
    const startTime = Date.now();
    const profile = normalizeOptimizationProfile(request.optimizationProfile);
...
export async function aiAgentCall(request: AgentRequest): Promise {
