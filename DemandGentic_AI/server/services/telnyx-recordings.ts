...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt  controller.abort(), TELNYX_TIMEOUT_MS);
 ...
 */
export async function fetchTelnyxRecording(callControlId: string): Promise {
  if (!TELNYX_API_KEY) {
...
async function isResponseEmpty(response: Response): Promise {
    try {
 ...
 */
export async function updateLeadRecording(leadId: string, telnyxCallId: string): Promise {
  try {
 ...
 */
export async function updateCallRecording(callId: string, telnyxCallId: string): Promise {
  try {
 ...
 */
export async function syncRecordingForLead(leadId: string): Promise {
  const [lead] = await db
    .select({
      lead: leads,
      contact: contacts,
