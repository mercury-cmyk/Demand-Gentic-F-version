...
type BrevoProvider = NonNullable>>;
...
async function ensureBrevoDomainHealthScore(domainAuthId: number, isVerified: boolean): Promise {
...
async function requireBrevoProvider(providerId: string): Promise {
...
async function loadLocalSenderMatches(): Promise> {
...
async function loadLocalDomainMatches(): Promise> {
...
function normalizeDnsRecords(raw: Record): BrevoManagedDomain['dnsRecords'] {
...
async function listBrevoSenders(provider: BrevoProvider): Promise {
...
export async function getBrevoDomain(providerId: string, domainName: string): Promise {
...
async function listBrevoDomains(provider: BrevoProvider): Promise {
  const domainMatches = await loadLocalDomainMatches();
...
async function listBrevoDedicatedIps(provider: BrevoProvider): Promise {
...
export async function getBrevoInfrastructureOverview(providerId: string): Promise {
  const provider = await requireBrevoProvider(providerId);
...
export async function deleteBrevoSender(providerId: string, senderId: string): Promise {
  const provider = await requireBrevoProvider(providerId);
  await brevoRequest(provider, `/senders/${encodeURIComponent(senderId)}`, {
    method: 'DELETE',
  });
...
export async function validateBrevoSender(providerId: string, senderId: string, otp: string): Promise {
  const provider = await requireBrevoProvider(providerId);
...
export async function authenticateBrevoDomain(providerId: string, domainName: string): Promise {
  const provider = await requireBrevoProvider(providerId);
...
export async function deleteBrevoDomain(providerId: string, domainName: string): Promise {
  const provider = await requireBrevoProvider(providerId);
...
export async function syncBrevoAssetsToDashboard(providerId: string): Promise {
  const provider = await requireBrevoProvider(providerId);
