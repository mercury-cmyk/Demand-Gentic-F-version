 ...
 *   const result = await analyzeJSON(prompt, { maxTokens: 4096 });
 */
...
// Default: 100% DeepSeek — all analysis routed through DeepSeek API.
// Other providers remain as fallbacks (weight 0) in case DeepSeek is unhealthy.
// Override via env: AI_ROUTER_DEEPSEEK_WEIGHT=50 AI_ROUTER_VERTEX_WEIGHT=20 etc.
const VERTEX_WEIGHT = parseInt(process.env.AI_ROUTER_VERTEX_WEIGHT || "0", 10);
const CLAUDE_WEIGHT = parseInt(process.env.AI_ROUTER_CLAUDE_WEIGHT || "0", 10);
const DEEPSEEK_WEIGHT = parseInt(process.env.AI_ROUTER_DEEPSEEK_WEIGHT || "100", 10);
const OPENAI_WEIGHT = parseInt(process.env.AI_ROUTER_OPENAI_WEIGHT || "0", 10);
...
async function callDeepSeek(prompt: string, options: AnalysisOptions): Promise {
  ...
  return parseJSONResponse(text, "DeepSeek");
}

async function callOpenAI(prompt: string, options: AnalysisOptions): Promise {
...
async function callClaude(prompt: string, options: AnalysisOptions): Promise {
...
async function callVertex(prompt: string, options: AnalysisOptions): Promise {
  if (options.deep) {
    return deepAnalyzeJSON(prompt, {
      temperature: options.temperature ?? 0.2,
...
function parseJSONResponse(text: string, provider: string): T {
...
function selectProvider(options: AnalysisOptions): AnalysisProvider {
  ...
  if (isProviderAvailable("vertex")) {
    ...
    if (tick (prompt: string, options: AnalysisOptions = {}): Promise {
  ...
  for (const provider of providers) {
    try {
      ...
      switch (provider) {
        case "vertex":
          result = await callVertex(prompt, providerOptions);
          break;
        case "claude":
          result = await callClaude(prompt, providerOptions);
          break;
        case "deepseek":
          result = await callDeepSeek(prompt, providerOptions);
          break;
        case "openai":
          result = await callOpenAI(prompt, providerOptions);
          break;
      }
 ...
 */
export async function deepAnalyze(prompt: string, options: Omit = {}): Promise {
  return analyzeJSON(prompt, { ...options, deep: true });
}
