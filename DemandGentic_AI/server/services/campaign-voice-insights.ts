...
async function calculateCampaignMetrics(
  ...
  const qualityResult = await db
    .select({
      avgScore: sql`AVG((${leads.analysis}->>'qualityScore'->>'overallScore')::numeric)`,
    })
    .from(leads)
...
async function getHourlyPerformance(
  ...
  const hourlyData = await db
    .select({
      hour: sql`EXTRACT(HOUR FROM ${dialerCallAttempts.createdAt})`,
      total: count(),
      connected: sql`SUM(CASE WHEN ${dialerCallAttempts.connected} = true THEN 1 ELSE 0 END)`,
      qualified: sql`SUM(CASE WHEN ${dialerCallAttempts.disposition} = 'qualified_lead' THEN 1 ELSE 0 END)`,
    })
    .from(dialerCallAttempts)
    .where(and(...conditions))
...
function generateCampaignInsights(
  ...
  } else if (metrics.qualificationRate  20) {
    insights.push({
      type: "warning",
...
export async function getCampaignDispositions(
  ...
  const periodData = await db
    .select({
      period: sql`TO_CHAR(${dialerCallAttempts.createdAt}, ${dateFormat})`,
      disposition: dialerCallAttempts.disposition,
      count: count(),
    })
    .from(dialerCallAttempts)
