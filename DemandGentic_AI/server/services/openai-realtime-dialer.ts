...
interface OpenAIRealtimeSession {
  ...
  responseTimeoutHandle: ReturnType | null;
  softTimeoutTriggered: boolean;
  systemPromptOverride: string | null;
  firstMessageOverride: string | null;
  voiceOverride: string | null;
  agentSettingsOverride: Partial | null;
  voiceVariables: Record | null;
...
const ENGAGED_DISPOSITIONS = new Set([
  "qualified_lead",
  "not_interested",
  "do_not_call",
  "callback_requested",
      ...
      try {
        ...
        if (message.event === "start") {
          ...
          const agentSettingsOverride = customParams.agent_settings
            ...
            ? (customParams.agent_settings as Partial)
            : null;
...
function mergeAgentSettings(raw?: Partial): VirtualAgentSettings {
  return {
    systemTools: {
...
async function initializeOpenAISession(session: OpenAIRealtimeSession): Promise {
  ...
  try {
    ...
    const agentSettings = mergeAgentSettings(baseSettings as Partial | undefined);
    ...
    openaiWs.on("open", async () => {
      ...
      const transcriptionConfig: Record = transcriptionEnabled ? {
...
async function initializeGoogleSession(session: OpenAIRealtimeSession): Promise {
...
async function handleOpenAIMessage(session: OpenAIRealtimeSession, message: any): Promise {
  const { type } = message;
 ...
 */
async function injectIdentityLockReminder(session: OpenAIRealtimeSession): Promise {
  if (!session.openaiWs || session.openaiWs.readyState !== WebSocket.OPEN) {
    return;
  }
...
async function handleFunctionCall(session: OpenAIRealtimeSession, message: any): Promise {
...
async function checkForVoicemailDetection(session: OpenAIRealtimeSession, transcript: string): Promise {
...
async function handleTelnyxMedia(session: OpenAIRealtimeSession, message: any): Promise {
  if (!session.openaiWs || session.openaiWs.readyState !== WebSocket.OPEN) {
    return;
  }
...
async function endCall(callId: string, outcome: 'completed' | 'no_answer' | 'voicemail' | 'error'): Promise {
  ...
  let dispositionResult: Awaited> | null = null;
...
async function validateSessionIdentifiers(
  ...
  try {
    ...
    if (queueItem.lockExpiresAt && new Date(queueItem.lockExpiresAt)  {
...
async function getCampaignConfig(campaignId: string): Promise {
  ...
  try {
    ...
    return {
      ...
      campaignObjections: campaign.campaignObjections as Array | null,
...
function buildVoiceTemplateValues({
  ...
  baseValues?: Record | null;
  contactInfo?: any;
  callerId?: string | null;
  calledNumber?: string | null;
}): Record {
  const values: Record = { ...(baseValues ?? {}) };
...
export async function terminateSession(callId: string): Promise {
  const session = activeSessions.get(callId);
...
function startAudioHealthMonitor(session: OpenAIRealtimeSession): void {
  const logHealth = () => {
    ...
    if (
      ...
      && timeSinceUserSpeech  maxDurationSeconds) {
  ...
  - Stream ID: ${session.streamSid || 'NOT SET'}`);
    ...
    if (session.audioFrameCount > 10 && framesPerSecond  50) {
