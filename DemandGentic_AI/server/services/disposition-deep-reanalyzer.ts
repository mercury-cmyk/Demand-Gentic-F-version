...
function pruneDeepAnalysisCache(now: number = Date.now()): void {
  ...
  if (deepAnalysisCache.size  a[1].createdAt - b[1].createdAt);
  const toDelete = deepAnalysisCache.size - DEEP_ANALYSIS_CACHE_MAX_ENTRIES;
  for (let i = 0; i  stableJson(v)).join(",")}]`;
  }
...
export function runLightweightDispositionTriage(
  ...
  if (!transcriptStr || transcriptStr.trim().length  {
  ...
  if (!structuredTranscript || structuredTranscript.length  maxChars) {
  ...
  if (cached && now - cached.createdAt  typeof tp === "string" ? tp : tp.point || tp.content || JSON.stringify(tp)).join("\n  - ")
    : "";
...
- Short calls (,
  ...
  "callQuality": {
    "campaignAlignmentScore": ,
    "talkingPointsCoverage": ,
  ...
  try {
    ...
    const parsed = await deepAnalyze(fullPrompt, { temperature: 0.1, maxTokens: 5000, label: "disposition-reanalyzer", preferredProvider: "deepseek" });
...
export async function deepReanalyzeBatch(
  ...
  if (cursor) {
    ...
    conditions.push(
      sql`(${callSessions.startedAt}  0 ? and(...conditions) : isNotNull(callSessions.aiTranscript);
  ...
  if (filters.minTurns !== undefined || filters.maxTurns !== undefined) {
    sessions = sessions.filter((s) => {
      try {
        ...
        if (filters.minTurns !== undefined && turnCount  filters.maxTurns) return false;
        return true;
      } catch {
        return false;
      }
  ...
  const strengthsFreq = new Map();
  const weaknessesFreq = new Map();
  const missedTPFreq = new Map();
  ...
  while (chunkStart  {
    ...
    for (const result of chunkResults) {
      ...
      if (filters.confidenceThreshold &&
          deepResult.dispositionAssessment.confidence  0
        ? previewTurns.map(t => `${t.role === "agent" ? "Agent" : "Contact"}: ${t.text}`).join(" | ").slice(0, 400)
        : String(transcript || "").slice(0, 400);
    ...
    } else if (elapsedMs  b[1] - a[1]).slice(0, 5).map(([s]) => s),
...
export async function pushCallsToQA(
  ...
  for (const csId of callSessionIds) {
    try {
      ...
      if (durationSec  r.success).length,
...
export async function getContactsByDisposition(
  ...
  if (filters.minTurns || filters.maxTurns) {
    filteredSessions = filteredSessions.filter(s => {
      ...
      if (filters.minTurns && turnCount  filters.maxTurns) return false;
      return true;
    });
  }
...
export async function validateCallsForClientSamples(
  ...
  for (const csId of callSessionIds) {
    ...
    const durationSec = session.durationSec || 0;
    if (durationSec  ({ role: t.role || "unknown", text: t.message || t.text || "" }));
          ...
          if (turnCount  SCREENER_PATTERN.test(t.text))) {
