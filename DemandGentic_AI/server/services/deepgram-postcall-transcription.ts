...
async function getRefreshedUrlFromS3(recordingS3Key: string | null | undefined): Promise {
...
async function getRefreshedUrlFromTelnyx(telnyxCallId: string | null | undefined): Promise {
  if (!telnyxCallId) {
    return null;
  }
...
async function submitToDeepgram(audioUrl: string): Promise {
  if (!DEEPGRAM_API_KEY) {
  ...
  if (rawUtterances.length === 0) {
    for (let chIdx = 0; chIdx  (a.start || 0) - (b.start || 0));
  }
 ...
 */
export async function submitToDeepgramBuffer(audioBuffer: Buffer, mimetype?: string): Promise {
  if (!DEEPGRAM_API_KEY) {
