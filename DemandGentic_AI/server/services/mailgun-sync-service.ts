...
export class MailgunSyncService {
   ...
   */
  async getDomainFromMailgun(domain: string): Promise {
   ...
   */
  private async updateSenderProfilesForDomain(domainAuthId: number, isVerified: boolean): Promise {
    try {
   ...
   * Sync single domain
   */
  async syncDomain(domainId: number): Promise {
    if (!this.isConfigured()) {
      return false;
    }
