...
function identifyAgentSpeaker(utterances: StructuredTranscript["utterances"]): string {
  ...
  utterances.forEach((u, idx) => {
    ...
    if (idx === 0 && words.length  bestScore) {
      bestScore = score;
      bestSpeaker = speaker;
    }
  }
...
function normalizeTranscriptUtterances(
  ...
  if (utterances.length  setB.has(w)));
...
function computeTurnMetrics(turns: PrecisionTurn[]): TurnMetrics {
  ...
  for (let i = 0; i = 0 && gap  30s (likely pauses/holds)
...
async function evaluateCampaignOutcome(
  ...
  try {
    ...
    const qaParams = (campaign.qaParameters as Record | null) || {};
...
export async function runPostCallAnalysis(
  ...
  try {
    ...
    const existingAnalysis = session.aiAnalysis as Record | null;
    if (
    ...
    if (callDurationSec  upload still in progress
    ) {
    ...
    const transcriptLifecycle = ((session.aiAnalysis as Record | null)?.transcriptLifecycle || {}) as Record;
    const hasProvisionalTranscript =
      transcriptLifecycle.needsRecordingTranscription === true ||
      transcriptLifecycle.status === 'provisional';
    const existingStructuredTurns = Array.isArray((session.aiAnalysis as Record | null)?.postCallAnalysis?.turns)
      ? (session.aiAnalysis as Record).postCallAnalysis.turns
      : [];
    ...
    if (!structuredTranscript || structuredTranscript.text.length  ({
        role: t.speaker,
        text: t.text,
    ...
    if (campaignId) {
      try {
        ...
        if (campaign) {
          ...
          const aiSettings = campaign.aiAgentSettings as Record | null;
          campaignData = {
            name: campaign.name,
...
export function schedulePostCallAnalysis(
  ...
  const attemptAnalysis = async (attemptIndex: number) => {
    try {
      ...
      if (attemptIndex + 1  attemptAnalysis(attemptIndex + 1), retryDelays[attemptIndex + 1]);
      } else {
