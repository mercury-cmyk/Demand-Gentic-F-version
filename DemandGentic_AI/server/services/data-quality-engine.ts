...
const STANDARD_INDUSTRIES: Record = {
...
export const JOB_FUNCTION_TO_DEPARTMENT: Record = {
  'IT': 'IT / Engineering',
...
export async function getAudienceOverview() {
  ...
  const [contactStats] = await db.select({
    total: sql`count(*)::int`,
    withEmail: sql`count(case when ${contacts.email} is not null then 1 end)::int`,
    withTitle: sql`count(case when ${contacts.jobTitle} is not null then 1 end)::int`,
    withPhone: sql`count(case when ${contacts.directPhone} is not null or ${contacts.mobilePhone} is not null then 1 end)::int`,
    withAccount: sql`count(case when ${contacts.accountId} is not null then 1 end)::int`,
  }).from(contacts);

  // Account stats
  const [accountStats] = await db.select({
    total: sql`count(*)::int`,
    withIndustry: sql`count(case when ${accounts.industryStandardized} is not null then 1 end)::int`,
    withRevenue: sql`count(case when ${accounts.annualRevenue} is not null then 1 end)::int`,
    withEmployees: sql`count(case when ${accounts.staffCount} is not null or ${accounts.employeesSizeRange} is not null then 1 end)::int`,
    withDomain: sql`count(case when ${accounts.domain} is not null then 1 end)::int`,
  }).from(accounts);
