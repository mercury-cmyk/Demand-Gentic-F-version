...
export class RealtimeCallTelemetry {
  ...
  private readonly completionHashesByTurn = new Map>();
  ...
  emit(event: RealtimeTelemetryEvent, actor: TelemetryActor, payload: Record = {}, turnId?: number): void {
  ...
  isDuplicateCompletionForTurn(turnId: number, responseHash: string): boolean {
    const existing = this.completionHashesByTurn.get(turnId) ?? new Set();
