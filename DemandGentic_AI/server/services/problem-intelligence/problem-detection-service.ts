 ...
 */
export async function detectAccountSignals(accountId: string): Promise {
  // Get account data
 ...
 */
export async function loadProblemDefinitions(): Promise {
...
function parseTechStack(account: Account): {
  ...
  if (account.aiEnrichmentData && typeof account.aiEnrichmentData === "object") {
    const enrichment = account.aiEnrichmentData as Record;
...
function calculateEngagementLevel(
  ...
  if (daysSinceTouch  5) return "low";
  return "none";
}
  ...
  if (rules.minRevenue !== undefined || rules.maxRevenue !== undefined) {
    ...
    if (typeof firmographic.revenue === 'number') {
      ...
      const inRange =
        ...
        (rules.maxRevenue === undefined || revenue = rules.minEmployees) &&
        (rules.maxEmployees === undefined || employees  0 && firmographic.region) {
    weight += 0.05;
