...
function getProjectMetadataValue(project: any, key: string): string | undefined {
  const metadata = (project?.metadata || {}) as Record;
...
export async function generateLandingPage(params: LandingPageParams): Promise {
...
export async function generateEmailTemplate(params: EmailTemplateParams): Promise {
...
export async function generateBlogPost(params: BlogPostParams): Promise {
  assertDeepSeekConfigured();
  const orgIntel = await assertOrganizationIntelligence(params.organizationId);
  const startTime = Date.now();
  const brandContext = await getBrandContext(params.brandKitId);
...
export async function generateEbook(params: EbookParams): Promise {
  assertDeepSeekConfigured();
  const orgIntel = await assertOrganizationIntelligence(params.organizationId);
  const startTime = Date.now();
  const brandContext = await getBrandContext(params.brandKitId);
...
Output as JSON:
{
  ...
  const chaptersHtml = (result.chapters || [])
    .map((ch: any, i: number) => `Chapter ${i + 1}: ${ch.title}\n${ch.contentHtml || ch.content}`)
    .join('\n\n');
  const fullHtml = `${result.title}\n${result.subtitle || ''}\nExecutive Summary\n${result.executiveSummary || ''}\n${chaptersHtml}\nConclusion\n${result.conclusion || ''}`;
...
export async function generateSolutionBrief(params: SolutionBriefParams): Promise {
  assertDeepSeekConfigured();
  const orgIntel = await assertOrganizationIntelligence(params.organizationId);
  const startTime = Date.now();
  const brandContext = await getBrandContext(params.brandKitId);
