...
const DEFAULT_THRESHOLDS: AlertThresholds = {
  ...
  minHumanDetectionRate: 10, // Alert if  40% human detection (suspicious)
...
export function checkAlertThresholds(
  ...
  if (metrics.humanDetectionRate  thresholds.maxHumanDetectionRate) {
    alerts.push(
 ...
 */
export async function generateDailyReport(date: Date = new Date()): Promise {
 ...
 */
export async function detectPotentialFalsePositives(hours: number = 24): Promise {
...
export async function getVoicemailDetectionEfficiency(days: number = 7): Promise {
  ...
  const distribution = await db.execute(sql`
    SELECT
      CASE
        WHEN call_duration_seconds 90s'
      END as bucket,
      COUNT(*) as count
    FROM dialer_call_attempts
