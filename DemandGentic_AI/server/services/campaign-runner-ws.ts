...
class CampaignRunnerService {
  ...
  private runners: Map = new Map();
  private taskQueue: Map = new Map(); // campaignId -> tasks
  private processingInterval: NodeJS.Timeout | null = null;
  private campaignConfigs = new Map();
  ...
  private async resetStuckQueueItems(): Promise {
    try {
  ...
  private async handleMessage(ws: WebSocket, message: IncomingMessage): Promise {
    switch (message.type) {
      case 'register':
  ...
  private async handleRegister(ws: WebSocket, message: IncomingMessage): Promise {
  ...
  private async handleRequestTask(ws: WebSocket): Promise {
    const runner = this.runners.get(ws);
    if (!runner) return;
   ...
   */
  private async hydrateTaskCallerId(task: CampaignTask): Promise {
  ...
  private async handleTaskCompleted(ws: WebSocket, message: IncomingMessage): Promise {
    const runner = this.runners.get(ws);
  ...
  private async handleTaskFailed(ws: WebSocket, message: IncomingMessage): Promise {
    const runner = this.runners.get(ws);
    if (!runner || !message.taskId) return;
  ...
  private async handleStopCampaign(ws: WebSocket, message: IncomingMessage): Promise {
    const runner = this.runners.get(ws);
  ...
  private async loadCampaignTasks(campaignId: string): Promise {
  ...
  private async tryAssignTask(runner: RunnerClient): Promise {
  ...
  private async campaignHasMoreItems(campaignId: string): Promise {
  ...
  private async requeueTask(task: CampaignTask): Promise {
