    ...
    if (policyMatchesRequest(policy, entityType, action)) {
      ...
      if (scopeValid && evaluateConditions(policy.conditions, resourceMetadata)) {
        return { 
          ...
          matchedPolicy: policy.id,
          conditions: policy.conditions as Record
        };
      }
    }
  }
    ...
    if (grantActions.includes(action) || grantActions.includes('*')) {
      if (evaluateConditions(grant.conditions, resourceMetadata)) {
        return { 
          ...
          conditions: grant.conditions as Record
        };
      }
    }
  }
...
function evaluateConditions(conditions: any, resourceMetadata?: Record): boolean {
  ...
  for (const [key, expectedValue] of Object.entries(conditions)) {
    ...
    if (Array.isArray(expectedValue)) {
      ...
      const opValue = expectedValue as Record;
      ...
      if ('$lt' in opValue && typeof opValue.$lt === 'number' && !(actualValue ;
  fieldRules?: Record;
  effect?: 'allow' | 'deny';
  isSystem?: boolean;
  createdBy: string;
