...
export class CostTracker extends EventEmitter {
  ...
  private bigQueryInitPromise: Promise | null = null;
   ...
   */
  async getCurrentMonthCost(): Promise {
    try {
   ...
   */
  async getHistoricalCosts(): Promise> {
    try {
      await this.ensureBigQuery();
      const query = `
        SELECT 
   ...
   */
  async getCostByProject(): Promise> {
    try {
      await this.ensureBigQuery();
      const now = new Date();
      const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
