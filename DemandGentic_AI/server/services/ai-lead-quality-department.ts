...
function truncateTranscript(transcript: string): { text: string; truncated: boolean } {
  ...
  try {
    ...
    const qaParams = (campaign?.qaParameters as Record) || {};
    const clientCriteria = (qaParams.client_criteria as Record) || {};
  ...
  try {
    ...
    const result: LeadQualityDepartmentResult = {
      ...
      interestIdentification: {
        ...
        interestEvidence: ensureArray(interest.interestEvidence),
      },
      qualificationAnalysis: {
        ...
        qualificationCriteria: ensureArray(qualification.qualificationCriteria),
      },
      outcomeCategorization: {
      ...
      campaignAlignmentNotes: ensureArray(raw.campaignAlignmentNotes),
