 ...
 */
async function submitBatch(callIds: string[], attempt: number = 1): Promise {
  try {
    if (callIds.length === 0) {
...
async function processPendingJob(): Promise {
  try {
    ...
    const batch = await db.execute(
      sql`SELECT call_id
       FROM transcription_regeneration_jobs
       WHERE (status = 'pending' OR (status = 'in_progress' AND attempts  r.call_id);
  ...
  const workerLoop = async () => {
    while (isRunning) {
      try {
        ...
        while (activeJobsCount  setTimeout(resolve, 5000));
            continue;
          }
