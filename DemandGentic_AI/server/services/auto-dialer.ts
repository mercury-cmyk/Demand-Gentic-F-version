...
export class PowerDialerEngine {
  ...
  private pacingMetrics: Map = new Map(); // campaignId -> metrics
   ...
   */
  private async processQueue(campaignId: string): Promise {
    try {
   ...
   */
  private async getAvailableAgents(campaignId: string): Promise {
  ...
  private calculateDynamicDialRatio(campaignId: string, baseRatio: number): number {
    ...
    if (!metrics || metrics.callsAnswered + metrics.callsAbandoned  0.01) {
   ...
   */
  private async checkDnc(phone: string | null): Promise {
   ...
   */
  async handleCallAnswered(callAttemptId: string, telnyxCallId: string): Promise {
    try {
      const attempt = await storage.getCallAttempt(callAttemptId);
      if (!attempt) {
   ...
   */
  async handleCallEnded(callAttemptId: string, disposition?: string, duration?: number): Promise {
    try {
      const attempt = await storage.getCallAttempt(callAttemptId);
      if (!attempt) {
        console.error(`[AutoDialer] Call attempt not found: ${callAttemptId}`);
   ...
   */
  async completeAfterCallWork(agentId: string): Promise {
    try {
      await storage.updateAgentStatus(agentId, {
        status: 'available',
      });
