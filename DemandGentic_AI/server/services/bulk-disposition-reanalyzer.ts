 ...
 */
export async function analyzeSingleCall(callSessionId: string): Promise {
...
export async function getDispositionStats(
  ...
  const rows = await db
    .select({
      ...
      count: sql`count(*)::int`,
      avgDuration: sql`coalesce(avg(${callSessions.durationSec})::int, 0)`,
      withTranscript: sql`sum(case when ${callSessions.aiTranscript} is not null then 1 else 0 end)::int`,
      withRecording: sql`sum(case when ${callSessions.recordingUrl} is not null then 1 else 0 end)::int`,
    })
    .from(callSessions)
    .where(whereClause)
...
async function createLeadForReanalysis(
  ...
  if (durationSec  transcriptStr.includes(p))) {
