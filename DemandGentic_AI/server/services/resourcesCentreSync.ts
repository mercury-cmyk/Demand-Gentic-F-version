...
export class ResourcesCentreSync {
  ...
  private async fetchFromResourcesCentre(endpoint: string): Promise {
  ...
  async syncSpeakers(): Promise {
    this.validateConfig();
    const result = { created: 0, updated: 0, errors: 0 };

    try {
      const response = await this.fetchFromResourcesCentre('/api/v1/speakers');
  ...
  async syncOrganizers(): Promise {
    this.validateConfig();
    const result = { created: 0, updated: 0, errors: 0 };

    try {
      const response = await this.fetchFromResourcesCentre('/api/v1/organizers');
  ...
  async syncSponsors(): Promise {
    this.validateConfig();
    const result = { created: 0, updated: 0, errors: 0 };

    try {
      const response = await this.fetchFromResourcesCentre('/api/v1/sponsors');
