...
// Map — timestamps allow stuck-number detection
...
export class AllNumbersAtHourlyLimitError extends Error {
  public readonly filterStats: Record;
  public readonly totalNumbers: number;
  public readonly numbersAtHourlyCap: number;

  constructor(filterStats: Record, totalNumbers: number) {
 ...
 */
export function getNumberPoolStatus(): { inUse: number; numbers: Array } {
  const now = Date.now();
 ...
 */
async function resolveConnectionIdForEngine(engine?: 'texml' | 'sip'): Promise {
 ...
 */
async function getEligiblePool(request: NumberSelectionRequest): Promise {
  ...
  if (_poolCache && _poolCache.key === cacheKey && (Date.now() - _poolCache.cachedAt) ();
...
export async function recordCallOutcome(
  ...
  await db.insert(numberMetricsWindow).values({
    ...
    isShortCall: outcome.durationSec > 0 && outcome.durationSec  0 && outcome.durationSec < 3,
