 ...
 */
export async function deleteNumber(numberId: string): Promise {
...
export async function getCallStats(): Promise {
  ...
  const [aggregates] = await db
    .select({
      totalCallsToday: sql`COALESCE(SUM(${telnyxNumbers.callsToday}), 0)::int`,
      totalCallsThisHour: sql`COALESCE(SUM(${telnyxNumbers.callsThisHour}), 0)::int`,
      activeNumbersUsedToday: sql`COUNT(CASE WHEN ${telnyxNumbers.callsToday} > 0 THEN 1 END)::int`,
      totalActiveNumbers: sql`COUNT(CASE WHEN ${telnyxNumbers.status} = 'active' THEN 1 END)::int`,
    })
    .from(telnyxNumbers);
 ...
 */
export async function ensureReputationRecords(): Promise {
 ...
 */
async function initializeReputation(phoneNumberE164: string): Promise {
  const [number] = await db
