...
export class SmtpOAuthService {
   ...
   */
  async refreshGoogleToken(encryptedRefreshToken: string): Promise {
   ...
   */
  async getGoogleUserEmail(accessToken: string): Promise {
   ...
   */
  async exchangeMicrosoftCode(code: string, redirectUri: string): Promise {
    const params = new URLSearchParams({
      client_id: process.env.MICROSOFT_CLIENT_ID!,
   ...
   */
  async refreshMicrosoftToken(encryptedRefreshToken: string): Promise {
    const refreshToken = this.decryptToken(encryptedRefreshToken);
   ...
   */
  async getMicrosoftUserEmail(accessToken: string): Promise {
   ...
   */
  async getValidAccessToken(provider: SmtpProvider): Promise {
   ...
   */
  async createTransporter(provider: SmtpProvider): Promise {
   ...
   */
  async testConnection(provider: SmtpProvider): Promise {
    try {
  ...
  async sendTestEmail(
    ...
    try {
      ...
      const info = await transporter.sendMail({
        from: `${provider.displayName || "DemandGentic"} `,
        to: toEmail,
        ...
        html: `
            ...
            SMTP Provider Test
            This is a test email to verify your SMTP provider configuration.
            If you received this email, your SMTP provider is configured correctly!
   ...
   */
  async updateRateLimits(providerId: string): Promise {
    const now = new Date();
    ...
    const resetHourly = !provider.sentHourResetAt || new Date(provider.sentHourResetAt)  {
    const now = new Date();
    const hourStart = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours());
    const dayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
