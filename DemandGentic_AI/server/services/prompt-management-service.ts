 ...
 */
export async function getPromptByKey(promptKey: string): Promise {
  try {
    // Check cache first
 ...
 */
export async function invalidatePromptCache(promptKey: string): Promise {
  const redis = getRedisClient();
  if (redis) {
    try {
 ...
 */
export async function getPromptVersionHistory(promptId: string, limit: number = 50): Promise {
  try {
 ...
 */
export async function getPromptVersion(promptId: string, version: number): Promise {
  try {
