...
function matchesEmployeeRange(count: number | null, rule: EmployeeSizeRule): boolean {
  ...
  return count >= min && count  s + r.ai_priority_score, 0) / totalScored);
  ...
  const tiers = [
    ...
    { name: "Tier 4 ( {
    const tierRows = rows.filter(r => r.ai_priority_score >= tier.min && r.ai_priority_score  0 ? Math.round(tierRows.reduce((s, r) => s + r.ai_priority_score, 0) / tierRows.length) : 0,
  ...
  const scoreHistogram = buckets.map(bucket => ({
    ...
    count: rows.filter(r => r.ai_priority_score >= bucket && r.ai_priority_score  ({
    queueId: r.queue_id,
...
export async function getSegmentAnalysis(
  ...
  const tiers = tierDefs.map(td => {
    const tierRows = rows.filter(r => r.ai_priority_score >= td.min && r.ai_priority_score ();
...
export async function getContactScores(
  ...
  if (tier) {
    ...
    tierFilter = ` AND cq.ai_priority_score >= $${params.length + 1} AND cq.ai_priority_score  ({
      queueId: r.queue_id,
...
async function loadQueuedContacts(campaignId: string): Promise {
  const result = await pool.query(
    `
    SELECT
      cq.id AS queue_id,
 ...
 */
async function loadAgentQueueContacts(campaignId: string): Promise {
  const result = await pool.query(
    `
    SELECT
      aq.id AS queue_id,
...
function calculateAccountFitScore(
  ...
  if (targetSize.minEmployees || targetSize.maxEmployees) {
    ...
    if (staffCount) {
      ...
      if (staffCount >= min && staffCount = min && revenue  0) {
    factors++;
...
function extractScoringKeywords(parts: Array): string[] {
  const text = parts
    .filter(Boolean)
