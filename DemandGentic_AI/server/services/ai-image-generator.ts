...
class AIImageGenerator {
   ...
   */
  async generateImages(request: ImageGenerationRequest): Promise {
   ...
   */
  async listJobs(userId?: string, limit: number = 20): Promise {
  ...
  private async buildEnhancedPrompt(request: ImageGenerationRequest): Promise {
    const parts: string[] = [];
  ...
  private async createJob(jobId: string, request: ImageGenerationRequest): Promise {
  ...
  private async failJob(jobId: string, errorMessage: string, durationMs: number): Promise {
    await db
      .update(aiImageGenerationJobs)
      .set({
        status: 'failed',
