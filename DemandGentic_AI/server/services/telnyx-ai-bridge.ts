...
export class TelnyxAiBridge extends EventEmitter {
  ...
  private callStateByControlId: Map = new Map();
  ...
  private activePhoneNumbers: Set = new Set();
  ...
  private carrierFailureCounts: Map = new Map();
  ...
  private _cleanupInterval: ReturnType | null = null;
  ...
  private async telnyxFetch(url: string, init: RequestInit = {}, apiKeyOverride?: string): Promise {
  ...
  getMetrics(): CallMetrics {
    ...
    const fastCalls = this.callDurations.filter(d => d  0 
  ...
  private trackCallDuration(durationMs: number): void {
    ...
    if (metrics.totalCalls >= 10 && metrics.fastCallPercentage  {
    ...
    if (existing && (now - existing.lastAttempt) = this.MAX_CARRIER_RETRIES) {
  ...
  async initiateAiCall(
    ...
    try {
      ...
      // Use TeXML API for automatic streaming setup via 
  ...
  async handleWebhookEvent(event: TelnyxCallEvent): Promise {
  ...
  private async handleTranscriptionEvent(callId: string, call: ActiveAiCall, data: any): Promise {
  ...
  private async handleCallAnswered(callId: string, call: ActiveAiCall): Promise {
    console.log(`[TelnyxAiBridge] Call answered: ${callId}`);
    call.isAnswered = true;

    // With TeXML, streaming is handled automatically via 
  ...
  private async pollCallStatus(callId: string, callControlId: string, agent: AiVoiceAgent): Promise {
    let attempts = 0;
  ...
  private async speakText(callControlId: string, text: string, voice?: string): Promise {
    if (!text || !text.trim()) {
  ...
  private async speakWithGoogle(callControlId: string, text: string, voice: string): Promise {
  ...
  private async speakWithOpenAI(callControlId: string, text: string, voice: string, apiKey: string): Promise {
  ...
  private async speakWithTelnyxTTS(callControlId: string, text: string, voice?: string): Promise {
    // Map voice preference to gender (Telnyx basic TTS only supports male/female)
    const voiceGenderMap: Record = {
      alloy: "female",
      echo: "male", 
      fable: "male",
      onyx: "male",
  ...
  private async handleAmdResult(callId: string, call: ActiveAiCall, data: any): Promise {
  ...
  private async handleCallHangup(callId: string, call: ActiveAiCall): Promise {
  ...
  private async handleHandoff(callId: string, reason: string, transferNumber: string): Promise {
    const activeCall = this.activeCalls.get(callId);
  ...
  // startMediaStreaming() removed - TeXML handles streaming automatically via 

  private async hangupCall(callControlId: string): Promise {
  ...
  async notifyCallEndedByCallId(callId: string): Promise {
    const call = this.activeCalls.get(callId);
    if (call) {
  ...
  async processTranscribedSpeech(callId: string, text: string): Promise {
    const call = this.activeCalls.get(callId);
    if (!call) return null;
  ...
  // Gemini Live native audio via TeXML  → /voice-dialer WebSocket.
  // The 'answered' case here only applies to legacy Call Control API calls.
  async handleSimpleWebhookEvent(eventType: string, payload: any): Promise {
  ...
  private async startRecording(callControlId: string): Promise {
    const callId = this.getCallIdByControlId(callControlId);
  ...
  private async startGather(callControlId: string): Promise {
    const callId = this.getCallIdByControlId(callControlId);
