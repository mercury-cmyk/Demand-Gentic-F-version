...
export function mergeAgentSettings(raw?: Partial): VirtualAgentSettings {
  return {
    systemTools: {
 ...
 */
export async function getVirtualAgentConfig(virtualAgentId: string): Promise | null;
} | null> {
  ...
  try {
    ...
    return {
      ...
      settings: (agent.settings as Partial | null) ?? null,
    };
  } catch (error) {
