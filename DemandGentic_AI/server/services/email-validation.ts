...
function mergeOptions(options?: BusinessEmailValidationOptions): Required {
...
const LIKELY_DELIVERABLE_STATUSES = new Set([
  'acceptable',
  ...
  for (const contact of contactRecords) {
    ...
    try {
      ...
      for (const lead of contactLeads) {
        ...
        const currentQaData = (lead.qaData as Record) || {};
        const updatedQaData = {
          ...currentQaData,
          emailValidation: {
