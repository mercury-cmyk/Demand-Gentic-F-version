...
function toNgrams(tokens: string[], n: number): string[] {
  ...
  for (const [term, count] of termCounts.entries()) {
    if (count  0 ? Math.round((coverage / totalCalls) * 100) : 0,
    });
  }
...
function buildBucket(key: string, rows: PhraseInsightInputRow[], opts: Required): PhraseInsightBucket {
  const keywordCounts = new Map();
  const bigramCounts = new Map();
  const trigramCounts = new Map();

  const keywordCoverage = new Map>();
  const bigramCoverage = new Map>();
  const trigramCoverage = new Map>();
...
export function buildDispositionPhraseInsights(
  ...
  const byDispositionMap = new Map();
