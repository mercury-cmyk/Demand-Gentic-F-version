 ...
 */
export async function getCompanyProfile(companyNumber: string): Promise {
  if (!COMPANIES_HOUSE_API_KEY) {
    throw new Error('COMPANIES_HOUSE_API_KEY not configured');
  }
...
export async function batchValidateCompanies(
  companyNames: string[],
  delayMs: number = 1200 // Rate limit: 1.2s per lead = 50 leads/min × 2 API calls = 100 calls/min > {
