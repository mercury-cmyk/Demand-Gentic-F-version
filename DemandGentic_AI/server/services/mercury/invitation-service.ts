...
export class BulkInvitationService {
  ...
  private invitationSchemaInitPromise: Promise | null = null;
  ...
  private async ensureInvitationTokenSchema(): Promise {
  ...
  async getCandidates(): Promise {
    ...
    for (const user of users) {
      ...
      const inviteExpired = existingToken ? new Date(existingToken.expiresAt)  {
    ...
    const templatePreview = preview?.rendered || {
      ...
      html: 'Template "client_invite" needs to be created.',
    };
  ...
  async sendBulkInvitations(params: {
    ...
    for (let i = 0; i  setTimeout(resolve, MERCURY_DEFAULTS.batchDelayMs));
      }
    }
