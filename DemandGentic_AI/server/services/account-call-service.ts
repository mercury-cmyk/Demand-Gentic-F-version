...
export async function buildParticipantCallContext(contactId: string): Promise {
...
[Your Name]`;
  ...
  const html = `Hi ${params.recipient.name},
Thanks for the time today. ${brief.insight}
${notes ? `${notes}` : ""}
If it's useful, happy to share a short framework or clarify anything from the call.
Best,[Your Name]`;
...
function normalizeFollowupEmail(
  ...
  const html = typeof parsed?.html === "string" ? parsed.html : `Hi ${params.recipient.name},${brief.insight}Best,[Your Name]`;
