...
function extractTranscriptLifecycle(aiAnalysis: unknown): { needsRecordingTranscription: boolean; status?: string } {
  ...
  const lifecycle = (aiAnalysis as Record).transcriptLifecycle;
 ...
 */
export async function checkTranscriptStatus(callAttemptId: string): Promise {
  try {
    const [attempt] = await db
      .select({
        fullTranscript: dialerCallAttempts.fullTranscript,
...
export async function attemptFallbackTranscription(
  ...
  try {
    ...
    if (!urlToUse) {
      ...
      // Check if the call ended recently ( ({
      ...
      if (transcriptWithSummary.length > TRANSCRIPT_MIN_LENGTH) {
        ...
        if (attemptForSession?.callSessionId) {
          ...
          const existingAiAnalysis = (session?.aiAnalysis as Record | null) || {};
          const existingPostCall = (existingAiAnalysis.postCallAnalysis as Record | null) || {};
...
export async function processMissingTranscripts(): Promise {
  ...
  try {
    ...
    const uploadingSessionIds = new Set();
    ...
    for (const call of callsWithoutTranscripts) {
      ...
      if (hasReliableDuration && callDurationSec  120 ? 5 : callDurationSec > 30 ? 3 : 1;
  ...
  try {
    const results = await db
      .select({
        ...
        withTranscript: sql`count(*) filter (where full_transcript is not null or ai_transcript is not null)`,
        avgLength: sql`avg(coalesce(length(full_transcript), length(ai_transcript), 0))`,
      })
      .from(dialerCallAttempts)
 ...
 */
async function waitForRecordingUpload(callAttemptId: string, callDurationSec: number): Promise {
  if (callDurationSec  setTimeout(resolve, LONG_CALL_UPLOAD_POLL_MS));
    }
 ...
 */
export async function markForBackgroundTranscription(callAttemptId: string): Promise {
  try {
    await db.update(dialerCallAttempts)
      .set({
