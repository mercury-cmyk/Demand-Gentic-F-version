...
interface ProviderClient {
  name: string;
  generate: (systemPrompt: string, userPrompt: string, options: ProviderOptions) => Promise;
}
...
class GeminiProvider implements ProviderClient {
  ...
  async generate(systemPrompt: string, userPrompt: string, options: ProviderOptions): Promise {
...
export class UnifiedEmailRouter {
  ...
  async generateEmail(request: EmailGenerationRequest): Promise {
    ...
    for (let i = 0; i  0 && request.allowFallback === false) {
        break;
      }
   ...
   */
  private async buildSystemPrompt(request: EmailGenerationRequest, layersApplied: string[]): Promise {
...
`);
  ...
  private runComplianceChecks(email: {
    ...
    // Count images
    const imageMatches = content.match(/ 100000) {
   ...
   */
  private async checkCache(request: EmailGenerationRequest): Promise {
