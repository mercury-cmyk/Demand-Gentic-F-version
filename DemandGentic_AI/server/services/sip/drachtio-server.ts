...
class RTPPortManager {
  private usedPorts: Set = new Set();
  private portRange = { min: RTP_PORT_MIN, max: RTP_PORT_MAX };

  allocate(): number {
    for (let port = this.portRange.min; port  = new Map();
  ...
  update(callId: string, data: Partial): void {
...
export class DrachtioSIPServer {
  ...
  private async connectToDrachtio(): Promise {
   ...
   */
  private async setupMediaHandlers(call: DrachtioCall, rtpPort: number): Promise {
    ...
    try {
      ...
      const uacOptions = {
        headers: {
          ...
          'Contact': ``,
          'P-Asserted-Identity': ``,
          'Remote-Party-ID': `;party=calling;privacy=off;screen=yes`,
        },
