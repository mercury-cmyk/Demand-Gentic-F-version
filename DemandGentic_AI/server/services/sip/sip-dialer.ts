...
const sipHardStopTimers = new Map>();
 ...
 */
export async function initializeSipDialer(): Promise {
  if (!USE_SIP_CALLING) {
 ...
 */
export async function initiateAiCall(params: InitiateCallParams): Promise {
  const callId = uuidv4();
 ...
 * End an active call
 */
export async function endCall(callId: string, reason?: string): Promise {
