 ...
 */
export async function logSmiAudit(entry: SmiAuditEntry): Promise {
  try {
...
export async function getSmiAuditLog(filters: {
  ...
  return results.map(row => ({
    ...
    inputData: (row.inputData as Record) || undefined,
    outputData: (row.outputData as Record) || undefined,
...
export async function getAuditStatistics(
  ...
  const byTypeResult = await db.execute(sql`
    ...
    FROM smi_audit_log
    WHERE created_at >= ${startDate} AND created_at `AVG(processing_time_ms)`,
  ...
  const topEntitiesResult = await db.execute(sql`
    ...
    FROM smi_audit_log
    WHERE created_at >= ${startDate} AND created_at  = {};
...
export async function purgeOldAuditLogs(
  ...
  const result = await db.execute(sql`
    ...
    WHERE created_at  {
  async logAudit(entry: SmiAuditEntry): Promise {
  ...
  createContext(baseContext: Partial) {
