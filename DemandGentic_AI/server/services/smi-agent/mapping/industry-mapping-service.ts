 ...
 */
async function lookupBySicCode(sicCode: string): Promise {
 ...
 */
async function lookupByNaicsCode(naicsCode: string): Promise {
  const result = await db
    .select()
    .from(industryTaxonomy)
    .where(
 ...
 */
async function tryExactIndustryMatch(rawIndustry: string): Promise {
 ...
 */
async function tryFuzzyIndustryMatch(rawIndustry: string): Promise {
 ...
 */
async function classifyIndustryWithAI(rawIndustry: string): Promise {
  try {
 ...
 */
export async function getIndustryIntelligence(industryId: number): Promise {
  const result = await db
    .select()
    .from(industryTaxonomy)
  ...
  return {
    ...
    industryCode: industry.industryCode,
    typicalChallenges: parseJsonArray(industry.typicalChallenges),
    regulatoryConsiderations: parseJsonArray(industry.regulatoryConsiderations),
    buyingBehaviors: parseJsonObject(industry.buyingBehaviors) || {
    ...
    seasonalPatterns: parseJsonObject(industry.seasonalPatterns) || {
      peakBuyingSeason: [],
      budgetPlanningPeriod: 'Q4',
      avoidPeriods: [],
      fiscalYearEnd: 'December',
    },
    technologyTrends: parseJsonArray(industry.technologyTrends),
    competitiveLandscape: parseJsonObject(industry.competitiveLandscape) || {
...
export async function getIndustryDepartmentPainPoints(
  ...
  const data = result[0];
  return parseJsonArray(data.painPoints);
}
 ...
 */
function parseJsonArray(json: unknown): T[] {
  if (!json) return [];
 ...
 */
function parseJsonObject(json: unknown): T | null {
  if (!json) return null;
...
export class IndustryMappingService implements IIndustryMappingService {
  ...
  async getIndustryIntelligence(industryId: number): Promise {
