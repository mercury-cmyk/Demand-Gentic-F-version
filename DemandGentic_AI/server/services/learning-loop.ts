 ...
 */
export async function recordCallOutcome(record: CallOutcomeRecord): Promise {
  try {
 ...
 */
async function processSMILearning(record: CallOutcomeRecord): Promise {
  try {
...
export function extractSuccessfulBehaviors(signals: CallOutcomeSignals): string[] {
  ...
  if (signals.time_pressure === "none" && signals.response_time_ms && signals.response_time_ms  5000) {
