...
Return your response as valid JSON with this structure:
{
  ...
  try {
    ...
    if (input.useReasoningModel) {
      ...
      const response = await generateJSON(`${systemPrompt}\n\n---\n\n${context}`, {
        temperature: 0.3,
        maxTokens: 8192,
      });
...
export async function determineDisposition(
  ...
  const isIncompleteTranscript = transcriptLength (prompt, { temperature: 0.1 });
  } catch (error) {
...
export async function summarizeCall(
  ...
  const dispositionOutcomes: Record = {
...
Return JSON:
{
  ...
  try {
    return await generateJSON(prompt, { temperature: 0.5 });
  } catch (error) {
