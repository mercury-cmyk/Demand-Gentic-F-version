...
export class VertexClientAgenticHub extends EventEmitter {
   ...
   */
  async createCampaignOrder(request: CampaignOrderRequest): Promise> {
...
Return JSON:
{
  ...
  "optimizedCriteria": { ... any suggested improvements }
      ...
      let analysis = await generateJSON(enrichedAnalysisPrompt, { temperature: 0.2 });
   ...
   */
  async runVoiceSimulation(request: VoiceSimulationRequest): Promise> {
...
Return JSON:
{
  ...
  "coachingPoints": ["point 1", "point 2"]
      ...
      let simulation = await generateJSON(enrichedScenarioPrompt, { temperature: 0.7 });
...
Return JSON:
{
  ...
  "detailedFeedback": "paragraph of coaching feedback"
      ...
      let analysis = await generateJSON(enrichedSimAnalysisPrompt, { temperature: 0.3 });
...
Return JSON:
{
  ...
  "scenarioSetup": {
    ...
    "buyingSignals": ["signal 1", "signal 2"]
      ...
      let persona = await generateJSON(enrichedPersonaPrompt, { temperature: 0.6 });
   ...
   */
  async generateEmails(request: EmailGenerationRequest): Promise> {
...
Return JSON:
{
  ...
  "copywritingNotes": "explanation of approach and A/B testing recommendations"
      ...
      const rawResult = await generateJSON(enrichedEmailPrompt, { temperature: 0.7 });
...
Return JSON:
{
  ...
  "emails": [
    {
      ...
      const rawResult = await generateJSON(enrichedSequencePrompt, { temperature: 0.6 });
   ...
   */
  async generateMarketingImages(request: ImageGenerationRequest): Promise> {
...
Return JSON:
{
  ...
  "technicalSpecs": "technical details for the image"
      ...
      const rawResult = await generateJSON(enrichedImagePrompt, { temperature: 0.4 });
