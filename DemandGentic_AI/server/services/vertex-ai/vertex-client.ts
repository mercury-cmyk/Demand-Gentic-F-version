...
let _vertexActive = 0;
const _vertexWaitQueue: Array void; reject: (err: Error) => void; timer: ReturnType }> = [];
...
function _acquireVertexSlot(): Promise {
  ...
  // Prune old entries
  while (_rpmWindow.length > 0 && _rpmWindow[0] = VERTEX_MAX_RPM) {
  ...
  // Check concurrency
  if (_vertexActive ((resolve, reject) => {
...
async function withVertexThrottle(fn: () => Promise, label?: string): Promise {
 ...
 */
export function getVertexAI(config?: Partial): VertexAI {
  if (config) {
 ...
 */
export async function generateImage(prompt: string, aspectRatio: string = "16:9"): Promise {
 ...
 */
async function withRetry(fn: () => Promise, maxRetries = 2): Promise {
  for (let attempt = 0; attempt  0 ? `retry-${attempt}` : undefined);
    } catch (error: any) {
...
export async function reason(
  ...
  if (cooldownRemainingMs > 0) {
    ...
    const thinkingMatch = fullText.match(/([\s\S]*?)/);
    const thinking = thinkingMatch ? thinkingMatch[1].trim() : "";
    const answer = fullText.replace(/[\s\S]*?/g, "").trim();
  ...
  // Fallback: if no thought parts, try legacy  tag parsing
  if (!thinking && answer) {
    const thinkingMatch = answer.match(/([\s\S]*?)/);
    if (thinkingMatch) {
 ...
 */
export async function deepAnalyzeJSON(
  prompt: string,
  options: GenerationOptions = {}
): Promise {
  ...
  if (cooldownRemainingMs > 0) {
    ...
    return generateJSON(prompt, { ...options, responseFormat: "json" });
  }
