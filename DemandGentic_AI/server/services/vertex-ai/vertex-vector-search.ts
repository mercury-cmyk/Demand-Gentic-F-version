...
const normalizeMetadata = (metadata: Record | null | undefined) =>
  metadata ?? {};
...
const upsertDocuments = async (docs: VectorDocument[]): Promise => {
  if (docs.length === 0) {
    return 0;
  }

  let total = 0;
  for (let i = 0; i  doc.content);
...
const searchDocuments = async (
  ...
  const rows = await db
    .select({
      ...
      score: sql`1 - (${vectorDocuments.embedding}  ${vectorParam})`,
    })
  ...
  return rows.map((row, index) => ({
    document: {
      ...
      content: row.content,
      metadata: normalizeMetadata(row.metadata as Record | null),
 ...
 */
export async function indexAccounts(accountIds?: string[]): Promise {
 ...
 */
export async function indexContacts(contactIds?: string[]): Promise {
 ...
 */
export async function indexCallTranscripts(limit: number = 100, offset: number = 0): Promise {
 ...
 */
export async function initializeMatchingEngine(config: MatchingEngineConfig): Promise {
