...
function getRoutingWeight(provider: ProviderRow): number {
  const raw = Number((provider.providerMetadata as Record | null)?.routingWeight ?? 1);
  if (!Number.isFinite(raw) || raw  getRoutingWeight(provider));
  const totalWeight = weights.reduce((sum, weight) => sum + weight, 0);

  if (totalWeight  {
  const now = Date.now();
  if (providerCache && now - providerCache.loadedAt ;
