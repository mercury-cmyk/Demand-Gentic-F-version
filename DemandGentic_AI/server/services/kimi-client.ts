 ...
 */
export async function kimiGenerateJSON(
  prompt: string,
  options: KimiChatOptions = {}
): Promise {
...
RESPONSE FORMAT:
Start with a  section where you reason through the problem step by step, consider multiple angles, evaluate evidence, and form your analysis. Then provide your final answer after .
...
${context ? `\nCONTEXT PROVIDED:\n${context}` : ""}`;
  ...
  const thinkingMatch = fullResponse.match(/([\s\S]*?)/);
  const thinking = thinkingMatch ? thinkingMatch[1].trim() : "";
  const answer = thinkingMatch
    ? fullResponse.replace(/[\s\S]*?/, "").trim()
    : fullResponse;
