...
export interface PersonalizedEmail {
  ...
  htmlContent: string;
  textContent: string;
  personalizationVariables: Record;
...
const RESOURCE_FORMAT_MAP: Record = {
  ebook: "ebook",
...
async function resolveCampaignContentEmailContext(
  ...
  const metadata = (link.metadata || {}) as Record;
...
async function buildParticipantContext(data: any): Promise {
 ...
 */
async function getEngagementSignals(contactId: string): Promise {
  try {
 ...
 */
export async function shouldContinueSequence(contactId: string): Promise {
