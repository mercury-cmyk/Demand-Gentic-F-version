...
const GENERIC_TONE_GUIDELINES: Record = {
 ...
 */
export async function getFullOrganizationIntelligence(organizationId?: string): Promise {
 ...
 */
export async function assertOrganizationIntelligence(organizationId?: string): Promise {
...
export async function getBrandContext(brandKitId?: string): Promise {
 ...
 */
export async function getFeatureRegistryContext(organizationId?: string): Promise {
...
- Do NOT include any JSON-LD, schema.org structured data, or `);
    } else {
      $.root().append(``);
    }
...
function enforceBrandColors(html: string, orgIntel: OrgIntelligenceContext): string {
  ...
  try {
    ...
    if (colorVars.length > 0) {
      const styleTag = `:root { ${colorVars.join(' ')} }`;
      if ($('head').length) {
...
Make the HTML fully self-contained with inline styles and a  block for responsive breakpoints. Use modern, clean design. Mobile-responsive.`;
