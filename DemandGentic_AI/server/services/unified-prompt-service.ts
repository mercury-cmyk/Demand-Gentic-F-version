...
class UnifiedPromptService {
   ...
   */
  async setLocked(id: string, isLocked: boolean, userId?: string): Promise {
   ...
   */
  async deactivate(id: string, userId?: string): Promise {
    const current = await this.getById(id);
   ...
   */
  async restoreVersion(id: string, version: number, userId?: string): Promise {
  ...
  async getPromptAnalytics(promptId: string, options: {
    ...
    for (let i = 0; i  t.testedAt.toISOString().split('T')[0] === dateStr);
   ...
   */
  async generateLearnings(promptId: string): Promise {
    const prompt = await this.getById(promptId);
    if (!prompt) return null;

    const analytics = await this.getPromptAnalytics(promptId);
    if (!analytics || analytics.totalCalls = 0.7) {
    ...
    // Analyze engagement
    if (analytics.avgEngagementScore  b - a)[0];
   ...
   */
  async revertToDefault(id: string, userId?: string): Promise {
   ...
   */
  async getDriftedPrompts(): Promise {
   ...
   */
  async getDepartmentCounts(): Promise> {
   ...
   */
  async getFunctionCounts(): Promise> {
    const rows = await db.select({
   ...
   */
  async getDependencies(promptId: string): Promise {
    return db.select()
   ...
   */
  async setOwnership(promptId: string, ownerId: string, ownerDepartment?: string): Promise {
    await db.update(promptRegistry)
      .set({
        ownerId,
   ...
   */
  async setStatus(promptId: string, status: 'draft' | 'live' | 'archived' | 'deprecated', userId?: string): Promise {
    await db.update(promptRegistry)
      .set({
        status,
