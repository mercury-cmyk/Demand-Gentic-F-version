 ...
 */
export async function loadSecretsToEnv(options: LoadSecretsOptions = {}): Promise {
  ...
  try {
    ...
    for (const secret of needsDecryption) {
      try {
        const value = decryptJson(secret.encryptedValue, masterKey);
...
export async function getSecret(
  ...
  for (const env of envsToTry) {
    try {
      ...
      if (result.length > 0) {
        const value = decryptJson(result[0].encryptedValue, ensureMasterKey());
        const stringValue = typeof value === 'string' ? value : JSON.stringify(value);
 ...
 */
export async function hasSecrets(service?: string): Promise {
