...
const pendingCallState = new Map();
const CLEANUP_INTERVAL_MS = 60 * 1000;

let cleanupInterval: ReturnType | null = null;
let redisClient: Redis | null = null;
let redisAvailable = false;
let redisInitPromise: Promise | null = null;

async function initializeRedis(): Promise {
...
function storeInMemory(callId: string, context: Record): void {
...
async function removeFromRedis(callId: string): Promise {
...
async function readFromRedis(callId: string, consume: boolean): Promise | null> {
 ...
 */
export async function hasPendingCallState(callId: string): Promise {
