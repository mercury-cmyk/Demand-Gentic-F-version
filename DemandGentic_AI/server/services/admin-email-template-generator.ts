...
export interface AdminEmailTemplateGenerationInput {
  ...
  organizationIntelligence?: Record | null;
...
interface GeneratorDependencies {
  generateWithCoreEmailAgent: (args: {
    ...
    roleRelevantBrief: ReturnType;
    ctaUrlWithUtm: string;
    attempt: number;
    forceInternalLanguageRewrite: boolean;
  }) => Promise;
  loadEventContextFromDb: (input: AdminEmailTemplateGenerationInput) => Promise;
  fetchArgylePageHtml: (url: string) => Promise;
  loadCachedEventBrief: (campaignId: string, sourceUrl: string) => Promise;
  saveCachedEventBrief: (campaignId: string, sourceUrl: string, eventBrief: EventContext) => Promise;
}

function logEmailTemplateDebug(event: string, payload: Record = {}): void {
  console.info(`${EMAIL_TEMPLATE_DEBUG_PREFIX} ${event}`, payload);
}

async function loadEventContextFromDb(input: AdminEmailTemplateGenerationInput): Promise {
...
async function fetchArgylePageHtml(url: string): Promise {
...
async function loadCachedEventBrief(campaignId: string, sourceUrl: string): Promise {
  const [campaignRow] = await db
  ...
  const refs = campaignRow?.audienceRefs as Record | null | undefined;
...
async function saveCachedEventBrief(campaignId: string, sourceUrl: string, eventBrief: EventContext): Promise {
  const [campaignRow] = await db
    .select({ audienceRefs: campaigns.audienceRefs })
    .from(campaigns)
    .where(eq(campaigns.id, campaignId))
...
function firstNonEmptyString(...values: Array): string {
...
function clampLength(value: string, maxChars: number): string {
  ...
  if (normalized.length  20 ? sliced.slice(0, lastSpace) : sliced.slice(0, maxChars);
  return bounded.trim();
}
...
function isLikelyBoilerplate(value: string): boolean {
  ...
  return (
    normalized.length  190) return true;
...
function pickHeadingSectionItems($: ReturnType, headingPattern: RegExp, maxItems = 5): string[] {
...
function findFirstDateTimeCandidate($: ReturnType): string {
  ...
  const candidates = $('main p, article p, section p, main li, article li, section li, div, span')
    ...
    .get()
    .filter((text) => text.length >= 10 && text.length ): string {
...
function deriveTakeawaysFromOverview(overview: string, maxItems = 3): string[] {
  const sentences = overview
    .split(/(? normalizeTakeaway(sentence))
    .filter((sentence) => !isWeakTakeaway(sentence));

  return uniqueItems(sentences, maxItems);
}

function extractSpeakersFromText($: ReturnType, maxItems = 3): string[] {
  ...
  for (const line of lineCandidates) {
    ...
    if (!stripped || stripped.length = 3 && candidate.length  normalizeTakeaway(item))
...
type StructuredTemplateContext = ReturnType;
...
function sentenceToTakeaway(value: string): string {
  ...
  if (cleaned.length  sentenceToTakeaway(item))
    .filter((item) => !isWeakTakeaway(item));
...
function buildDefaultSubject(context: StructuredTemplateContext): string {
  ...
  if (shortDate && compactBenefit.length + shortDate.length + 3 
): {
  subject: string;
  preheader: string;
  bodyText: string;
...
export async function generateAdminEmailTemplateFromPromptSystem(
  ...
  for (let attempt = 0; attempt  firstNonEmptyString(typeof value === 'string' ? value : ''))
        .filter(Boolean)
