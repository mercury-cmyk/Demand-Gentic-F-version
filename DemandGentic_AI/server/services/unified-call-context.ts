...
export async function resolveAgentConfig(
  ...
  const mergedSettings = mergeAgentSettings(
    assignment.settings as Partial | undefined
  );
 ...
 */
export async function resolveAgentAssignment(campaignId: string): Promise {
  // Fetch campaign
  ...
  const aiSettings = (campaign as any).aiAgentSettings as Record | null;
  if (aiSettings) {
...
`);
  ...
  if (dbAssignment) {
    return {
      ...
      settings: dbAssignment.settings as Record | null,
    };
  }
 ...
 */
export async function buildUnifiedCallContext(input: BuildCallContextInput): Promise {
 ...
 */
export function contextToClientStateParams(ctx: UnifiedCallContext): Record {
  return {
    // Call identifiers
    call_id: ctx.callId,
    run_id: ctx.runId,
 ...
 */
export async function storeCallSession(ctx: UnifiedCallContext): Promise {
