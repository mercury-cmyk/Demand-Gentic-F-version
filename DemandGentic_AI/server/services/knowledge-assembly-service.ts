...
export async function assembleAgentDefaultKnowledge(
  ...
  let agentOverrides: Map = new Map();
 ...
 */
export async function getUniversalKnowledgeForDialer(): Promise {
  try {
 ...
 */
export async function getVoiceControlKnowledgeForDialer(): Promise {
  try {
    const blocks = await getKnowledgeBlocks({
