...
function randomDelay(minMs: number, maxMs: number): Promise {
 ...
 */
export async function processProxySubmissionJob(jobId: string): Promise {
  // Load job
  ...
  for (const item of items) {
    ...
    const payload = item.formPayload as Record;
    const submitPayload: Record = {
    ...
    const result = await submitSingleForm(
      job.pageSlug,
      submitPayload,
      (job.utmDefaults as Record) || undefined,
    );
    ...
    if (items.indexOf(item)  0 ? `${failed} of ${items.length} submissions failed` : null,
    })
    .where(eq(proxyFormSubmissionJobs.id, jobId));
 ...
 */
export async function cancelProxySubmissionJob(jobId: string): Promise {
