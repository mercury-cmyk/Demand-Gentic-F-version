...
export function decryptServiceAccount(encrypted: string): string {
  ...
  const value = decryptJson(encrypted, MASTER_KEY);
 ...
 */
export async function generateMigrationChecklist(account: GoogleCloudAccount): Promise {
...
export async function checkAccountHealth(account: GoogleCloudAccount): Promise {
...
export async function applyAccount(account: GoogleCloudAccount): Promise {
...
// ---------------------------------------------------------------------------
export async function loadActiveAccountOnStartup(): Promise {
  try {
...
export async function getAccountById(id: string): Promise | null> {
  const [row] = await db
    .select()
    .from(googleCloudAccounts)
