...
export class CloudLoggingService {
   ...
   */
  async getLogMetrics(hours: number = 24): Promise {
   ...
   */
  async searchLogs(query: string, hours: number = 24, limit: number = 100): Promise {
    const startTime = new Date(Date.now() - hours * 60 * 60 * 1000);
   ...
   */
  async getRecentLogs(minutes: number = 5, limit: number = 50): Promise {
