...
const inMemoryCallSessions = new Map();
 ...
 */
export async function initializeCallSessionStore(): Promise {
 ...
 */
export async function setCallSession(session: CallSession): Promise {
 ...
 */
export async function getCallSession(callId: string): Promise {
  if (isRedisAvailable() && redisClient) {
 ...
 */
export async function getCallSessionByControlId(callControlId: string): Promise {
  if (isRedisAvailable() && redisClient) {
 ...
 */
export async function deleteCallSession(callId: string): Promise {
  const session = await getCallSession(callId);
 ...
 */
export async function getActiveCallSessions(): Promise {
  if (isRedisAvailable() && redisClient) {
 ...
 */
export async function setPreviewSession(session: PreviewSession): Promise {
  session.updatedAt = new Date().toISOString();
 ...
 */
export async function getPreviewSession(sessionId: string): Promise {
  if (isRedisAvailable() && redisClient) {
 ...
 */
export async function deletePreviewSession(sessionId: string): Promise {
  if (isRedisAvailable() && redisClient) {
...
export async function validateCallControlId(callControlId: string): Promise {
  ...
  if (session.status === 'active' && lastUpdate  {
...
const inMemoryCallParams = new Map();

/**
 * Store call custom params (for large data like system_prompt)
 */
export async function setCallParams(callId: string, params: CallCustomParams): Promise {
 ...
 */
export async function getCallParams(callId: string): Promise {
  const key = `${CALL_PARAMS_PREFIX}${callId}`;
 ...
 */
export async function deleteCallParams(callId: string): Promise {
  const key = `${CALL_PARAMS_PREFIX}${callId}`;
