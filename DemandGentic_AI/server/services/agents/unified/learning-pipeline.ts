...
export class LearningPipelineService {
  ...
  private pipelineStates: Map = new Map();

  /** All generated recommendations, indexed by agent type */
  private recommendations: Map = new Map();

  /** Analysis history */
  private analysisHistory: Map = new Map();
  ...
  async generateRecommendations(
    ...
    capabilities: AgentCapability[],
    mappings: CapabilityPromptMapping[],
    currentPromptSections: Record // sectionId -> content
  ): Promise {
    const state = this.pipelineStates.get(agentType);
  ...
  private async createRecommendation(
    ...
    const severityScores: Record = { critical: 90, warning: 60, info: 30 };
    const typeBonus: Record = { degradation: 20, anomaly: 15, pattern: 10, opportunity: 5, insight: 0 };
...
5. Output ONLY the rewritten prompt section content. Do not include any other text, preamble, or explanation.
   ...
   */
  private async invokeGenerativeAI(prompt: string): Promise {
    try {
      const { analyzeJSON } = await import('../../ai-analysis-router');
      const result = await analyzeJSON(
        prompt + '\n\nRespond with a JSON object: { "content": "" }. Output ONLY valid JSON.',
  ...
  private schedulerInterval: ReturnType | null = null;
  ...
  async recordCompletedCall(data: {
    ...
    const collectorData: { sourceType: LearningSourceType; metrics: Record; insights: string[] }[] = [];
   ...
   */
  async collectAndAnalyzeVoiceData(): Promise {
    ...
    for (const r of records) {
      if (Array.isArray(r.issues)) {
        for (const issue of r.issues.slice(0, 2)) {
          if (typeof issue === 'object' && issue?.description && insights.length  i.toLowerCase().includes('objection')).slice(0, 2),
        sampleSize,
        timeRange,
      },
      {
   ...
   */
  private async triggerAutoAnalysis(): Promise {
