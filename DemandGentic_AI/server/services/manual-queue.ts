...
export class ManualQueueService {
  ...
  async addContactsToAgentQueue(
    ...
    try {
      ...
      const directPhoneUpdates: Array = [];
      const mobilePhoneUpdates: Array = [];
   ...
   */
  async pullNextContact(agentId: string, campaignId: string): Promise {
    try {
      ...
      const result = await db.transaction(async (tx) => {
        ...
        const selectResult = await tx.execute(sql`
          ...
          WHERE aq.agent_id = ${agentId}
            ...
            AND (aq.scheduled_for IS NULL OR aq.scheduled_for  {
    await db
      .update(agentQueue)
      .set({
   ...
   */
  async markCompleted(queueItemId: string, agentId: string): Promise {
    await db
      .update(agentQueue)
      .set({
   ...
   */
  async releaseLock(queueItemId: string, agentId: string): Promise {
    await db
      .update(agentQueue)
      .set({
        queueState: 'queued',
   ...
   */
  async boostPriority(queueItemId: string): Promise {
    await db
      .update(agentQueue)
      .set({
   ...
   */
  private async releaseStaleLocksForAgent(agentId: string, campaignId: string): Promise {
   ...
   */
  async clearCompletedItems(agentId: string, campaignId: string): Promise {
