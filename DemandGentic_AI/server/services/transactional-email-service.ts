...
export class TransactionalEmailService {
  private async sendPasswordResetFallbackSmtp(params: {
    ...
    try {
      ...
      const html = `
          ...
          This link expires in ${params.expiresIn}.
          If you did not request this, you can safely ignore this email.
          Or copy and paste this URL into your browser:${params.resetLink}
   ...
   */
  async sendTransactionalEmail(request: TransactionalEmailRequest): Promise {
    try {
   ...
   */
  async sendImmediate(request: TransactionalEmailRequest): Promise {
    try {
      const template = await this.getTemplate(request.eventType, request.templateId);
      if (!template) {
   ...
   */
  async triggerWelcomeEmail(user: { id: string; email: string; firstName?: string }): Promise {
   ...
   */
  async getTemplatesForEventType(eventType: TransactionalEventType): Promise {
    return db
      .select()
      .from(transactionalEmailTemplates)
