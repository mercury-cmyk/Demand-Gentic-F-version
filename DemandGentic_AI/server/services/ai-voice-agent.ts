...
async function generateGeminiText(prompt: string, maxTokens: number): Promise {
  try {
...
export class AiVoiceAgent extends EventEmitter {
  ...
  private systemPromptPromise: Promise | null = null;
...
- Log any key information the prospect shares for follow-up`;
  ...
  async processIncomingAudio(audioBuffer: Buffer): Promise {
    return null;
  }

  async generateResponse(humanText: string): Promise {
  ...
  private getSystemPromptCached(): Promise {
  ...
  private emitVoiceTurnTelemetry(event: string, payload: Record): void {
  ...
  async endConversation(disposition: string): Promise {
