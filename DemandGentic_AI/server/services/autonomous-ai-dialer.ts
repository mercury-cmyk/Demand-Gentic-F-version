...
class AutonomousAIDialerService {
  ...
  private campaignActiveCounts = new Map(); // campaignId → active count
  private campaignConfigs = new Map(); // campaignId → config
  private consecutiveErrors = new Map();  // campaignId → error count
  ...
  // Main polling loop
  // ──────────────────────────────────────────────────────────────────────────
  private async pollAndFill(): Promise {
    ...
    try {
      ...
      for (const campaign of activeCampaigns) {
        ...
        if (slotsAvailable = sessionStats.max) {
  ...
  // ──────────────────────────────────────────────────────────────────────────
  private async fillSlots(campaignId: string, slotsToFill: number, defaultFromNumber: string): Promise {
    ...
    for (const item of queueItems) {
      ...
      try {
        ...
        if (filled  setTimeout(resolve, 500));
        }
      } catch (err) {
  ...
  private async pullEligibleQueueItems(campaignId: string) {
    ...
    const skippedItemsByNextAttempt = new Map();
  ...
  private async tryFillSlotsForCampaign(campaignId: string): Promise {
    ...
    if (slotsAvailable  MAX_CALL_AGE_MS) {
