...
export class KimiVoiceProvider extends BaseVoiceProvider {
  ...
  private sttStream: ReturnType | null = null;
  ...
  private silenceTimer: ReturnType | null = null;
  ...
  private sttRestartTimer: ReturnType | null = null;
  ...
  private async processUserUtterance(text: string): Promise {
    if (!text.trim()) return;

    // Rate limit
    const now = Date.now();
    if (now - this.lastResponseTime  0) {
  ...
  private async generateKimiResponse(): Promise {
...
Respond with JSON only.`;
  ...
  private async textToSpeechAndSend(text: string, responseId: string): Promise {
    ...
    try {
      ...
      for (const sentence of sentences) {
        ...
        for (let offset = 0; offset  s.trim()).filter((s) => s.length > 0);
  }

  async generateAndSpeakResponse(): Promise {
    const response = await this.generateKimiResponse();
    this.conversationHistory.push({ role: "assistant", content: response });
