...
function initializeTables(): void {
  ...
  for (let i = 0; i > 4) & 0x07;
    const mantissa = ulawByte & 0x0f;
    const magnitude = ((mantissa > 4) & 0x07;
    const mantissa = alawByte & 0x0f;

    let magnitude: number;
    if (exponent === 0) {
      magnitude = (mantissa > 8) & 0x80;
  ...
  const ulawByte = ~(sign | (exponent > 8) & 0x80;
  if (sign) sample = -sample;
  ...
  if (sample >= 256) {
    ...
    exponent = 1;
    while (tempSample >= 512 && exponent >= 1;
      exponent++;
    }
  ...
  const alawByte = (sign | (exponent  ALWAYS defaults to PCMU (µ-law)
...
export function g711ToPcm8k(g711Buffer: Buffer, format: G711Format): Buffer {
  ...
  for (let i = 0; i  peakSample) peakSample = Math.abs(sample);
  }
  ...
  for (let i = 0; i  peak) peak = sample;
  }
  ...
  for (let i = 0; i = 0 && srcIdx = historyBytes) {
...
function applyEarlyNoiseSupression(pcmBuffer: Buffer): Buffer {
  ...
  let sumSquared = 0;
  let peak = 0;
  for (let i = 0; i  peak) peak = Math.abs(sample);
  }
  const rms = Math.sqrt(sumSquared / samples);
  ...
  for (let i = 0; i  Telnyx audio path
 *
