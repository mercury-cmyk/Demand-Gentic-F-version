...
export class GeminiLiveProvider extends BaseVoiceProvider {
  ...
  private connectionTimeout: ReturnType | null = null;
  private setupComplete: boolean = false;
  private currentResponseId: string | null = null;
  private pendingFunctionCalls: Map = new Map();
  ...
  private keepaliveInterval: ReturnType | null = null;
  ...
  private recognizeStream: ReturnType | null = null;
  ...
  private sttRestartTimeout: ReturnType | null = null;
  private lastAudioReceivedTime: number = 0; // Track when we last received audio
  private sttIdleTimeout: ReturnType | null = null;
   ...
   */
  async connectWithRetry(maxRetries: number = 2): Promise {
  ...
  private async attemptReconnection(): Promise {
    ...
    try {
      ...
      if (this.reconnectAttempts  this.attemptReconnection(), 500);
      } else {
  ...
  private async sendSetupMessage(config: VoiceProviderConfig): Promise {
    try {
      if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
   ...
   */
  private async initializeSpeechToText(): Promise {
   ...
   */
  private async startRecognizeStream(): Promise {
    if (!this.speechClient || !this.sttEnabled || !this.sttActive) return;

    // Anti-loop protection: Check restart frequency
    const now = Date.now();
    if (now - this.sttLastRestartTime  5 restarts in short succession), force a cooldown
    ...
    this.recognizeStream.on('end', () => {
      ...
      if (this.sttEnabled && this.sttActive && this.speechClient && timeSinceLastAudio  {
          if (this.sttActive) {
  ...
  private handleServerContent(message: any): void {
    ...
    if (content.input_transcription || content.inputTranscription) {
      ...
      if (trimmedUserText) {
        ...
        if (trimmedUserText === this.lastEmittedUserText && timeSinceLast  w.length > 0).length;
        ...
        if (isRepetition) {
          ...
          if (this.consecutiveRepetitions  this.MAX_RECENT_PHRASES) {
          ...
          } else if (this.consecutiveRepetitions  {
