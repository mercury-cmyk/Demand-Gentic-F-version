...
let campaignPatternsCache: Map = new Map();
 ...
 */
export async function learnCampaignPatterns(): Promise> {
  const now = Date.now();
  if (campaignPatternsCache.size > 0 && now - patternsCacheAge `count(*)`,
      avgDuration: sql`round(avg(${leads.callDuration}))`,
      minDuration: sql`percentile_cont(0.1) within group (order by ${leads.callDuration})`,
      avgAiScore: sql`round(avg(cast(${leads.aiScore} as numeric)))`,
    })
    .from(leads)
  ...
  for (const stat of campaignStats) {
    ...
    const engagementPatterns: Array = [];
    for (const sample of transcriptSamples) {
...
function scoreWithLearnedPatterns(
  ...
  if (patterns.totalQualifiedLeads  s + Math.min(5, k.frequency), 0);
...
export async function runQualificationBridge(options?: {
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      // Create the lead
      const isShortDuration = (candidate.duration || 0)  0 ? `Learned pattern match: ${learnedScore.compositeScore}%` : null,
      ].filter(Boolean).join(' | ');
