...
const OPENAI_TO_GEMINI_QUICK_MAP: Record = {
  'alloy': 'Aoede',
  'echo': 'Charon',
  'fable': 'Fenrir',
  'nova': 'Kore',
...
export async function handleGeminiLiveConnection(ws: WebSocket, req: IncomingMessage) {
  ...
  function ensureTelnyxOutboundPacer(): void {
    ...
    telnyxOutboundPacer = setInterval(() => {
      try {
        ...
        if (telnyxOutboundBuffer.length  64000) {
          return;
        }
        ...
        if (framesDue  0.5 ? TELNYX_MAX_FRAMES_PER_TICK * 2 : TELNYX_MAX_FRAMES_PER_TICK;
        const framesToSend = Math.min(framesDue, framesAvailable, maxFrames);
        if (framesToSend  setB.has(w)));
  ...
  function hasRecentTransferHoldCue(windowMs = TRANSFER_HOLD_GRACE_WINDOW_MS): boolean {
    ...
    return (Date.now() - lastTransferHoldCueAt)  t.role === 'contact')
  ...
  function inEarlyVoicemailWindow(): boolean {
    ...
    return Date.now() - firstContactSpeechAt  {
...
8. Silence or background noise is NOT a prompt to speak again — just wait.`;
    ...
    setTimeout(() => {
      ...
      if (chunksReceived  console.error('[Gemini Live] Dead exchange hangup failed:', err));
        }
          ...
          if (clientStateB64) {
            try {
              ...
              callContext = {
                ...
                maxCallDurationSeconds: (() => {
                  ...
                  if (!Number.isFinite(raw) || raw = AUDIO_CHUNKS_BEFORE_SPEAKING) {
            callAnswered = true;
            ...
            if (callId) {
              setTimeout(() => {
                ...
                if (!currentMetrics || currentMetrics.audioChunksSent  console.error('[Gemini Live] Quality gate hangup failed:', err));
                  }
        ...
        case 'stop':
          ...
          if (!dispositionProcessed && callContext.callAttemptId) {
            ...
            try {
              ...
              const openingNotDelivered = callAnswered && !openingMessageSent && callDurationSec  {
      if (!geminiConnected) {
      ...
      try {
        ...
        if (outputTranscription?.text) {
          ...
          if (agentText) {
            ...
            const now = Date.now();
            if (lastTurn && lastTurn.role === 'agent' && (now - lastTurn.timestamp)  MAX_TRANSCRIPT_TURNS) transcriptTurns.shift();
        ...
        if (inputTranscription?.text) {
          ...
          if (contactText) {
            ...
            const lastTurn = transcriptTurns[transcriptTurns.length - 1];
            const now = Date.now();
            if (lastTurn && lastTurn.role === 'contact' && (now - lastTurn.timestamp) = 0.8) {
        ...
        if (response.serverContent?.modelTurn?.parts) {
          for (const part of response.serverContent.modelTurn.parts) {
            ...
            if (part.inlineData?.data) {
              ...
              if (greetingCooldownUntil > 0 && Date.now()  0) {
              ...
              if (ws.readyState === WebSocket.OPEN) {
                ...
                if (metrics.audioChunksReceived === 0 || metrics.audioChunksReceived % 50 === 0) {
                  let audioRms = 0;
                  let audioPeak = 0;
                  for (let i = 0; i  audioPeak) audioPeak = linearApprox;
                  }
          ...
          for (const call of toolCall.function_calls) {
            ...
            if (call.name === 'submit_disposition') {
              ...
              if (disposition === 'qualified_lead') {
                ...
                if (callElapsedSec  t.role === 'contact');
                if (realContactTurns.length  t.text.toLowerCase())
                  .join(' ');
              ...
              if (disposition === 'callback_requested' || disposition === 'call_back' || disposition === 'callback') {
                ...
                if (callbackElapsedSec  t.role === 'contact')
                  .map(t => t.text.toLowerCase())
                  .join(' ');
            ...
            if (call.name === 'end_call') {
              ...
              const isPrematureTermination = callDurationSeconds  t.role === 'contact')
                .slice(-3)
              ...
              if (!openingPurposeDelivered && callDurationSeconds  t.role === 'agent');
  ...
  ws.on('error', async (error: any) => {
    ...
    if (!dispositionProcessed && callContext.callAttemptId) {
      ...
      try {
        ...
        const callDurationSec = (Date.now() - metrics.startTime) / 1000;
        const hadMeaningfulConversation = callDurationSec > 60 || metrics.audioChunksSent > 500;
        const openingNotDelivered = callAnswered && !openingMessageSent && callDurationSec  null);
            } catch {}
          }
        }
      } catch (closeError) {
