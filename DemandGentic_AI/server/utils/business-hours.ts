...
function isWithinDailyWindow(currentMinutes: number, startMinutes: number, endMinutes: number): boolean {
  ...
  if (startMinutes = startMinutes && currentMinutes = startMinutes || currentMinutes  = {
    // Eastern Time
  ...
  const canadaProvinceTimezoneMap: Record = {
    // Atlantic Time
  ...
  const australiaStateTimezoneMap: Record = {
  ...
  const countryTimezoneMap: Record = {
    // United Kingdom
...
export function getNextAvailableTime(
  ...
  for (let dayOffset = 0; dayOffset  0) {
