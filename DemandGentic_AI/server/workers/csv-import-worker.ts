...
export interface CSVImportJobData {
  ...
  fieldMapping?: Record; // Custom field mapping (csvColumn -> dbColumn)
}
 ...
 */
const DEFAULT_FIELD_MAPPING: Record = {
  'full_name': 'fullName',
 ...
 */
export async function processCSVImport(job: Job): Promise {
  ...
  const accountCache = new Map(); // companyName -> accountId
   ...
   */
  async function findAccount(companyName: string): Promise {
   ...
   */
  async function mapRowToRecord(row: Record): Promise {
    const mapped: any = {
      campaignId,
      sourceType,
      createdAt: new Date(),
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Process each row
      csvParser.on('data', async (row: Record) => {
        totalRows++;
