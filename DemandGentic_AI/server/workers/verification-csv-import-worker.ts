      ...
      try {
        ...
        if (alreadyTriggered) {
          ...
          if (coverage.meetsThreshold) {
            ...
            console.log(`[VerificationCSVImportWorker] ⏸️  Workflow NOT triggered: ${coverage.percentage.toFixed(1)}% coverage  0 ? Math.ceil(coverage.total * 0.2) : 0} contacts with job titles, currently have ${coverage.withTitles})`);
          }
        }
      } catch (workflowError) {
