...
async function appendContactIdsToList(listId: string, contactIds: string[]): Promise {
...
export async function processContactsCSVImport(
  ...
  const accountCache = new Map(); // normalized name -> account ID
  let pendingListAssignments: string[] = [];

  async function flushPendingListAssignments(force = false): Promise {
    if (!listId || pendingListAssignments.length === 0) {
      return;
    }

    if (!force && pendingListAssignments.length  {
   ...
   */
  async function processBatchLegacy(records: any[]): Promise {
    ...
    try {
      await workerDb.transaction(async (tx) => {
        ...
        const uniqueContacts = new Map(); // surrogate ID -> contact data
        const { randomUUID } = await import('crypto');
        ...
        if (contactsWithoutEmail.length > 0) {
          ...
          batchResults.processedIds.push(...(insertResults.rows as Array).map(row => row.id));
        }
      });
