...
export function initializeEmailQueue(): Queue {
  ...
  const emailWorker = new Worker('email-send', async (job: Job) => {
 ...
 */
async function processEmailJob(data: EmailJobData): Promise {
 ...
 */
export async function queueEmail(jobData: EmailJobData): Promise {
  if (!emailQueue) {
