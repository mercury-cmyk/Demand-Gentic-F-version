...
function resolveLearningWindowDays(): number {
  ...
  const [emailStats, callStats, leadStats] = await Promise.all([
    db
      .select({
        delivered: sql`COUNT(CASE WHEN ${emailEvents.type} = 'delivered' THEN 1 END)::int`,
        ...
        bounced: sql`COUNT(CASE WHEN ${emailEvents.type} = 'bounced' THEN 1 END)::int`,
    ...
    db
      .select({
        ...
        connected: sql`COUNT(CASE WHEN ${callAttempts.disposition} IN (${sql.join(
          connectedDispositions.map((value) => sql`${value}`),
          sql`, `
        )}) THEN 1 END)::int`,
        qualified: sql`COUNT(CASE WHEN ${callAttempts.disposition} = 'qualified' THEN 1 END)::int`,
        notInterested: sql`COUNT(CASE WHEN ${callAttempts.disposition} = 'not_interested' THEN 1 END)::int`,
        dnc: sql`COUNT(CASE WHEN ${callAttempts.disposition} = 'dnc-request' THEN 1 END)::int`,
      })
      .from(callAttempts)
    ...
    db
      .select({
        ...
        qualified: sql`COUNT(CASE WHEN ${leads.qaStatus} IN ('approved', 'published') THEN 1 END)::int`,
      })
      .from(leads)
...
export async function getOrganizationLearningSummary(): Promise {
  const cacheTtlMs = resolveLearningCacheMs();
  if (learningCache && Date.now() - learningCache.generatedAt  0 ||
 ...
 */
export async function getOrganizationPromptSettings(): Promise {
  const now = Date.now();
  if (_settingsCache && (now - _settingsCache.fetchedAt)  {
  try {
 ...
 */
export async function getSuperOrgOIContext(): Promise {
  const now = Date.now();
  if (_superOrgOICache && (now - _superOrgOICache.fetchedAt)  {
...
## Core Agent Identity: Strategic Influence Through Genuine Value
You are not passive — you are strategically proactive. While you are always kind, warm, and human, you also understand that people often need help seeing the value in front of them. This is not selling — it is service. These principles guide your influence:

- **Anticipatory Awareness**: Before every interaction, mentally map the likely friction points. What might cause this person to disengage? What concerns might they have before you even speak? Address these proactively in how you frame your message — not reactively after they object.
- **Outcome-Driven Framing**: Every message you deliver — whether spoken or written — focuses on the outcome for the recipient, never on the features of what you are offering. You speak in their language about their world, not your product.
- **Strategic Empathy**: You do not just acknowledge feelings — you use understanding to guide the conversation toward genuinely helpful outcomes. When someone says "I'm not interested," you hear "I don't yet see why this matters to me" — and you have ONE chance to bridge that gap with something genuinely valuable.
- **Conversational Architecture**: You are always aware of where the conversation is going. Every question you ask, every statement you make, and every response you give is purposeful — it moves the conversation forward toward a clear outcome. You never ramble, never repeat, and never lose the thread.
- **Confidence Without Pressure**: You believe in what you are sharing because it genuinely helps people. This belief comes through naturally in your tone, your specificity, and your poise. You never need to pressure because the value speaks for itself when framed correctly.
- **Graceful Persistence**: When someone is on the fence, you do not give up immediately — but you also never push. You offer one thoughtful reframe or alternative that might change their perspective. If it does not, you exit with warmth and leave them better than you found them.
 ...
 */
export async function researchCompanyCore(domain: string): Promise {
 ...
 */
export async function researchMarketPosition(domain: string): Promise {
  const { searchWeb } = await import('./web-search');
  const findings: string[] = [];
  const sources: string[] = [];
 ...
 */
export async function researchCustomerIntelligence(domain: string): Promise {
  const { searchWeb } = await import('./web-search');
  const findings: string[] = [];
  const sources: string[] = [];
  const companyName = domain.split('.')[0];
 ...
 */
export async function researchNewsAndTrends(domain: string): Promise {
  const { searchWeb } = await import('./web-search');
  const findings: string[] = [];
  const sources: string[] = [];
  const companyName = domain.split('.')[0];
