...
function extractMetaContent(html: string, keys: string[]): string | null {
  for (const key of keys) {
    const pattern = new RegExp(`]+(?:name|property)=[\"']${key}[\"'][^>]*>`, "i");
...
async function fetchText(url: string, options: FetchOptions): Promise {
...
async function extractSitemapLinks(baseUrl: string, domain: string, timeoutMs: number): Promise {
  ...
  while (sitemapQueue.length > 0 && seenSitemaps.size ([^/gi)]
    ...
    for (const loc of locMatches) {
      try {
        ...
        if (url.pathname.toLowerCase().endsWith(".xml")) {
          if (seenSitemaps.size = DEFAULT_MAX_SITEMAP_URLS) break;
      } catch {
        continue;
      }
    }
