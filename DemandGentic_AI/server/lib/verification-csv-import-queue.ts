 ...
 */
export let verificationCSVImportQueue: Queue | null = null;

/**
 * Verification CSV Import Worker
 */
let verificationCSVImportWorker: Worker | null = null;
...
export function initializeVerificationCSVImportQueue(): void {
  ...
  // Create queue
  verificationCSVImportQueue = createQueue('verification-csv-import', {
    attempts: 2,
    backoff: {
      type: 'exponential',
      delay: 10000,
  ...
  verificationCSVImportWorker = createWorker(
 ...
 */
export async function addVerificationCSVImportJob(jobData: VerificationCSVImportJobData): Promise {
  if (!verificationCSVImportQueue) {
