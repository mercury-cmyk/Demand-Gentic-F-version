 ...
 * Bulk List Operation Queue
 */
export let bulkListQueue: Queue | null = null;

/**
 * Bulk List Worker
 */
let bulkListWorker: Worker | null = null;
...
export function initializeBulkListQueue(): void {
  ...
  // Create queue
  bulkListQueue = createQueue('bulk-list-operation', {
    attempts: 3,
    backoff: {
      type: 'exponential',
      delay: 3000,
 ...
 * @returns Job ID
 */
export async function addBulkListJob(jobData: BulkListJobData): Promise {
  if (!bulkListQueue) {
 ...
 */
export async function closeBulkListQueue(): Promise {
  if (bulkListWorker) {
