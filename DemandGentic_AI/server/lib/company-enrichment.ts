...
export class CompanyEnrichmentService {
   ...
   */
  static needsAddressEnrichment(contact: Partial): boolean {
   ...
   */
  static needsPhoneEnrichment(contact: Partial): boolean {
    ...
    try {
      ...
      for (const sibling of companyContacts) {
        ...
        const processPhone = (rawPhone: string | null | undefined, source: string, priority: number) => {
          ...
          if (!normalized || normalized.replace(/\D/g, '').length ();
      ...
      for (const candidate of candidates) {
        ...
        if (isHighPriority || hasConsensus) {
          ...
          if (!selectedPhone) {
            selectedPhone = group;
          } else if (group.bestPriority  selectedPhone.count) {
      ...
      const confidence = selectedPhone.bestPriority ,
    accountName: string
  ): Promise {
    try {
      // Validate inputs
...
Output valid JSON only.`;
  ...
  private static async runEnhancedWebSearch(
    ...
    try {
      ...
      // SHORT-CIRCUIT: Phone confidence  {
    try {
...
- Below 0.7 = mark as not found`);
   ...
   */
  static async checkAccountEnrichment(accountId: string): Promise {
    const { db } = await import("../db");
   ...
   */
  static async propagateToContact(contactId: string, accountId: string): Promise {
    const { db } = await import("../db");
    ...
    if (enrichmentData.address && enrichmentData.address.address1 && enrichmentData.address.city) {
      ...
      if (addressConfidence >= 0.7) {
        ...
        console.log(`[CompanyEnrichment] Skipping address propagation - confidence ${addressConfidence} = 0.55) {
        updateData.aiEnrichedPhone = enrichmentData.phone;
        updateData.phoneEnrichmentStatus = 'completed';
      } else {
        console.log(`[CompanyEnrichment] Skipping phone propagation - confidence ${phoneConfidence}  0) {
      await db
  ...
  /**
   * Store enrichment with confidence fallback (≥0.55 but  {
    const { db } = await import("../db");
    const { verificationContacts } = await import("@shared/schema");
    const { eq } = await import("drizzle-orm");
