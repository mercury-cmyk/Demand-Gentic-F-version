 ...
 */
export let capEnforcementQueue: Queue | null = null;

/**
 * Cap Enforcement Worker
 */
let capEnforcementWorker: Worker | null = null;
...
export function initializeCapEnforcementQueue(): void {
  ...
  // Create queue
  capEnforcementQueue = createQueue('cap-enforcement', {
    attempts: 3,
    backoff: {
      type: 'exponential',
  ...
  capEnforcementWorker = createWorker(
    'cap-enforcement',
    async (job) => {
      try {
 ...
 */
export async function closeCapEnforcementQueue(): Promise {
  if (capEnforcementWorker) {
