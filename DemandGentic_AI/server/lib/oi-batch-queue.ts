...
export let oiBatchQueue: Queue | null = null;
let oiBatchWorker: Worker | null = null;
...
export function initializeOiBatchQueue(): void {
  ...
  oiBatchQueue = createQueue('oi-batch', {
...
export async function addOiBatchJob(jobData: OiBatchJobData): Promise {
  if (!oiBatchQueue) {
...
export async function closeOiBatchQueue(): Promise {
  if (oiBatchWorker) {
