...
const STRICT_US_ONLY_CAMPAIGN_NAMES = new Set(
  [
...
const STRICT_US_ONLY_CAMPAIGN_IDS = new Set(
...
const campaignEmptyPoolBackoffUntil = new Map();

async function getConcurrencyLimits(): Promise {
  const now = Date.now();
  if (now - _concurrencyLastFetched  {
  const now = Date.now();
  if (_cachedHasActivePoolNumbers !== null && now - _activePoolLastFetched `count(*)::int` })
      .from(telnyxNumbers)
...
function shouldEnforceStrictUsOnly(campaign: {
  ...
  const aiSettings = campaign.aiAgentSettings && typeof campaign.aiAgentSettings === 'object'
    ? campaign.aiAgentSettings as Record
    : null;
...
async function teardownAiCampaignOrchestrator(reason: string): Promise {
 ...
 */
async function getInProgressCount(campaignId: string): Promise {
...
async function getGlobalInProgressCount(): Promise {
  const result = await db.select({ count: sql`count(*)::int` })
    .from(campaignQueue)
 ...
 */
async function startupResumeStuckCalls(): Promise {
  try {
 ...
 */
async function resetStuckItems(): Promise {
  try {
...
async function getQueuedItems(
  ...
  const result = await db.execute(sql`
    ...
    WHERE cq.campaign_id = ${campaignId}
      ...
      AND (cq.next_attempt_at IS NULL OR cq.next_attempt_at = CURRENT_DATE
          AND (
 ...
 */
async function setOrchestratorStallReason(campaignId: string, reason: string | null): Promise {
  try {
 ...
 */
async function processCampaign(campaignId: string, options?: ProcessCampaignOptions): Promise {
  ...
  const outsideHoursUpdates: Array = [];

  // Track timezone stats for logging
  const timezoneStats = new Map();

  // Track rejected countries for debugging
  const rejectedCountries = new Map();
  ...
  for (const item of queueItems) {
    ...
    if (strictUsOnly && !isUnitedStatesCountry(effectiveCountry)) {
      ...
      if (countryNotEnabled  direct > HQ, validated against contact country
    ...
    if (!phone) {
      ...
      if (noPhone  a.priority - b.priority);
  ...
  const isColdStart = (Date.now() - orchestratorBootTime)  COLD_START_MAX_CALLS) {
  ...
  for (let i = 0; i  {
    ...
    if (i + PARALLEL_CALL_BATCH_SIZE  setTimeout(resolve, DELAY_BETWEEN_CALLS_MS));
    }
  }
 ...
 */
async function orchestratorTick(): Promise {
  try {
  ...
  try {
    ...
    // Create queue
    orchestratorQueue = createQueue('ai-campaign-orchestrator', {
      attempts: 1,
      removeOnComplete: 10,
      removeOnFail: 50,
    });
    ...
    orchestratorWorker = createWorker(
      'ai-campaign-orchestrator',
      async (job: Job) => {
 ...
 */
export async function triggerCampaignReplenish(campaignId: string): Promise {
