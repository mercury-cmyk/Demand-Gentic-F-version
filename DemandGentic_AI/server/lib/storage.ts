 ...
 */
export async function readFromGCS(key: string): Promise {
  const file = bucket.file(key);
 ...
 */
export async function s3ObjectExists(key: string): Promise {
  const file = bucket.file(key);
 ...
 */
export async function getPublicUrl(key: string): Promise {
  const publicBase = process.env.GCS_PUBLIC_BASE || process.env.S3_PUBLIC_BASE;
