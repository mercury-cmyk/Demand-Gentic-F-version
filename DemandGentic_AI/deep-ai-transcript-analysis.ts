...
async function main() {
  ...
  // Analyze each call
  const missedQualified: Array}> = [];
  const correctQualified: Array}> = [];
  const voicemailsWithLeads: Array}> = [];
  const possibleMisclassified: Array}> = [];
