...
- [ ] Test SIP port (from external machine):
  ```bash
  timeout 5 bash -c 'cat  /dev/udp//5060' 2>/dev/null && echo "SIP OK" || echo "SIP FAILED"
  ```
- [ ] Test TURN port:
  ```bash
  nc -u -w1  3478 :5000/api/health
  curl http://:5000/api/sip/stats
  ```
...
- [ ] Regular image updates: `docker pull `
...
- [ ] View instance logs:
  ```bash
  gcloud logging read "resource.type=gce_instance AND resource.labels.instance_id=" --limit=50
  ```
- [ ] Filter by component:
  ```bash
  gcloud logging read "resource.labels.instance_id= AND jsonPayload.component=drachtio" --limit=20
  ```
...
- [ ] Set up continuous health checks:
  ```bash
  while true; do
    curl -s http://:5000/api/health | jq .
    sleep 60
  done
  ```
...
- [ ] Run basic test:
  ```bash
  sipp -u user1 -s "sip:user@:5060" -r 10 
  ```
- [ ] Monitor during test:
  - [ ] CPU usage: `top`
  - [ ] Memory: `free -h`
...
3. **Restore from Backup**: Restore database/configs
   ```bash
   gcloud sql backups restore  --instance=demandgentic-postgres
   ```
