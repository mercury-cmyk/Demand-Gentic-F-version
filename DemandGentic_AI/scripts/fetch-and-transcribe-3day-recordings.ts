...
async function listTelnyxRecordings(): Promise {
...
async function findLinkedRecord(rec: TelnyxRecording): Promise {
  try {
...
async function downloadRecording(wavUrl: string, recId: string): Promise {
...
async function processRecording(rec: TelnyxRecording, idx: number, total: number): Promise {
...
async function findDbRecordsMissingTranscripts(): Promise> {
  const since = new Date();
  since.setDate(since.getDate() - DAYS_BACK);
  ...
  const leadRows = await pool.query(
    `SELECT id, recording_url, recording_s3_key, telnyx_call_id
     ...
     WHERE created_at >= $1
       ...
       AND (transcript IS NULL OR length(transcript)  $2
  ...
  const csRows = await pool.query(
    `SELECT id, recording_url, recording_s3_key, telnyx_call_id
     ...
     WHERE started_at >= $1
       AND (recording_url IS NOT NULL OR recording_s3_key IS NOT NULL)
       AND (ai_transcript IS NULL OR length(ai_transcript)  $2
...
async function transcribeMissingDbRecords(records: Array) {
  let count = 0;
