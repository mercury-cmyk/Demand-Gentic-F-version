...
async function resolveAudioUrl(
  ...
  if (candidate.toNumber || candidate.fromNumber) {
    try {
      ...
      async function searchBy(field: "to" | "from", value: string): Promise {
...
function mapSpeaker(raw: unknown, dynamicMap: Map): string {
...
async function diarizeFromUrl(audioUrl: string): Promise {
...
async function transcribeStrictTwoWay(
  ...
  if (!strictSatisfied && STRICT_MODE) throw new Error("strict two-way check failed:  {
...
async function updateCallSession(candidate: Candidate, structured: StructuredTranscript): Promise {
  const formatted = formatTwoWayTranscript(structured);
  await pool.query(
...
async function processOne(candidate: Candidate, index: number, total: number): Promise {
