  ...
  try {
    ...
    const { rows: leadsToTranscribe } = await pool.query(`
      ...
      WHERE l.qa_status = 'new'
        ...
        AND (l.transcript IS NULL OR length(l.transcript)  new Promise(resolve => setTimeout(resolve, ms));
    ...
    async function processLead(lead: any, index: number) {
      ...
      if (lead.recording_s3_key || lead.session_recording_s3_key || lead.alt_session_s3_key) {
        ...
        const gcsBucket = process.env.GCS_BUCKET || 'demandgentic-prod-storage-2026';
        ...
        if (transcript && transcript.trim().length > 10) {
          ...
          if (lead.call_session_id) {
            await pool.query(`
              ...
              SET ai_transcript = $1 
              WHERE id = $2 AND (ai_transcript IS NULL OR length(ai_transcript)  DONE (${transcript.trim().length} chars)`);
        } else {
