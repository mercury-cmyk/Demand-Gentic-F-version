...
type ClientAccountRow = {
  id: string;
  name: string;
  visibility_settings: Record | null;
};
...
function getDisposition(lead: LeadRow): string | null {
  const analysis = (lead.ai_analysis || {}) as Record;
  const direct = typeof analysis.disposition === "string" ? analysis.disposition : null;
  if (direct) return direct;

  const nested = analysis.dispositionReview as Record | undefined;
...
async function main() {
  ...
  try {
    ...
    const accountsRes = await client.query(accountsQuery, accountParams);
    ...
    const existingMockCallsRes = await client.query(
      `
      SELECT
        id,
        client_account_id,
    ...
    const existingMockByLeadAndClient = new Map();
    ...
    const existingQaByClientAndMock = new Map();
    if (existingMockCallIds.length > 0) {
      const existingQaRes = await client.query(
        `
        SELECT
          id,
          content_id,
    ...
    for (const account of accounts) {
      const currentVisibility = (account.visibility_settings || {}) as Record;
      ...
      for (const leadId of existingLeadIds) {
        ...
        if (mockCallId) {
          ...
          if (options.execute) {
            const aiAnalysisPayload: Record = {
            ...
            const insertedMockRes = await client.query(
              `
        ...
        if (!qa) {
          if (options.execute) {
            const insertedQaRes = await client.query(
              `
