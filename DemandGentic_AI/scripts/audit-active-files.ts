...
function traverseReachable(graph: ImportMap, entrypoints: string[]): Set {
...
function extractScriptEntrypointsFromPackageJson(): string[] {
  ...
  try {
    const parsed = JSON.parse(fs.readFileSync(pkgPath, "utf8")) as { scripts?: Record };
...
function buildMarkdownReport(report: any): string {
  ...
  lines.push("");
  lines.push(`-  declarations in App.tsx: ${report.frontendRoutes.declaredCount}`);
