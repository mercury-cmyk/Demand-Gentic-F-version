...
const TABLE_REQUIREMENTS: Record = {
  campaign_queue: {
...
function findMissing(expected: string[], actual: Set): string[] {
  return expected.filter((col) => !actual.has(col));
}

async function getTableColumns(pool: Pool, tableName: string): Promise> {
