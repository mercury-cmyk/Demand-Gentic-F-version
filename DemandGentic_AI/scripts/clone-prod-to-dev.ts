...
async function getTables(client: InstanceType): Promise {
...
async function getDependencies(client: InstanceType): Promise {
  const result = await client.query(
    `
...
async function getColumns(client: pgPkg.PoolClient, table: string): Promise {
  const result = await client.query(
    `
      SELECT column_name
...
async function getCount(client: pgPkg.PoolClient, table: string): Promise {
  const result = await client.query(`SELECT COUNT(*)::bigint AS c FROM ${qt("public", table)}`);
  return BigInt(result.rows[0].c);
}

async function resetSequences(targetClient: pgPkg.PoolClient): Promise {
  const serialCols = await targetClient.query(
    `
  ...
  for (const row of serialCols.rows) {
    ...
    const maxRes = await targetClient.query(
    ...
    if (missingTables.length > 0) {
      throw new Error(
        `Target is missing ${missingTables.length} source tables: ${missingTables.join(", ")}. ` +
          "Initialize schema first (e.g. DATABASE_URL= npm run db:push).",
      );
    }
