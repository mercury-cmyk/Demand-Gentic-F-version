...
async function getActiveCalls() {
  ...
  let query = db
    .select({
      ...
      contactName: sql`COALESCE(${callSessions.contactName}, 'Unknown')`,
...
async function getCallStats() {
  ...
  const [stats] = await db
    .select({
      ...
      voicemail: sql`COUNT(CASE WHEN ${dialerCallAttempts.disposition} = 'voicemail' OR ${dialerCallAttempts.voicemailDetected} = true THEN 1 END)::int`,
      notInterested: sql`COUNT(CASE WHEN ${dialerCallAttempts.disposition} = 'not_interested' THEN 1 END)::int`,
      dnc: sql`COUNT(CASE WHEN ${dialerCallAttempts.disposition} = 'do_not_call' THEN 1 END)::int`,
    })
