...
## Timing SLAs
- Voicemail detection decision: `<=3000ms` from first responder audio.
- Identity-confirmed to purpose start: `<=700ms`.
- Direct identity/connectivity question acknowledgment: `<=1000ms`.
- Maximum continuous dead air: `<=1200ms`.
- Opening repeat count limit: `1` (second repeat is defect).

## Voicemail Cues (High Priority)
- "leave a message"
- "after the beep" / "after the tone"
- "the person you are calling is not available"
- "mailbox is full"
- audible long beep after greeting cadence

## Pseudocode
```ts
onCallAnswered();
enter(S1_CLASSIFY_RESPONDER, timeoutMs = 3000);

if (detectVoicemailCue()) {
  enter(SX_VOICEMAIL_EXIT);
  runVoicemailPolicy();
  endCall();
}

if (detectHumanSpeech()) {
  enter(S2_IDENTITY_REQUEST);
  speak("May I speak with {contactName}?");
}

onIdentityConfirmedOrCorrected(() => {
