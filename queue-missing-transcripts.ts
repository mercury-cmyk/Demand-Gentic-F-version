...
async function regenerateMissingTranscripts() {
  ...
  try {
    ...
    const allCalls = [
      ...(callSessionsResult.rows as any[]),
      ...(dialerResult.rows as any[]),
    ];

    console.log(`✅ Found ${allCalls.length} calls with missing transcriptions`);
    ...
    // Import services needed for transcription
    const { searchRecordingsByDialedNumber } = await import("./server/services/telnyx-recordings.ts");
    const { transcribeFromRecording } = await import("./server/services/deepgram-postcall-transcription.ts");
    const { attemptFallbackTranscription } = await import("./server/services/transcription-reliability.ts");
    ...
    console.log(`🔄 Processing transcriptions with Telnyx phone lookup...\n`);
    ...
    for (const call of allCalls) {
      try {
        ...
        if (processed % 100 === 0) {
          console.log(`   ✓ Processed ${processed}/${allCalls.length} | Succeeded: ${succeeded}, Failed: ${failed}`);
        }

        if (call.source === 'call_sessions') {
          // Search Telnyx
          const phoneNumber = call.phone;
          if (!phoneNumber || !call.started_at) {
            failed++;
            continue;
          }

          const searchStart = new Date(call.started_at);
          searchStart.setMinutes(searchStart.getMinutes() - 30);
          const searchEnd = new Date(call.started_at);
