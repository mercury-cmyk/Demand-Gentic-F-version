...
export default function AdminTodoBoardPage() {
  ...
  const [aiPlan, setAiPlan] = useState<AiTaskPlanResponse | null>(null);
  ...
  const generateAiPlanMutation = useMutation({
    mutationFn: async (payload: { objective?: string; maxSuggestions?: number }) => {
      ...
      return parseApiJson<AiTaskPlanResponse>(response, "/api/admin/tasks/ai-plan");
    },
    onSuccess: (result) => {
      setAiPlan(result);
      toast({
  ...
  const applyAiPlanMutation = useMutation({
    mutationFn: async (payload: { suggestions: AiTaskSuggestion[] }) => {
      ...
      return parseApiJson<{ createdCount: number; createdTasks: AdminTask[] }>(response, "/api/admin/tasks/ai-plan/apply");
    },
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/tasks"] });
      toast({
  ...
  return (
    <PageShell
      ...
      description="AI-powered, role-aware team board to detect issues, prioritize work, and assign tasks with clarity."
      breadcrumbs={[
        { label: "Home", href: "/" },
            ...
            <div className="grid gap-3 lg:grid-cols-[1fr_auto_auto]">
              ...
              {canManageBoard && (
                <Button
                  ...
                  disabled={!aiPlan || aiPlan.suggestions.length === 0 || applyAiPlanMutation.isPending}
                  data-testid="todo-ai-apply"
                >
            ...
            {aiPlan ? (
              <div className="space-y-4">
                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                  <Badge variant="outline">Engine: {aiPlan.engine === "openai" ? "OpenAI" : "Heuristic fallback"}</Badge>
                  <span>Generated {new Date(aiPlan.generatedAt).toLocaleString()}</span>
                </div>

                {aiPlan.assignmentSummary.length > 0 && (
                  <div className="rounded-md border p-3">
                    <p className="text-sm font-medium mb-2">Proposed Workload Distribution</p>
                    <div className="flex flex-wrap gap-2">
                      {aiPlan.assignmentSummary.map((entry) => (
                        <Badge key={entry.assigneeName} variant="secondary" className="text-xs">
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    {aiPlan.issues.length === 0 ? (
                      ...
                      aiPlan.issues.map((issue, index) => (
                        <div key={`${issue.issue}-${index}`} className="rounded-md border p-3 space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <Badge variant="outline" className={cn("text-[11px]", PRIORITY_BADGE_CLASS[issue.priority])}>
                  ...
                  <div className="space-y-2">
                    <p className="text-sm font-medium">Suggested Task Assignments</p>
                    {aiPlan.suggestions.length === 0 ? (
                      <p className="text-xs text-muted-foreground">No task recommendations generated.</p>
                    ) : (
                      aiPlan.suggestions.map((suggestion, index) => (
                        <div key={`${suggestion.title}-${index}`} className="rounded-md border p-3 space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            <Badge variant="outline" className={cn("text-[11px]", PRIORITY_BADGE_CLASS[suggestion.priority])}>
                          ...
                          <p className="text-xs text-muted-foreground">{suggestion.issue}</p>
                          {suggestion.rationale && (
                            <p className="text-xs text-muted-foreground">Why: {suggestion.rationale}</p>
                          )}
                        </div>
                      ))
                    )}
