...
export function useUnifiedContacts(filterValues?: FilterValues, page: number = 1, limit: number = 100) {
  ...
  const { data, ...rest } = useQuery<PaginatedResponse<UnifiedContact>>({
  ...
  const createMutation = useMutation({
    mutationFn: (newContact: Omit<UnifiedContact, 'id'>) =>
...
export function useUnifiedAccounts(filterValues?: FilterValues, page: number = 1, limit: number = 100) {
  ...
  const { data, ...rest } = useQuery<PaginatedResponse<UnifiedAccount>>({
  ...
  const createMutation = useMutation({
    mutationFn: (newAccount: Omit<UnifiedAccount, 'id'>) =>
