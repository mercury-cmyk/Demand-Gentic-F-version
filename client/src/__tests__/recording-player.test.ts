...
describe('Recording Player — Auto-Retry', () => {
  it('auto-retries once on first playback error via warmAndReload', () => {
    ...
    if (retryCount < maxAutoRetries) retryCount++;
  ...
  it('warmAndReload calls POST /recording-link then bumps cacheBust', () => {
    const recordingId = 'session-123';
...
describe('Recording Player — Error UI', () => {
  it('error state shows retry, refresh, resync (if needed), and open-in-tab', () => {
    ...
    expect(errorState.retryCount < errorState.maxRetries).toBe(true); // retry visible
    expect(!errorState.telnyxRecordingId).toBe(true); // resync visible
  });
