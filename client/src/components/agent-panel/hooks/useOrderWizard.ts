...
export function useOrderWizard(
  ...
  const handleFileUpload = useCallback(async (
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        if (!uploadUrl || !key) throw new Error(`Invalid upload URL for "${file.name}"`);
        ...
        newFiles.push({ name: file.name, key, type: file.type || 'application/octet-stream' });
      }
  ...
  useEffect(() => {
    if (!isActive || config.volume <= 0) return;
    const timer = setTimeout(() => estimateMutation.mutate(), 500);
  ...
  const updateConfig = useCallback((updates: Partial<OrderConfiguration>) => {
