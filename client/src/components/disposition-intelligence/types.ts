...
export const DISPOSITION_COLORS: Record<DispositionType, string> = {
...
export interface AgentPerformanceResponse {
  openingAnalysis: {
    ...
    commonOpeningIssues: Array<{ issue: string; frequency: number }>;
  };
  engagementMetrics: {
    avgEngagementScore: number | null;
    avgClarityScore: number | null;
    avgEmpathyScore: number | null;
    interruptionPatterns: Array<{ type: string; count: number }>;
  };
  objectionHandling: {
  ...
  closingAnalysis: {
    avgScore: number | null;
    closingIssues: Array<{ issue: string; frequency: number }>;
  };
  flowCompliance: {
    avgScore: number | null;
    topMissedSteps: Array<{ step: string; frequency: number }>;
    topDeviations: Array<{ deviation: string; frequency: number }>;
  };
  bestVsWorst: {
    best: Array<{
      callSessionId: string;
...
export interface CampaignAnalysisResponse {
  ...
  qualificationAnalysis: {
    ...
    topMissedTalkingPoints: Array<{ point: string; frequency: number }>;
  };
  dispositionBreakdown: Record<
    string,
    { count: number; avgDuration: number; avgQuality: number | null }
  >;
