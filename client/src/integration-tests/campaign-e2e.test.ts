...
describe('Campaign Creation E2E Flow', () => {
  ...
  it('should complete full campaign creation flow', async () => {
    ...
    renderWithProviders(<CampaignWizard />);
    ...
    await user.type(
      emailContentArea,
      '<h1>Hello {{firstName}}</h1><p>Check out our offer!</p>'
    );
  ...
  it('should show preview of email', async () => {
    ...
    await user.click(emailContent);
    await user.type(emailContent, '<h1>Preview Content</h1>');
  ...
  it('should validate email addresses', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      const nextBtn = screen.queryAllByRole('button', {
        name: /continue|next/i
      })[0];
      if (nextBtn) {
