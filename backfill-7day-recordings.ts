...
async function backfillRecordingsToGCS() {
  ...
  for (const row of missing.rows as any[]) {
    ...
    try {
      ...
      const isWav = downloadUrl.includes('.wav');
      const extension = isWav ? 'wav' : 'mp3';
      const contentType = isWav ? 'audio/wav' : 'audio/mpeg';
      const s3Key = `${RECORDING_PREFIX}/${leadId}.${extension}`;

      console.log(`    ⬆️  Uploading ${(buffer.length / 1024).toFixed(0)} KB to GCS: ${s3Key}`);
      await uploadToS3(s3Key, buffer, contentType);

      // Update lead with GCS key and fresh recording URL
      await db.execute(sql`
        UPDATE leads
        SET recording_s3_key = ${s3Key},
            recording_url = ${downloadUrl},
            updated_at = NOW()
        WHERE id = ${leadId}
      `);

      console.log(`    ✅ Done: ${row.contact_name} | ${s3Key}`);
      downloaded++;
    } catch (err: any) {
  ...
  console.log(`\n=== RESULTS ===`);
  console.log(`  Downloaded & stored in GCS: ${downloaded}`);
  console.log(`  Copied from call_sessions: ${copiedFromSession}`);
  console.log(`  Failed: ${failed}`);
  console.log(`  No URL available: ${noUrl}`);
  console.log(`  Total processed: ${missing.rows.length}`);
}

backfillRecordingsToGCS().catch(console.error).finally(() => process.exit(0));
